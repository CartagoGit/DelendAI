...
const CURRENT = 'docs/mcp-vertex/proposals/done/feats/f00299-token-budgets.md';
...
describe('staleSelfPathsFor', () => {
	it('finds a self-path left behind by a move', () => {
		...
		const text =
			'- **Files**: `docs/mcp-vertex/proposals/review/f00299-token-budgets.md`\n';
	...
	it('never touches a reference to a DIFFERENT proposal', () => {
		...
		const text =
			'related: `docs/mcp-vertex/proposals/ready/feats/f00300-other.md`\n';
		expect(staleSelfPathsFor(text, CURRENT)).toEqual([]);
	});
