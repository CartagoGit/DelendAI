...
for (const sliceId of ['S1', 'S2']) {
	...
	console.log(`\n=== ${sliceId} approve ===`);
	const note = {
		S1: 'Opt-out knob works via env var MCP_VERTEX_RELAX_EXACT_OPTIONAL; tsconfig.relax.json overrides the flag; typecheck.script.ts dispatches based on env.',
		S2: 'AGENT-BOOTSTRAP.md documents the trade-off (3-7% LLM fix-cycle cost) and the opt-out procedure.',
	}[sliceId]!;
	console.log(
		(
			await handler!({
