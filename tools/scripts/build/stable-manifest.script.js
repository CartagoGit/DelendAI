...
const isSortedByName = (manifest) =>
	manifest.tools.every(
		(tool, index, tools) =>
			index === 0 || tools[index - 1].name.localeCompare(tool.name) <= 0,
	);
const assertCanonicalManifest = (manifest, packageVersion) => {
...
const readExistingManifest = (abs) => {
...
const pickGeneratedAt = (existingManifest, packageVersion) => {
	if (
...
const runVerifier = (abs, previous) => {
