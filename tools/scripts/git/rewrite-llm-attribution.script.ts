...
const LLM_PHRASES: ReadonlyArray<readonly string[]> = [
	...
	["claude", "fable", "5"], ["claude", "minimax", "m3"], ["claude", "chat"],
	["claude", "m3"], ["claude", "4"], ["claude", "5"], ["claude", "3"],
	["minimax", "m3"], ["minimax", "opus"], ["minimax", "sonnet"],
	["minimax", "pro"], ["minimax", "mini"],
	["gpt", "3"], ["gpt", "4"], ["gpt", "5"], ["gpt", "4o"], ["gpt", "5o"],
	["chatgpt"],
	["gemini", "1"], ["gemini", "2"], ["gemini", "3"], ["gemini", "pro"],
	["gemini", "ultra"], ["gemini", "flash"],
	["copilot", "minimax", "m3"], ["copilot", "gpt"], ["copilot", "claude"],
	["codex", "gpt", "5"], ["codex", "gpt"], ["codex", "minimax"],
...
const LLM_DOMAINS: ReadonlyArray<string> = [
	"anthropic.com", "minimax.ai", "minimax.local",
	"users.noreply.github.com", "copilot@local", "copilot@anthropic",
];

const tokenize = (s: string): readonly string[] =>
	s.toLowerCase().split(/[^a-z0-9]+/u).filter((t) => t.length > 0);

const matchesLlmPhrase = (tokens: readonly string[]): readonly string[] | null => {
...
const matchesLlmDomain = (value: string): string | null => {
	...
	for (const d of LLM_DOMAINS) {
		const re = new RegExp(`(?:^|[^a-z0-9])@?${d.replace(/\./gu, "\\.")}`, "iu");
...
const trailerIsLlm = (line: string): boolean => {
	...
	return matchesLlmPhrase(tokens) !== null || matchesLlmDomain(value) !== null;
};

const runGit = (cwd: string, args: readonly string[]): { ok: boolean; stdout: string; stderr: string } => {
	const res = spawnSync("git", ["--no-pager", ...args], { cwd, encoding: "utf8" });
...
const main = (): void => {
	if (BACKUP) {
		...
		if (!remote.ok) {
			console.error("rewrite-llm-attribution: cannot determine origin URL");
			process.exit(2);
		}
		const url = remote.stdout.trim();
		const stamp = new Date().toISOString().replace(/[:.]/gu, "-");
		const target = join(process.cwd(), `..`, `mcp-vertex-backup-${stamp}.git`);
		console.log(`Cloning mirror to ${target}…`);
		const res = spawnSync("git", ["clone", "--mirror", url, target], { encoding: "utf8" });
		if (res.status !== 0) {
	...
	const log = runGit(process.cwd(), [
		"log", "--all", "--format=%H%n%B%n---END---",
	]);
	if (log.stdout.length === 0) {
		console.error("git log returned no output:", log.stderr || "(no stderr)");
		process.exit(2);
	}
	const commits = log.stdout.split("---END---").map((c) => c.trim()).filter(Boolean);
	...
	for (const c of commits) {
		...
		if (offending.length > 0) {
			...
			if (sampleCommits.length < 5) {
				sampleCommits.push(`  ${hash.slice(0, 12)}: ${offending.length} trailer(s)`);
			}
		}
	}
	report("Trailer scan", (
...
`;
		...
		const fr = spawnSync("git", [
			"filter-repo", "--force",
			"--mailmap", ".mailmap",
			"--message-callback", `cat > /tmp/msg-filter-input; python3 ${callback} < /tmp/msg-filter-input`,
		], { encoding: "utf8" });
		if (fr.status !== 0) {
		...
		console.warn("git-filter-repo not available; using git filter-branch (slower).");
		const fb = spawnSync("git", [
			"filter-branch", "-f", "--msg-filter", `cat > /tmp/msg; node -e "
