#!/usr/bin/env bun
/**
 * check-workflow-invariants.script.ts — the workflow checks itself.
 *
 ...
 * the remedy attached to each. `delendai work doctor` (and this script)
 * answers in one screen whether the model is actually holding, and if
 * not, which promise broke and what fixes it.
 *
...
import type {
	...
	IInvariantResult,
	IInvariantScope,
} from './check-workflow-invariants.interface';
...
if (import.meta.main) {
	...
	// `--show-toplevel` answers the WORKTREE this runs from, and the
	...
	// shared checkout — which is what makes `doctor` tell the truth when
	// an agent runs it from inside its own worktree.
	const commonDir = git(process.cwd(), [
		'rev-parse',
	...
	const onlyForge = process.argv.includes('--forge');
	const report = checkWorkflowInvariants({
		root,
		policy,
		...(onlyForge ? { scopes: ['forge' as const] } : {}),
	});
