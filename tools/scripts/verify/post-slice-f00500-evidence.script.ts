...
const grepInTree = (pattern: string, path: string, includeFilter?: string): number => {
...
checks.push({
	...
	fn: () => {
		if (!existsSync("plugins/commit-policy/src/lib/contracts/options.ts")) return false;
		const text = readFileSync("plugins/commit-policy/src/lib/contracts/options.ts", "utf8");
...
checks.push({
	...
	fn: () => {
		...
		return /no-llm-attribution:/u.test(text) && /no-llm-attribution\.script\.ts/u.test(text);
	},
});
...
if (failed === 0) {
	...
	console.log(`f00500: ${failed} failure${failed === 1 ? "" : "s"} across ${checks.length} checks.`);
	process.exit(1);
}
