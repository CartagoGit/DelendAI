...
export const buildOperationalEventLogKnowledge = (
	...
	return [
		...
		'## Tools',
		'',
		'- `' + p + '_query` — filter by `since` / `until` / `kind` / `agent` / `taskId` / `outcome` / `severity` (>=, inclusive) / `incidentType`, cursor pagination.',
		'- `' + p + '_tail` / `' + p + '_errors_tail` — newest N, optionally filtered. `errors_tail` reads the curated error stream only and includes `meta` by default (the fast path for "where do I look for bugs").',
		'- `' + p + '_correlate` — chronological chain for one `taskId` or `agent` with gap detection.',
		'- `' + p + '_subscribe` — SSE-friendly projection of the recent error events.',
		'- `' + p + '_log` — write-side: any peer plugin / agent records a structured incident. `severity` (defaults to `warning`), `incidentType` (must match `^[a-z][a-z0-9-]{0,63}$`), `message` (required), `files?`, `agent?`, `context?`. Lands in the main timeline (and the error stream when `severity` is `error` or above).',
		'- `' + p + '_search` — full-text or regex search across `summary` / `error.message`+`error.stack` / `args` / `result` / `all`. Use this when `query` cannot express what you want (substring inside an error message, regex over a stack, etc.).',
		...
		'- `' + p + '_redact_test` — audit the redactor against a sample payload.',
		'',
		'## Cross-plugin helper',
		'',
		'Other plugins can emit structured incidents through `ctx.logs.log({ severity, incidentType, message, files?, agent?, context? })`. The helper is the same writer `' + p + '_log` uses, so an entry is queryable by `query` / `search` / `incidents`. The helper is conditional on the `logs` plugin being loaded; null-check before calling (`ctx.logs?.log(...)`).',
		'',
		'## Recipe — "what is broken right now?"',
		'',
		'1. Call `' + p + '_incidents { minCount: 2 }` for the recurring cluster view.',
		'2. For a top cluster, take the `sampleError` and pass it to `' + p + '_search { pattern: <sampleError>, isRegex: true, scope: "error" }` to see every occurrence with full context.',
		'3. Use `' + p + '_correlate { taskId: <toolName> }` to see what happened before/after the first occurrence.',
		'4. For an ad-hoc diagnostic, call `' + p + '_log { severity: "critical", incidentType: "lock-conflict", message: "...", context: { ... } }` so a future `' + p + '_search` can find it.',
		'',
		'## Retention',
		'',
