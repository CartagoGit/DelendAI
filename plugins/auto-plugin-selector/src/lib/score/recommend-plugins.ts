/**
 * recommend-plugins.ts — f00142 S1 + r00025 S1-S4: the pure plugin-fit
 * scorer that combines project-shape signals with cost, latency and
 * historical-success signals into a single ranked `IPluginFit[]`.
 *
 ...
 *  - **Unmatched penalty**: every tag on the candidate that matched
 *    nothing contributes `-0.05` (mild so it doesn't crush a
 *    candidate with mostly-fitting tags and one stray tag).
 *
 * r00025 cost / latency / success signals:
 *
 *  - **Permission-risk penalty**: subtract the sum of
 *    `PERMISSION_RISK_WEIGHTS` across the candidate's permission
 *    categories, scaled by `weights.permissionRisk`.
 *  - **Token-tax penalty**: `scoreTokenTax()` returns `1 = cheap` /
 *    `0 = hard-cap break`; we apply `(1 - score) * weights.tokenTax`
 *    so expensive plugins drag the score down. Neutral 0.5 when no
 *    `tokenBudget` is declared (cold start).
 *  - **Latency-tax penalty**: same shape as token-tax, fed from the
 *    host-supplied `options.usageAggregations`. Neutral 0.5 on cold
 *    start.
 *  - **Historical-success bonus**: `scoreHistoricalSuccess()` returns
 *    a `0..1` benefit; we multiply it by `weights.historicalSuccess`
 *    and add to the score. Neutral 0.5 on cold start.
 *
 * Aggregation:
 *  - The `match` raw score is multiplied by `weights.match`.
 *  - All five signals are summed into a single `utility` per
 *    candidate. The top utility drives normalization (top = 1.0).
