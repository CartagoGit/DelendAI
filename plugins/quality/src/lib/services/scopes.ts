...
const packageManager = async (reader: IFileReader): Promise<string> => {
	if ((await reader.exists('bun.lock')) || (await reader.exists('bun.lockb')))
		return 'bun run';
	if (await reader.exists('pnpm-lock.yaml')) return 'pnpm';
	if (await reader.exists('yarn.lock')) return 'yarn';
	return 'npm run';
};

const QUALITY_ROLES = ['lint', 'typecheck', 'test', 'build'] as const;

const fromScripts = async (reader: IFileReader): Promise<IScopeMap> => {
	...
	try {
		const scripts =
			...
			{};
		const pm = await packageManager(reader);
