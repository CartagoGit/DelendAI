...
/**
 * The default set of core documents whose stable hashes anchor the
 * round context. x00053: the proposals registry index moved under
 * `<cacheDir>/proposals/index.json` (x00052); the default now points
 * at the cache path so a host that does not inject its own `coreDocs`
 * hashes a real file, not a frozen `'rh-missing'`.
 *
 * - `README.md` — top-level workspace README.
 * - `.cache/delendai/proposals/index.json` — proposal registry
 *   (regenerated by `proposals_sync_proposals`).
 *
 * Hosts that need the full historical "core four" (Copilot
 * instructions, the proposal-workflow skill, …) inject the extra
 * docs through the `coreDocs` parameter of
 * `computeCoreDocHashes` — the production wiring in
 * `plugins/proposals/src/index.ts:222` does this.
 */
