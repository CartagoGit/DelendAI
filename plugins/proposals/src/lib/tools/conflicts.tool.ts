...
export const conflictsOutputSchema = z.object({ conflicts: z.array(z.object({ entityType: z.enum(['proposal', 'plan', 'slice']), entityUid: z.string(), currentRevision: z.number().int(), expectedRevision: z.number().int() })), checkedAt: z.number().int().nonnegative() });
