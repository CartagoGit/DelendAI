...
export const dbDiffInputSchema = z.object({
	fromSha: z.string().min(1),
	untilSha: z.string().min(1),
});
export const dbDiffOutputSchema = z.object({
	fromSha: z.string(),
	untilSha: z.string(),
	entries: z.array(
		z.object({
			path: z.string(),
			fromDigest: z.string().nullable(),
			untilDigest: z.string().nullable(),
			change: z.enum(['added', 'removed', 'changed', 'unchanged']),
		}),
	),
});
export interface IDbDiffToolOptions
	extends Pick<IDbDiffInput, 'proposalsDirAbs'> {
	readonly namespacePrefix?: string;
}
export const runDbDiffTool = (
	options: IDbDiffToolOptions,
	args: z.infer<typeof dbDiffInputSchema>,
) =>
	diffProposalsDb({
		proposalsDirAbs: options.proposalsDirAbs,
		fromSha: args.fromSha,
		untilSha: args.untilSha,
	});
export const buildDbDiffToolRegistration = (
	options: IDbDiffToolOptions,
): IToolRegistration => ({
	id: DB_DIFF_REGISTRATION_ID,
