...
type ICloseSliceThrownError = Error & {
  readonly kind?:
    "validation-error" | "quality-failed" | "peer-review-required";
...
export const REVIEW_EVIDENCE_SCHEMA = z
  .object({
    ...
    testsPassing: z.number().int().min(1, "evidence.testsPassing must be >= 1"),
    ...
    [
      {
        command: [
          "bun run validate",
          "--json",
          ...(options.scopes ?? []).map((scope) => `--scope=${scope}`),
        ].join(" "),
        expect: "exit0",
        timeoutMs,
      },
...
export const CREATE_PROPOSAL_INPUT_SCHEMA = z.object({
  ...
  status: z.enum(["pending", "ready", "in_progress", "in-progress"]).optional(),
...
const sliceIdPattern = (id: string): string =>
  /^[sS]\d+$/.test(id) ? `[sS]${escapeRegExp(id.slice(1))}` : escapeRegExp(id);
...
interface ICreateProposalRequest {
  ...
  readonly extraFrontmatter?:
    Readonly<Record<string, IFrontmatterPrimitive>> | undefined;
}
...
export const createProposalDocument = async (
    ...
    if (args.kind !== undefined) {
      ...
      if (!match.ok) {
        return {
          ok: false,
          reason: match.reason,
          nextAction: "Ensure the ID prefix matches the specified kind.",
        };
      }
    }
  ...
  const plan = {
    proposalId: id,
    globalGate: (args.globalGate ?? "none") as "lint" | "type" | "e2e" | "none",
    slices: slices.map((s) => ({
      proposalId: id,
      sliceId: s.sliceId,
...
export const sliceRequiresValidation = (
  ...
  const gateMatch = block.match(/^[-*]\s*(?:\*\*Gate\*\*|gate):\s*([^\n]+)$/im);
  ...
  const nested = [...acceptBody.matchAll(/^\s+[-*]\s+"?([^"\n]+)"?\s*$/gm)].map(
  ...
  register: async (server) => {
    server.registerTool(
      ...
      async (args: {
        ...
        const resolved = await resolveIndexedDoc(options, args.proposalId);
        if (!resolved.ok) {
        ...
        let validationDecision: ICloseSliceValidationDecision | undefined;
        ...
        try {
          await withFileMutex(docPath, async () => {
            ...
            if (md === null) {
              throw new Error(`proposal file missing: ${docPath}`);
            }
            ...
            if (slicePlan === null) {
              throw new Error(`slice plan missing in ${entry.file}`);
            }
            ...
            if (closeSliceOptions.resolveValidationDecision !== undefined) {
              const decision =
              ...
              validationDecision = {
                ...
                ...(guidance !== undefined
                  ? {
                      blockingReasons: guidance.blockingReasons,
            ...
            if (typeof options.runQuality === "function") {
              const quality = await options.runQuality(
                validationDecision !== undefined
                  ? {
                      ...
                      mode:
                        validationDecision.mode === "blocked"
                          ? "full"
              ...
              if (quality.severity === "error") {
                const err: ICloseSliceThrownError = Object.assign(
                  new Error("quality gate reported severity=error"),
                  {
            ...
            if (options.requirePeerReview !== false && args.force !== true) {
            ...
            const persistResult = await maybePersistAfterSlice(
              ...
              canonicalSliceId(args.sliceId),
              {
                ...
                ...(options.agentWorktreeEnabled !== undefined
                  ? {
                      agentWorktreeEnabled: options.agentWorktreeEnabled,
                    }
                  : {}),
                cwd: options.workspaceRoot,
                ...(configuredPersist.allowForeignChanges === true
                  ? { allowForeignChanges: true }
                  : {}),
                ...(closeSliceOptions.commitAuthor !== undefined
                  ? {
                      commitAuthor: closeSliceOptions.commitAuthor,
                    }
                  : {}),
            ...
            const sliceClosedContent = md.replace(blockRe, `${m[1]}${block}`);
            const nextContent =
              (args.validationScope ?? options.validationScope) === "global"
                ? markProposalDoneForAutoTransition(
                    entry.id,
                    sliceClosedContent,
                    options.requirePeerReview === undefined
                      ? {}
                      : {
                          requirePeerReview: options.requirePeerReview,
                        },
                  ).markdown
                : sliceClosedContent;
          ...
          if (err.kind === "validation-error") {
            ...
            const envelope = {
              ...
              ...(err.validationDecision !== undefined
                ...
                : {}),
              ...(err.persist !== undefined ? { persist: err.persist } : {}),
            };
        ...
        if (
          ...
          if (branch !== null) {
            const worktreePath = await resolveWorktreeTopLevel(options.run);
        ...
        if (alreadyClosedPayload !== undefined) {
          return toolOk({
            ...
            ...(validationDecision !== undefined ? { validationDecision } : {}),
          });
        }
        return toolOk({
          ...buildCloseSliceClosedResult({
  ...
  register: async (server) => {
    server.registerTool(
      ...
      async (args: {
        ...
        let nextStatus!: "none" | "in_review" | "changes_requested" | "done";
        ...
        try {
          await withFileMutex(docPath, async () => {
            ...
            const acceptanceCriteria =
              parseProposalSlicePlan(entry.id, md)?.slices.find((slice) => {
            ...
            if (args.action === "approve") {
              ...
              if (!identityCheck.ok) {
                ...
                throw Object.assign(new Error(identityCheck.reason), {
                  toolError: toolError(
              ...
              if (evidenceError !== null) {
                throw Object.assign(new Error("missing empirical evidence"), {
            ...
            if (!result.ok || result.next === undefined) {
              ...
              if (
                result.reason?.toLowerCase().includes("than the implementer")
              ) {
              ...
              throw Object.assign(
                new Error(result.reason ?? "invalid review transition"),
                {
                  toolError: toolError(
            ...
            if (args.action === "approve") {
              approvalOutcome = describeApprovalOutcome(next, quorum);
            }
            ...
            if (
              ...
              await appendPeerReviewLog(peerReviewLogPathAbs, {
                ...
                agent: args.agent,
                verdict:
                  args.action === "approve" ? "approved" : "request_changes",
                ...(redactedNote.text !== ""
        ...
        let lockReleased = false;
        let assignmentReleased = false;
        if (nextStatus === "done" || nextStatus === "changes_requested") {
          lockReleased = await releaseSliceLock(
            options,
            entry.id,
            args.sliceId,
        ...
        if (autoTransitionRequested) {
          ...
          if (located === null || located.status !== "done") {
            await recordAutoTransitionRepair({
              ...
              path: entry.file,
              reason:
                "auto-transition did not leave the proposal in done after approve",
            });
          }
        }
    ...
    server.registerTool(
      ...
      async () => {
        ...
        const actionable = index.proposals.filter((p) =>
          ["pending", "ready", "in_progress", "in-progress"].includes(p.status),
        );
          ...
          actionable.map(async (p) => {
            ...
            if (parsed === null) {
              return {
                ...
                status: p.status,
                slices: [],
                unreadable: "the document has no parseable `## Slices` section",
              };
            }
