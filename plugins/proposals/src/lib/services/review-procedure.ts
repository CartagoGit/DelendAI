...
export const procedureFor = (prefix: string): string =>
	...
	`${prefix}_proposal_review and ${prefix}_proposal_transition call, and commit there after EACH verdict, so the progress shows on the work branch while you go; \`delendai work publish\` it once, when the round is done. If the claim is refused because another agent took it first, take the next one. Never review a proposal another agent holds. ` +
