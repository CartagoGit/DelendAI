...
export const checkRevisionInconsistencies = ({
	db,
	...
	const count =
		db
			.query<
				{ count: number },
				[]
			>(`SELECT COUNT(*) AS count FROM lifecycle_events e
		...
		WHERE (e.entity_type = 'proposal' AND p.revision < e.entity_revision)
		   ...
		   OR (e.entity_type = 'slice' AND s.revision < e.entity_revision)`)
			.get()?.count ?? 0;
	return checkCount(
		'revision_inconsistencies',
		count,
		`${count} lifecycle revision inconsistencies detected.`,
	);
};
