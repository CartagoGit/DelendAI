...
describe('@delendai/proposals optionsSchema (S9 F9 + S7)', async () => {
	...
	it('accepts proposalFolders as a string array', async () => {
	...
	it('rejects proposalFolders with a non-string element', async () => {
		const r = plugin.optionsSchema?.safeParse({ proposalFolders: [42] });
		expect(r?.success).toBe(false);
	});

	it('rejects a malformed proposalNarrativePatterns tuple', async () => {
		const r = plugin.optionsSchema?.safeParse({
