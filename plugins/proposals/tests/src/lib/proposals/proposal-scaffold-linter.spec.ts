...
describe('lintProposalMarkdown — happy paths', async () => {
	it('accepts a minimal valid proposal with terse slices', async () => {
		const result = lintProposalMarkdown({
			path: 'docs/delendai/proposals/ready/f00114-do-the-thing.md',
			markdown: doc(),
		});
...
`;
	...
	it('resolves filename/folder/kind/status consistently', async () => {
		const result = lintProposalMarkdown({
			path: 'docs/delendai/proposals/in-progress/x00042-fix-the-bug.md',
	...
	it('resolves the retired legacy "p" prefix against kind: legacy', async () => {
		const result = lintProposalMarkdown({
			path: 'docs/delendai/proposals/done/p00099-old-thing.md',
...
`;
	...
	it('flags a filename prefix that disagrees with frontmatter kind', async () => {
		const result = lint(
			doc({ kind: 'fix' }),
			'docs/delendai/proposals/ready/f00114-do-the-thing.md',
		);
		expect(
	...
	describe('terminal-status sub-folders (f119)', async () => {
		it('accepts done/audits/foo.md with status: done', async () => {
			const result = lint(
				doc({ status: 'done' }),
				'docs/delendai/proposals/done/audits/a00001-15-06-2026-foo.md',
			);
			expect(
		...
		it('accepts done/feats/foo.md with status: done', async () => {
			const result = lint(
				doc({ status: 'done' }),
				'docs/delendai/proposals/done/feats/f00114-do-the-thing.md',
			);
			expect(
				result.issues.some((i) => i.message.includes('expects folder')),
			).toBe(false);
		...
		it('accepts done/audits/2024/q1/foo.md (deeply nested)', async () => {
			const result = lint(
				doc({ status: 'done' }),
				'docs/delendai/proposals/done/audits/2024/q1/a00001-foo.md',
			);
			expect(
				result.issues.some((i) => i.message.includes('expects folder')),
			).toBe(false);
		...
		it('accepts retired/foo.md with status: retired', async () => {
			const result = lint(
				doc({ status: 'retired' }),
				'docs/delendai/proposals/retired/x00001-foo.md',
			);
			expect(
				result.issues.some((i) => i.message.includes('expects folder')),
			).toBe(false);
		...
		it('rejects ready/audits/foo.md with status: done (non-terminal)', async () => {
			const result = lint(
				doc({ status: 'done' }),
				'docs/delendai/proposals/ready/audits/f00114-foo.md',
			);
			expect(
				result.issues.some((i) => i.message.includes('expects folder')),
			).toBe(true);
		...
		it('uses nearest status ancestor, not first occurrence', async () => {
			...
			const result = lint(
				doc({ status: 'ready' }),
				'docs/delendai/proposals/ready/audits/done/f00114-foo.md',
			);
			expect(
				result.issues.some((i) => i.message.includes('expects folder')),
			).toBe(false);
...
`;
	...
	it('flags an unknown filename prefix', async () => {
		const result = lint(
			doc(),
			'docs/delendai/proposals/ready/z00114-do-the-thing.md',
		);
		expect(
			result.issues.some((i) =>
...
`;
	...
	it('accepts a minimal valid audit proposal', async () => {
		const result = lintProposalMarkdown({
			path: 'docs/delendai/proposals/ready/a00021-audit-report.md',
			markdown: auditDoc(),
		});
		expect(result.ok).toBe(true);
		expect(result.issues).toEqual([]);
...
`;
	...
	it('accepts cascadeOverride with a paired cascadeOverrideReason', async () => {
		...
		const result = lintProposalMarkdown({
			path: 'docs/delendai/proposals/ready/f00024-do-the-thing.md',
			markdown,
		});
		expect(
