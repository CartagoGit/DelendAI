...
const insertProposal = (
	...
	driver.handle
		.prepare(
			`INSERT INTO proposals (uid, slug, kind, status, title, revision, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 0, ?, ?)`,
		)
		.run(uid, uid.replaceAll('/', '-'), kind, status, title, now, now);
};
...
describe('proposals_search (f00516 S2)', () => {
	...
	it('preserves title substring search in legacy mode', async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				proposals: [
					{ id: 'x00001', file: 'legacy.md', status: 'ready' },
				],
			}),
			'utf8',
		);
