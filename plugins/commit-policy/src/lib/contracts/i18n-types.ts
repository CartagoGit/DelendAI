...
/**
 * Look up a localized string. Falls back to English when the locale
 * is unknown (hosts can pass `process.env.MCP_VERTEX_LOCALE` and the
 * plugin resolves it; absent = English). Returns the English string
 * as a structural guarantee — never `undefined` — so callers can
 * embed it in tool output without a null-check.
 */
export const localizedString = (
	locale: string | undefined,
	accessor: (catalog: IStringCatalog) => string,
): string => {
	const key: Locale = locale === 'es' ? 'es' : locale === 'en' ? 'en' : 'en';
