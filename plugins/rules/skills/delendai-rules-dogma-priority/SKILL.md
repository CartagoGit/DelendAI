---
name: delendai-rules-dogma-priority
...
| No project config, TS dogma resolves | `dogma` | `eslint apps/web --config .cache/delendai/rules/react-ts.eslint.config.mjs` |
| No config, no dogma | `default` | `eslint apps/web --config .cache/delendai/rules/vanilla-ts.eslint.config.mjs` |
...
3. **Do not edit the vendored default** under
   `.cache/delendai/rules/`. The cache is overwritten on
