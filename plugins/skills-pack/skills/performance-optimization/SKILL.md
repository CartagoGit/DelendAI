...
tools: ['delendai_perf_perf_bench', 'delendai_perf_perf_bundle', 'delendai_perf_perf_profile', 'delendai_quality_get_quality_scopes', 'delendai_quality_run_quality', 'delendai_quality_quality_run_all']
...
1. Capture a reproducible baseline with `delendai_perf_perf_bench` so you can
   compare before and after numbers on the same workload.
2. If the regression is build-output or delivery related, run
   `delendai_perf_perf_bundle` to locate oversized artifacts before changing
   runtime code.
3. Profile the suspected path with `delendai_perf_perf_profile` and treat the
   biggest hotspot as the default first target.
4. Decide whether the problem is algorithmic, I/O bound, or build related
   before editing anything.
5. Ask `delendai_quality_get_quality_scopes` which focused validation scopes
   exist for this workspace.
6. Use `delendai_quality_run_quality` on the narrowest relevant scope after
   the first change to make sure the optimization did not break the slice.
7. Finish with `delendai_quality_quality_run_all` only after the targeted
...
- `delendai_perf_perf_bench`
- `delendai_perf_perf_bundle`
- `delendai_perf_perf_profile`
