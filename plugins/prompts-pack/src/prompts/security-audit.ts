	...
	buildTemplatedPrompt({
		...
		template: ({ file }) =>
			[
				...
				'',
				'Use `mcp-vertex_env_env_check` first when runtime configuration or required executables could affect the analysis.',
				'Use `mcp-vertex_security_security_secrets` for credential or token leakage signals.',
				'Use `mcp-vertex_security_security_sast` for code-level findings in the file and nearby flows.',
				'Use `mcp-vertex_security_security_audit` to place file-level findings in project-level posture context.',
				'',
