	...
	buildTemplatedPrompt({
		...
		template: ({ file }) =>
			[
				...
				'',
				'Use `mcp-vertex_perf_perf_profile` to identify hotspots or bounded runtime summaries for the relevant path.',
				'Use `mcp-vertex_perf_perf_bench` to capture reproducible before and after measurements.',
				'Use `mcp-vertex_perf_perf_bundle` when the file contributes to build or bundle size risk.',
				'After any edit, run the relevant scope through `mcp-vertex_quality_run_quality` so speedups do not regress correctness.',
				'',
				'Deliverables:',
				'- baseline signal',
				'- likely bottleneck',
