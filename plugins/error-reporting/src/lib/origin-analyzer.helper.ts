...
const LLM_TOOL_SUFFIXES = [
	'_orchestrator-runner_invoke',
	'_auto-agent-selector_auto_run',
] as const;
...
const isLlmTool = (toolName: string | undefined): boolean =>
	typeof toolName === 'string' &&
	LLM_TOOL_SUFFIXES.some((suffix) => toolName.endsWith(suffix));
...
export const analyzeErrorOrigin = (input: {
	...
	if (isLlmTool(input.toolName) && anyMatches(parts, llmFormatPatterns)) {
		return {
			origin: 'llm-format',
