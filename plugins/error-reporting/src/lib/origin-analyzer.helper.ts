...
export const resolveFirstPartyLlmToolProvenance = (
	...
	if (identity.owner !== 'delendai') return undefined;
...
const isFirstPartySpecifier = (specifier: string): boolean =>
	...
	/(^|\/)delendai\/(plugins|packages)\//i.test(specifier);
...
export const analyzeErrorOrigin = (input: {
	...
	if (isLifecycleInfo(input.error)) {
		if (isFirstPartySpecifier(input.error.resolvedSpecifier)) {
			return {
				origin: 'internal',
				reason: 'first-party plugin lifecycle surface failed inside delendai',
			};
		}
		return {
			origin: 'project',
	...
	if (classified.isInternal) {
		return {
			origin: 'internal',
			reason: 'positive internal evidence from delendai error typing or frames',
		};
	}
	...
	if (
		...
		return {
			origin: 'llm-format',
			reason: 'provider rejected a malformed payload generated by an delendai LLM tool',
		};
	}
	if (anyMatches(parts, providerPatterns)) {
		return {
			origin: 'provider',
			reason: 'failure came from the external provider, not from delendai construction',
		};
	}
	...
	return {
		origin: 'project',
		reason: 'no positive delendai evidence was found, so the failure is treated as host/project-local',
	};
};
