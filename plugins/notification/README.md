...
Lock-release **push** for [`@delendai/core`](../../packages/core). Instead of
...
```bash
delendai --plugins=proposals,notification
```
...
Under stdio, each agent is its own `delendai` process. A release in process A
