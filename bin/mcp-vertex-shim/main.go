// Package main — mcp-vertex-shim: a tiny Go binary that bridges stdio
// JSON-RPC to the existing `bun packages/cli/src/index.ts` child
// process. Lets end-users `curl -sSL get.mcp-vertex.dev | sh` and run
// `mcp-vertex --help` without first installing Node or Bun.
//
...
//
// Build: `go build -o dist/mcp-vertex-shim ./bin/mcp-vertex-shim`
package main
...
// mcp-vertex CLI. The shim is bundled with the same repo, so we walk
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "mcp-vertex-shim: %v\n", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	cmd.Env = append(os.Environ(), "MCP_VERTEX_SHIM=1")
