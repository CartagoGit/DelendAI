/**
 * f00046 S8 — web-fetch command. Delegates 1:1 to `delendai_web-fetch_web_fetch`,
 * the opt-in, allow-listed, fail-closed URL fetcher.
 *
 * Tools mapped:
 *   - `delendai_web-fetch_web_fetch` ({ url, maxBytes?, timeoutMs? })
 */
...
const webFetchCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_web-fetch_web_fetch', {
				url,
