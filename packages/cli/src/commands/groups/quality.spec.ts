...
import { qualityCommands } from './quality';
...
describe('audit group (f00046 S4)', async () => {
	it('audit plan maps --kind to the tool scope', async () => {
		...
		expect(calls[0]).toEqual({
			tool: 'delendai_audit_audit_plan',
			args: { scope: 'security' },
		});
	});
	...
	it('audit consolidate forwards dir + top', async () => {
		...
		expect(calls[0]).toEqual({
			tool: 'delendai_audit_audit_consolidate',
...
describe('logs group (f00046 S4)', async () => {
	...
	it('logs errors-tail forwards its supported filters', async () => {
		...
		expect(calls[0]).toEqual({
			tool: 'delendai_logs_errors_tail',
	...
	it('logs redact-test requires a text sample', async () => {
		...
		expect(calls[0]).toEqual({
			tool: 'delendai_logs_redact_test',
			args: { text: 'sk-secret' },
		});
	});
});
