 ...
 *
 * Tools mapped:
 *   - `delendai_rules_get_rules`   ({ area? })
 *   - `delendai_rules_check_rules` ({ area?, compact? })
 *   - `delendai_rules_apply_rules` ({ area?, files? })
 */
...
const rulesGetCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_rules_get_rules', {
				...(area !== undefined ? { area } : {}),
			}),
		);
	},
...
const rulesCheckCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_rules_check_rules', {
				...(area !== undefined ? { area } : {}),
...
const rulesApplyCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_rules_apply_rules', {
				...(area !== undefined ? { area } : {}),
