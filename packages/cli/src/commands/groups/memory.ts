 ...
 *
 * Tools mapped:
 *   - `delendai_memory_save`   ({ title, body, tags?, ttlSeconds? })
 *   - `delendai_memory_recall` ({ query?, tags?, limit? })
 *   - `delendai_memory_list`   ({ limit?, offset? })
 *   - `delendai_memory_forget` ({ id })
 *   - `delendai_memory_export` ({ format?, includeExpired? })
 *   - `delendai_memory_import` ({ payload, format?, mode?, conflict? })
 */
...
const memorySaveCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_memory_save', {
				title,
				body,
				...(tags !== undefined ? { tags } : {}),
...
const memoryRecallCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_memory_recall', {
...
const memoryListCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_memory_list', {
				...(limit !== undefined ? { limit } : {}),
...
const memoryForgetCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(await request(ctx, 'delendai_memory_forget', { id }));
	},
};
...
const memoryImportCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_memory_import', {
				payload,
				...(format !== undefined ? { format } : {}),
				...(mode !== undefined ? { mode } : {}),
