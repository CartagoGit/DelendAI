/**
 * f00046 S6 — docs search command. Delegates 1:1 to `delendai_docs_docs_search`.
 * (`docs list` / `docs read` remain inline in the registry; this group
 * adds the free-text search verb.)
 *
 * Tools mapped:
 *   - `delendai_docs_docs_search` ({ query, include?, limit? })
 */
...
const docsSearchCommand: ICliCommand = {
	...
	async run(args, ctx) {
		...
		return data(
			await request(ctx, 'delendai_docs_docs_search', {
				query,
