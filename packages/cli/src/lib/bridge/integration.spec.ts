...
describe('bridge integration (real fs, b00239 S3)', () => {
	it('install → files exist with marker → status reports ours', async () => {
		...
		try {
			...
			const first = await readFile(join(dir, 'delendai'), 'utf8');
			expect(first).toContain(BRIDGE_MARKER);
			expect(first).toContain('delendai');
			...
			const st = await stat(join(dir, 'delendai'));
			expect(st.isFile()).toBe(true);
			if (process.platform !== 'win32') {
				const mode = (await stat(join(dir, 'delendai'))).mode & 0o777;
	...
	it('foreign file is NEVER overwritten; install reports refused for that shim', async () => {
		...
		try {
			...
			if (process.platform !== 'win32') {
				await chmod(join(dir, 'delendai'), 0o755);
			}

			const outcome = await installBridgeDirectory(env, io);
			// Action is 'unchanged' when the single declared legacy
			// name is already occupied by foreign content: nothing was
			// installed, nothing was overwritten. `perShim` carries the
			// refusal for the project maintainer to inspect.
			expect(outcome.perShim).toHaveLength(1);
			...
			const after = await readFile(join(dir, 'delendai'), 'utf8');
