...
const makeService = (
	responses: Parameters<typeof createFakeTransport>[0] = {
		delendai_logs_query: queryFixture,
		delendai_logs_tail: tailFixture,
		delendai_logs_subscribe: subscribeFixture,
		delendai_logs_correlate: correlateFixture,
		delendai_logs_redact_test: redactTestFixture,
	},
) => {
...
describe('LogsService', async () => {
	it('query calls delendai_logs_query and redacts secrets in summary + meta', async () => {
		...
		expect(calls[0]?.tool).toBe('delendai_logs_query');
	});
	...
	it('tail respects limit and passes filter through', async () => {
		...
		expect(last?.tool).toBe('delendai_logs_tail');
	...
	it('correlate forwards taskId and agent to the server', async () => {
		...
		const last = calls[calls.length - 1];
		expect(last?.tool).toBe('delendai_logs_correlate');
