...
const SURFACE_ICONS: Readonly<Record<string, string>> = {
	...
	settings: `<svg viewBox="0 0 24 24" aria-hidden="true"><line x1="3" y1="6" x2="21" y2="6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="3" y1="12" x2="21" y2="12" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="3" y1="18" x2="21" y2="18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><circle cx="8" cy="6" r="2" fill="var(--delendai-bg)"/><circle cx="16" cy="12" r="2" fill="var(--delendai-bg)"/><circle cx="10" cy="18" r="2" fill="var(--delendai-bg)"/></svg>`,
};
...
export function buildTabsBar(lang: ILangDict): string {
	...
	// markup with `class="delendai-tabs"`. It now delegates to the shared
	// `renderTabs` so the docs site, the extension dashboard, and
	// every future surface emit the same `<nav class="delendai-tabs__bar">`
	...
	// an `<li class="delendai-tabs__action">` — no `role="tab"`, no
	...
	const surfaceActions = `
		<div class="delendai-tabs__surface-actions" role="group" aria-label="${text('dashboardSections', 'Dashboard sections')}">
			<button class="delendai-tabs__action-btn" data-surface="proposals" type="button" title="${text('openProposalBoard')}" aria-label="${text('openProposalBoard')}">${SURFACE_ICONS.proposals}</button>
			<button class="delendai-tabs__action-btn" data-surface="knowledge" type="button" title="${text('openKnowledge')}" aria-label="${text('openKnowledge')}">${SURFACE_ICONS.knowledge}</button>
			<button class="delendai-tabs__action-btn" data-surface="configuration" type="button" title="${text('openConfigurationCenter')}" aria-label="${text('openConfigurationCenter')}">${SURFACE_ICONS.configuration}</button>
			<button class="delendai-tabs__action-btn" data-surface="settings" type="button" title="${text('openSettings')}" aria-label="${text('openSettings')}">${SURFACE_ICONS.settings}</button>
		</div>`;
			...
			`<details class="delendai-app-nav__group" open><summary>${group.label}<span aria-hidden="true">⌄</span></summary><div class="delendai-app-nav__items">${group.tabs
				.map((id) => {
					...
					return tab === undefined
						? ''
						: `<button type="button" class="delendai-app-nav__item" data-sidebar-trigger="${tab.id}" aria-current="${tab.id === 'overview' ? 'page' : 'false'}"><span>${text(tab.label, tab.fallback)}</span></button>`;
				})
				.join('')}</div></details>`,
	).join('');
	return (
		`<div class="delendai-app-nav__mobile"><button type="button" class="delendai-app-nav__menu" data-nav-toggle aria-expanded="false">☰ <span>Menu</span></button></div>` +
		`<aside class="delendai-app-nav" data-nav-panel aria-label="${text('dashboardSections', 'Dashboard sections')}">${sidebar}<div class="delendai-app-nav__actions">${surfaceActions}${sidebarRefreshHtml}${sidebarExpandHtml}</div></aside>` +
		`<div class="delendai-content"><section class="delendai-tabs delendai-tabs--underline">` +
		renderTabs({
			tabs: tabItems,
			variant: 'underline',
