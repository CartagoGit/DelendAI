...
const renderQueue = (
	...
	if (queue === null) {
		return `<p class="delendai-fg-muted">${escapeHtml(text('dashboard.health.noneQueue'))}</p>`;
	}
	return `<dl class="delendai-kv">
...
const renderStaleRows = (
	...
	const text = (key: string) => extensionText(lang, key);
	if (stale.length === 0) {
		return `<p class="delendai-fg-muted">${escapeHtml(text('dashboard.health.noneStaleAgents'))}</p>`;
	}
	return `<table class="delendai-table">
		...
		<tbody>${stale
			.map(
				...
				<td class="delendai-num">${formatNumber(s.missedBeats)}</td>
				<td class="delendai-fg-muted">${escapeHtml(s.lastSeen)}</td>
...
export const renderPanelHealth = (
	...
	const text = (key: string) => extensionText(lang, key);
	return `
<section class="delendai-panel" id="panel-health" role="tabpanel" aria-labelledby="tab-health">
	<h2 class="delendai-panel__title">${escapeHtml(text('tabHealth'))}</h2>
	<div class="delendai-grid">
		...
		<div class="delendai-card delendai-card--third">
			<h3 class="delendai-card__title">${escapeHtml(text('healthLocks'))}</h3>
			<p class="delendai-kpi__value">${formatNumber(model.locksActive)}</p>
			<p class="delendai-kpi__hint">${escapeHtml(text('dashboard.health.activeProposalLocks'))}</p>
		</div>
		<div class="delendai-card delendai-card--third">
			<h3 class="delendai-card__title">${escapeHtml(text('healthStale'))}</h3>
			<p class="delendai-kpi__value">${formatNumber(model.staleCount)}</p>
			<p class="delendai-kpi__hint">${escapeHtml(text('dashboard.health.threshold'))} ${escapeHtml(model.orphansThreshold)}</p>
		</div>
		<div class="delendai-card delendai-card--half">
			<h3 class="delendai-card__title">${escapeHtml(text('healthQueue'))}</h3>
			${renderQueue(model.queue, lang)}
		</div>
		<div class="delendai-card delendai-card--half">
			<h3 class="delendai-card__title">${escapeHtml(text('dashboard.health.activeAgents'))}</h3>
			${model.agents.length === 0 ? `<p class="delendai-fg-muted">${escapeHtml(text('dashboard.health.noneActiveAgents'))}</p>` : `<ul>${model.agents.map((a) => `<li><code>${escapeHtml(a)}</code></li>`).join('')}</ul>`}
		</div>
