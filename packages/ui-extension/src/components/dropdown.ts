 ...
 * (`transform` + `opacity`, 180ms ease-out via `--delendai-transition-base`).
 ...
 * attributes for a11y. The menu items carry `data-delendai-action` so the
...
export interface IDropdownItem {
	 ...
	 * navigates to the URL directly (the runtime's `data-delendai-action`
...
export interface IDropdownOptions {
	 ...
	 * icon / caret. Defaults to `'delendai-dropdown'` so the default
	 * `delendai-dropdown__*` BEM shape is preserved. The docs site passes
	 ...
	 * `delendai-dropdown__*` styles but nothing else.
	 */
 ...
 * `idPrefix: 'mv'`, default `classPrefix: 'delendai-dropdown'`), so every
 * existing caller keeps working without changes.
 */
export const renderDropdown = (opts: IDropdownOptions): string => {
	const align = opts.align ?? 'left';
	const classPrefix = opts.classPrefix ?? 'delendai-dropdown';
	...
	const trigger = `<button
		...
		data-delendai-toggle="dropdown"
		data-delendai-dropdown-id="${escapeHtml(baseId)}"
	>
	...
	const items = opts.items
		.map((item) => {
			...
			// click handler only acts on `[data-delendai-action]`, so the
			// anchor is left alone (its `data-delendai-dropdown-id` is
			...
			if (item.href) {
				...
				return `<li role="none">
					<a
						...
						data-delendai-action="${escapeHtml(item.id)}"
						data-delendai-dropdown-id="${escapeHtml(baseId)}"${target}${rel}
					>
						${icon}${label}
					</a>
				</li>`;
	...
	return `<div
		...
		class="${classPrefix}"
		data-delendai-dropdown="${escapeHtml(baseId)}"
	>${trigger}${menu}</div>`;
};
