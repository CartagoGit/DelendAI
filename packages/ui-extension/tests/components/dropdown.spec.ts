...
describe('renderDropdown', async () => {
	it('renders a trigger button with the given label', async () => {
		...
		expect(html).toContain('delendai-dropdown__trigger');
		expect(html).toContain('aria-haspopup="true"');
		expect(html).toContain('aria-expanded="false"');
		// Label is rendered with surrounding whitespace; assert by class.
		expect(html).toMatch(
			/delendai-dropdown__trigger[\s\S]*More[\s\S]*delendai-dropdown__caret/,
		);
	});

	it('renders each item as a menuitem button with data-delendai-action', async () => {
		const html = renderDropdown({
			id: 'dd-1',
			label: 'More',
			items: [
		...
		expect(html).toContain('data-delendai-action="open-foo"');
		expect(html).toContain('data-delendai-action="open-bar"');
	...
	it('uses an explicit `idPrefix` for trigger / menu / wrapper ids', async () => {
		...
		expect(html).toContain('data-delendai-dropdown-id="nav-more"');
	...
	it('uses an explicit `classPrefix` for every BEM class', async () => {
		...
		// No default delendai-dropdown class leaks through when overridden.
		expect(html).not.toContain('delendai-dropdown__trigger');
		expect(html).not.toContain('delendai-dropdown__menu');
		expect(html).not.toContain('delendai-dropdown__item');
		expect(html).not.toContain('delendai-dropdown__label');
		expect(html).not.toContain('delendai-dropdown__caret');
	});
	...
	it('aligns right when align is "right"', async () => {
		...
		expect(html).toContain('delendai-dropdown__menu--right');
	});
