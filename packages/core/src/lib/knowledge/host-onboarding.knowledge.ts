...
/**
 * How a NEW project adopts delendai. This is the npm-readiness doc
 *: everything a host must provide lives in
 * `IDelendaiHostConfig`; nothing project-specific hides inside the
 * framework.
 */
export const HOST_ONBOARDING_KNOWLEDGE: IKnowledgeEntry = {
	id: 'delendai-host-onboarding',
	title: 'Adopting @delendai/core in a new project',
	body: [
		'# Host onboarding',
		'',
		'1. Add the package and create a host-config module that returns an `IDelendaiHostConfig`:',
		...
		'   - `pathLayout`: start from `DEFAULT_PATH_LAYOUT` (a `.cache/` scratch dir plus `docs/delendai/proposals/`); override any field.',
		...
		'5. Hard rule: delendai never imports host packages. If you need host data inside an engine, add a parameter to the engine, never an import.',
	].join('\n'),
};
