...
model: GPT-5.4
description: |
    Bounded subagent for @delendai/core. Verifies acceptance, validation gates, and release readiness within the mcp-vertex MCP contract.
...
This file is only the Copilot adapter; the agent contract lives in `mcp-vertex`.

## Compact lane

1. First call `mcp-vertex_overview` once per turn (tool: `mcp-vertex/mcp-vertex_overview`); follow its `recommendedNextAction`.
