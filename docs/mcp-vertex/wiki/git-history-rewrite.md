...
1. **Author rewrite** via `.mailmap`. Every LLM-attributed author
   (`copilot-minimax-m3 <copilot-minimax-m3@mcp-vertex.local>`,
...
This clones the repo to `../mcp-vertex-backup-<timestamp>.git` with the
...
```bash
cd ../mcp-vertex-backup-<timestamp>.git
git fsck --full
cd -
```
...
rm -rf ../mcp-vertex-backup-<timestamp>.git
