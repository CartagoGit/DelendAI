...
The unified discovery surface (`mcp-vertex_overview` + `mcp-vertex_agent_catalog`)
...
- `docs/mcp-vertex/host-hints/agent-instructions.generated.md` (single fragment, f00092)
...
`<!-- mcp-vertex:begin -->` / `<!-- mcp-vertex:end -->` markers.
...
`docs/mcp-vertex/host-hints/`.

The generator reads `docs/mcp-vertex/agent-catalog.generated.json`, which is
itself produced from the live tool registry, `packages/core/skills/manifest.json`,
and `docs/mcp-vertex/proposals/index.json`. Drift in any of the four inputs
