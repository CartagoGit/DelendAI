...
mcp-vertex VS Code extension. Run **MCP Vertex: Open Configuration Center**
from the command palette. In a multi-root window, choose the workspace whose
`mcp-vertex.config.json` you want to inspect.
...
The center edits only `<workspace>/mcp-vertex.config.json`. These settings are
versionable project policy and affect the next mcp-vertex server process.

VS Code's `mcp-vertex.server.command`, `mcp-vertex.server.args` and
`mcp-vertex.server.prefix` preferences select how the extension starts and
...
| Invalid JSON | Repair `mcp-vertex.config.json` in a text editor; the center will not overwrite it. |
