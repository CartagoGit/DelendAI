# ADR 0007 — `@mcp-vertex/core/contracts` (subpath) vs a separate package
...
The fourth external audit (`docs/mcp-vertex/audits/legacy/2026-08-25-develop-external-audit-chatgpt-sol-cuarta-pasada.md`,
Track C / `r00029`) originally proposed extracting the type-only
surface of `@mcp-vertex/core` into a brand-new workspace package,
`@mcp-vertex/contracts`, so that consumers who only need types
...
`@mcp-vertex/core/contracts` re-exports pure interfaces and the
...
`@mcp-vertex/contracts` package," but the agent had shipped a
...
`@mcp-vertex/contracts`" reads like new work, when in fact it was
...
constants, no `@mcp-vertex/core` dependency, no Node imports). It
predates this ADR and is not superseded by it: nothing in
`packages/client`, the plugins, or the apps imports from
`@mcp-vertex/contracts` today (`grep -rln "from '@mcp-vertex/contracts'"`
outside the package itself returns zero hits) — every consumer of
pure types imports `@mcp-vertex/core/contracts`. `packages/contracts`
...
The general type-only surface of `@mcp-vertex/core` lives at the
**subpath** `@mcp-vertex/core/contracts`, not at a separate,
...
2. **Avoid a premature independent `npm publish` surface.** Nothing
   outside `@mcp-vertex/{core,client,vscode,web}` consumes these
...
- A consumer outside `@mcp-vertex/{core,client,vscode,web}` that
  wants only the pure types cannot depend on them without also
  depending on `@mcp-vertex/core` (even if bundlers tree-shake the
...
- `packages/contracts` (the standalone zod/primitives package
  described in Context) and `@mcp-vertex/core/contracts` (this
...
`@mcp-vertex/core/contracts` into its own published package (or to
merge it into the existing `packages/contracts`):

| # | Condition | Metric | Status |
|---|-----------|--------|--------|
| 1 | `core/contracts` is imported from outside `@mcp-vertex/{core,client,vscode,web}` by more than 3 external packages | importer count via `grep -rl '@mcp-vertex/core/contracts'` outside the monorepo's own apps/packages | measure quarterly |
| 2 | A consumer needs to `npm publish @mcp-vertex/contracts` independently of `core`'s release cadence | npm publish runbook exists and is requested | blocking |
...
- `r00029` — original "extract `@mcp-vertex/contracts` package"
