...
`mcp-vertex/surface` that, when declared by a host, opted the host
...
1. No mainstream MCP host (Claude Code, Cursor, VS Code Copilot Chat,
   Aider, Codex, MCP Inspector) declares `mcp-vertex/surface`.
...
2. **`mcp-vertex/surface` capability declared by client** — if
...
`mcp-vertex_vertex`, `mcp-vertex_status`) are always present. The
...
- **Add a new private capability `mcp-vertex/surface/auto` that
...
- The bootstrap set stays intact: a never-refreshing client can
  still reach any activated tool via `mcp-vertex_vertex`.
- A new private capability declaration (`mcp-vertex/surface:
...
- TOK-004 in `docs/mcp-vertex/audits/legacy/2026-08-25-develop-external-audit-chatgpt-sol-tercera-pasada.md`
