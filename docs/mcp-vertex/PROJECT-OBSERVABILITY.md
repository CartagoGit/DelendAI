...
to dogfood the feature on the mcp-vertex repository itself.

The formulas behind every metric live in
[`docs/mcp-vertex/specs/project-observability-economics.md`](./specs/project-observability-economics.md).
...
- KPI snapshots are persisted atomically under the configured cache path
  (`.cache/mcp-vertex/results/project-kpis/history.json`) with a retention
...
- **CLI**:
  - `mcp-vertex kpis summary` — human summary
  - `mcp-vertex kpis costs --json` — stable JSON for CI/scripts
  - `mcp-vertex kpis audit` — incongruence findings
  - `mcp-vertex kpis efficiency --window-days=7`
  - `mcp-vertex kpis usage --watch` — refresh loop
  - `mcp-vertex kpis summary --threshold=calls<10` — fail CI on breached threshold
- **VS Code**: the Project KPIs view (`mcp-vertex.kpis`); the status bar
  exposes the overall score and links to the dashboard.

## Enabling the plugin

`mcp-vertex.config.json` wires the plugin (and its dependencies
