...
- **review-log**: approved by claude-opus5-orchestrator — Independent
  ...
  Checked against the real code instead: the combined stdout/stderr policy is live in `run-command.ts` (a single shared collector budgets both streams); `run-command-bytes.spec.ts` passes.
