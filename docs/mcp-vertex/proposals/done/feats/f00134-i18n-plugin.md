...
Implemented as: pure key extraction for `t(...)` / `i18n.t(...)` / `__(...)`, injected source scanning via `realI18nDeps()`, and `unused-key` findings wired into `i18n_check` with fixture-backed tool tests.
...
Implemented as: pure `validateInterpolation()` ICU/interpolation analysis covering `{name}`, `{{name}}`, `%s`, `{0}`, `plural`, and `select`, plus a new read-only `i18n_validate` MCP tool with output schema, registration tests, and ICU fixture coverage.
...
Implemented as: `i18n` added to the `standard` preset catalog, README expanded to document both tools, and generated tool-output typing aligned for `i18n_validate` pending full script regeneration.
