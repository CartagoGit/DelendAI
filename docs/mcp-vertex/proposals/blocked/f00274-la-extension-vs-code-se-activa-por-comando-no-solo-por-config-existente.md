...
mcp-vertex — en lugar de permanecer inerte hasta que exista
...
`["workspaceContains:**/mcp-vertex.config.json"]` — una única entrada.
...
adoptar mcp-vertex es exactamente el momento en que la extensión está
...
`onCommand:mcp-vertex.adopt` (y los comandos de arranque afines) como
...
- Construir `mcpv adopt`/`adopt_project` en sí — ya existe
...
package.json#activationEvents:
    workspaceContains:**/mcp-vertex.config.json   (ya existe)
  + onCommand:mcp-vertex.adopt                     (nuevo)
  + onCommand:mcp-vertex.showAdoptionAssessment     (nuevo, si no existe
...
- **Files**:
    - `extensions/vscode/package.json` (`activationEvents`, nueva
      entrada `onCommand:mcp-vertex.adopt`)
...
- Test de activación: en un workspace **sin** `mcp-vertex.config.json`,
...
- **Riesgo: activar la extensión bajo demanda en un workspace sin
  ...
  confirmar que ninguno asume `mcp-vertex.config.json` existente antes
  de registrarse incondicionalmente.
- **Riesgo: `onCommand:mcp-vertex.adopt` no dispara si el ID de
