...
# f00275 — `mcpv doctor --deep/--ci/--offline` cubriendo los modos de fallo de la auditoría

## Goal

Extender `mcpv doctor` (que ya existe y es sólido) con tres modos —
...
```
mcpv doctor                → checks actuales (sin cambios)
mcpv doctor --ci           → checks actuales + salida estructurada
                              pensada para pipelines (igual que --json
                              hoy, pero con exit codes documentados
                              para CI, no sólo para humanos)
mcpv doctor --offline      → checks actuales, saltando los que
                              requieren red (marcados con una nueva
                              propiedad `requiresNetwork` en el
                              registro de checks)
mcpv doctor --deep         → checks actuales + DEEP_CHECKS[]
...
- `mcpv doctor --ci` produce salida estructurada con exit code no-cero
  en al menos un fixture roto.
- `mcpv doctor --offline` completa sin timeouts ni errores de red en
  un entorno sin conectividad simulado.
- `mcpv doctor --deep` invoca `runErrorReportingSelfTest` y reporta
