...
# f00277 — `AgentSession` + `mcpv agents`: ver qué hacen todos los agentes en worktrees sin cambiar de rama
...
proposals), y exponerla como `mcpv agents`: una proyección legible del
...
plano como primer incremento: `mcpv agents` puede construirse como un
...
- La vista de VS Code (`AUD-F03`/`f00274`) que consumiría esta
  proyección — se construye después, cuando `mcpv agents` ya emite
  datos estables.
...
- Con dos o más git worktrees activos con cambios distintos, `mcpv
