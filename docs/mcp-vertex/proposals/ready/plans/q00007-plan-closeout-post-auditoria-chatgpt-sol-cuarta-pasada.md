...
contains:
    proposals:
        ...
        - { id: c00144, kind: chore, required: true, priority: P0, track: governance,
...
# q00007 — Plan closeout post-auditoría ChatGPT 5.6 Sol (cuarta pasada)
...
honestamente, este plan `q00007` arranque y complete lo que el
...
`f00182` (engine), `t00018` (happy path cross-agent) deben estar
...
| Hija | Tipo | Prioridad | Resumen |
|------|------|-----------|---------|
| `c00144` | chore | P0 | Branch protection YAML bifurcada: `main` estricto, `develop` flexible, carve-out `agent/*`. |
...
disponibles — sobre los que `q00007` construye.
...
- **Acceptance**:
  ...
  - El hook de `predecessor-plans` permite transición
    `q00007: ready → in-progress` automáticamente (vía
...
- Track A → Slices S2.A1 (c00144), S2.A2 (c00145), S2.A3
  (x00272), S2.A4 (v00127).
...
## Acceptance criteria (cierre del plan)

- `mcp-vertex_proposals_close_plan q00007` retorna sin
