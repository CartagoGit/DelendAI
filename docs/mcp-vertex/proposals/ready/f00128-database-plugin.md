...
membership (r00011), catalog + wiki. Implemented with a deterministic
`buildMermaidEr(schema)` renderer, `db_erd` tool registration + filtering,
database plugin knowledge entry, README mermaid usage example, and unit
coverage for empty schemas, FK classification, and tool round-trips.
