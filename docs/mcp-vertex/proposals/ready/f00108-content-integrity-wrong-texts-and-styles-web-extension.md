...
- **Acceptance**:
	...
	- "Verified Configuration Center preview finding (2026-07-13): `dev:vscode` had no route for the new project editor, so the browser mode could neither inspect nor exercise its real layout. Repair adds a lazy Configuration page backed by bounded GET/POST dev APIs, exhaustive MCP pagination, the merge-aware local save service and the same 12-language renderer used by the production panel. Desktop and 390px captures use the live workspace inventory (13 plugins / 39 artifacts), not mock data."
	- "Verified Configuration Center visual findings (2026-07-13): optional schema fields whose value was `undefined` were falsely marked read-only/secret because the redaction detector called `.includes` on an undefined serialization; the long General form also pushed Save/Discard below the viewport because the editor and dev shell expanded instead of constraining their internal scroll regions. Repair makes absent values editable, keeps unsafe/unserializable values fail-closed, constrains the shell/editor to the available viewport, and pins the sticky footer plus undefined-value behavior with regressions."
	- "Verified stale-render finding (2026-07-13): navigating while the real dashboard MCP request was in flight let its late response repaint over the selected page. A latest-task render queue now serializes the active paint and coalesces pending clicks to the last destination; the regression proves `dashboard → settings → configuration` finishes on Configuration without accumulating obsolete renders."
