...
- **Gate**: bun run validate
- implementation:
  - `lib/calibrate/write-through.ts` translates a completed `eval_run` result
    into winner-based `IOutcomeRecord`s through an injected
    `ICalibrationStore`, with a pure `summarizeWinRates()` helper that matches
    the routing threshold of 5 samples.
  - `eval_run` performs the calibration write-through automatically after a
    successful harness run when a `calibrationStore` plugin option is supplied;
    omitting the store keeps the plugin standalone and makes persistence a no-op.
  - `README.md` documents the internal write-through flow, the 5-sample
    threshold, and the `calibrationStore` option seam.
  - S3 tests cover fake-store writes, missing-store no-op, thresholding,
    task-type grouping, and multi-provider winner persistence.
