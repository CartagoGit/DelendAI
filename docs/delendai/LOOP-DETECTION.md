...
When an agent is flagged as stuck, a JSON file is written to `.cache/delendai/handoff/<agent>-<timestamp>.json` with the following structure. Secrets (like Stripe keys, database passwords, or auth headers) are automatically redacted using `redactSecrets`.
...
```json
{
  ...
  "roundContextDigestPath": "/absolute/path/to/workspace/.cache/delendai/proposals/round-context.digest.json",
  "recentToolCalls": [
    {
...
The detector is enabled by default. It can be configured globally in `delendai.config.json` under the root `loopDetector` key:
...
- Maintains a per-config list `interactiveAgentPatterns` of agent
  ...
  can extend the list from `delendai.config.json`.
...
- Disable loop detection: `delendai --no-loop-detector`
- Customize repeat threshold: `delendai --loop-detector.repeat-threshold=5`
- Change handoff directory: `delendai --loop-detector.handoff-dir=/tmp/handoff`
- Tighten or relax the interactive-agent ignore list:
  `delendai --loop-detector.interactive-agent-patterns=*-default,cursor-*`
...
2. If `notification` plugin is loaded, the server pushes an MCP `notifications/message` log payload containing:
   ```json
   {
     ...
     "agent": "my-agent",
     "reason": "exact-repeat",
     "handoffPath": ".cache/delendai/handoff/my-agent-1234567.json"
   }
   ```
3. **The Host Reaction**:
