...
symptom: "After setting `cacheDir` or `docsDir` in `delendai.config.json`, the `proposals` plugin's state (locks, task queue, round-context) appears to reset or split across two locations — agents stop seeing each other's claims."
