...
> [`docs/delendai/proposals/in-progress/f00119-auto-agent-selector-plugin.md`](../proposals/in-progress/f00119-auto-agent-selector-plugin.md).
...
The plugin slug becomes `delendai-auto-agent-selector` for npm and
`@delendai/auto-agent-selector` for the workspace. Namespace prefix at
runtime: `auto-agent-selector_` (e.g. `delendai_auto-agent-selector_auto_run`).
...
`delendai.openAutoAgentSelector`) that opens a panel with:

- the roster (reachable / missing + install hint)
- the per-task recommendation for the current task type
- a **Pin** action to write a per-task pin into `delendai.config.json`
...
- Acceptance gate: [`docs/delendai/VALIDATION-MATRIX.md`](../VALIDATION-MATRIX.md)
