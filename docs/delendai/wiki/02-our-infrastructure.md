...
What `delendai` already has, mapped to the routing problem. The goal
...
- `IHostIdentity` (in `host-config.interface.ts`) carries `metadata`
  ...
  `IDelendaiHostConfig` has 5 sub-interfaces
...
- The config schema (`delendai.config.schema.json`) has **no**
...
- No ingestion of `process.env` values into plugin options — config
  is filesystem-only.
- No "secrets" sub-interface in `IDelendaiPluginConfig`.
...
**What's there:**
- `IDelendaiPluginConfig.options` is the typed tube from
...
[`docs/delendai/proposals/done/feats/f00006-feat-multi-model-audit-plugin.md`](../proposals/done/feats/f00006-feat-multi-model-audit-plugin.md)
...
| Plugin-options plumbing (`OptionsSchema → register → handler`) | `providers` schema in `delendai.config.schema.json` |
