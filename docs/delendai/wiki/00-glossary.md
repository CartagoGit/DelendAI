...
### Host capabilities (delendai concept)
...
declared (in `delendai.config.json`) and discovered (LiteLLM
...
The single cache directory for the whole `delendai` instance.
Default: `.cache/delendai` relative to the workspace root (NOT
...
`delendai.config.json#providers`. **Statically maintained by the
...
timestamp)` events from the runtime. **Not present in `delendai`
