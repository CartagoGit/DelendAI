...
> realizado por cada agente del delendai, por cada plugin, por cada
> modelo, y por cada extension"*

A single plugin that records every tool invocation across all of
`delendai` and exposes aggregate views by **agent**, **plugin**,
...
>    `delendai.config.json#plugins.web-fetch.options.allowList`
...
- **No quota enforcement.** It observes; `orchestrator-runner`
  enforces.
- **No cross-host sync.** The log is per-`delendai` instance. If
  you run `delendai` on three machines, you have three logs.
...
1. Extend `delendai.config.json` schema with `providers` block at
...
8. Docs: update `docs/delendai/wiki/04` and `05` to v2 (point at
