...
> `docs/delendai/proposals/ready/` referencing this page.
...
> **root** of `delendai.config.json`, not under any plugin. Plugins
...
```jsonc
// delendai.config.json — top-level extension
{
  "$schema": "...",
  "cacheDir": ".cache/delendai",     // existing
...
**Validation rules (in `delendai.config.schema.json`):**
...
| File | Change | Lines |
|---|---|---|
| `packages/core/schema/delendai.config.schema.json` | +`providers` block + `CapabilityTag` enum | +60 |
...
| `docs/delendai/wiki/scenarios/copilot-with-minimax-byok.md` | worked example | +80 |
