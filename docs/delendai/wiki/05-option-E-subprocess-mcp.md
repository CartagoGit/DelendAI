...
4. **Persist the roster.** `${cacheDir}/orchestrator-runner/roster.json`
   (cache, auto-regenerated) plus `delendai.config.json#providers`
...
> `delendai.config.json#providers` (versionado). See
...
7. User confirms → orchestrator copies the relevant subset to
   `delendai.config.json#providers` (versionado).
...
1. **Subprocess lifecycle.** Long-running `codex mcp-server` needs
   restart on crash. Today `delendai` has no process supervisor
...
2. **Bootstrap authority.** When the LLM proposes a roster from
   ...
   `delendai.config.json`, or do we trust the LLM and let the
