...
closed-evidence:
  - a00022 predates the shipped-in convention (pre-2026-07-24)
...
- **Perfecto**: `agent-loop-detector.ts` + `loop-detector-service.ts` (wiring confirmado en `plugins/proposals/src/index.ts:108` y verificado en vivo durante `bun run test`, logs `loop-detector: agent "a1" is stuck...`) — el hallazgo de soft-loop de `a00021` (`H4`/`f00120`) está **resuelto**, no diferido.
...
- El hallazgo de soft-loop de `a00021` (`H4`, antes asignado a una hipotética `f00120`) está **confirmado resuelto** en producción (`agent-loop-detector.ts` + `loop-detector-service.ts`, wiring en `plugins/proposals/src/index.ts:108`, disparado en vivo durante `bun run test`).
