...
related:
  - f00004 # audit plugin: this proposal records a new finding in its master audit doc
  - f00010 # the master audit's §9 explicitly deferred this post-closure backlog to a future proposal
kind: fix
...
# x00003 — Post-closure audit: orchestration crash fix + remaining hardening (M25/M28/M32/M45/M46)
...
- **M45 — `auto_work`/`continue_proposal` lanzaban un crash de
  ...
  proposals actionable (el caso común tras cerrar f00010). Causa: un
...
- **M46 — `docsDir` del propio repo apuntaba a `docs/delendai`**
...
- No tocar f00007 (`keepLegacy` por defecto) — diferida explícitamente
