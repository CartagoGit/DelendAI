...
3. Surfaces the protocol in `AGENTS.md` / `docs/delendai/AGENT-BOOTSTRAP.md` so every host picks it up.
...
- `docs/delendai/AGENT-BOOTSTRAP.md` [MODIFY] — add a one-liner: "every agent MUST hold an `agent_lock` claim for the files it edits; the gate is `lint:agents`".
...
Add the "Closing a slice" sub-section to `multi-agent-coordination/SKILL.md` and the one-liner to `docs/delendai/AGENT-BOOTSTRAP.md`. The documentation changes are minimal: 2 paragraphs + 1 sentence.
