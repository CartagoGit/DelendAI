...
date: 2026-06-27
kind: fix
title: `loop-detector.handoffDir` is hardcoded to `.cache/delendai/handoff` and bypasses the host-configured `cacheDir` — agents that relocate the cache end up writing handoffs to a stray path
runner: copilot
model: minimax-m3
...
# x00054 — `loop-detector.handoffDir` is hardcoded to `.cache/delendai/handoff`

## goal

`LOOP_DETECTOR_DEFAULTS.handoffDir` in
`plugins/proposals/src/lib/agents/loop-detector-config.ts:113` is
hardcoded to the string `'.cache/delendai/handoff'`. The loop
...
- registry under `.cache/custom-cache/subagent-registry.json`
  (correct, same),
- but the loop-detector handoff under `.cache/delendai/handoff/`
...
- **No migration of strays** — a host that already has
  `.cache/delendai/handoff/` from before this fix can delete it
...
- New unit test in
  ...
  legacy `.cache/delendai/handoff/...` path. This pins the
  regression: if a future refactor reintroduces the hardcoded
  default, the assertion fails.

- Existing test that resolves `mockCtx.workspace.resolve('.cache/delendai/handoff')`
  ...
  `.cache/delendai/handoff/...` when the host does not override
  the default. The test must be updated to either (a) explicitly
  inject `cacheDir: '.cache/delendai'` in the mock ctx, or
...
- The new test in `loop-detector-service.spec.ts` passes; a
  ...
  `.cache/delendai` ⇒ handoff lands under the default) also
  passes.
- A host that sets `cacheDir: '/tmp/alt'` in
  `delendai.config.json` gets the handoff at
  `/tmp/alt/handoff/stuck-agent.json`, not at
  `/tmp/delendai/handoff/...`.
...
- **Stray directories**: a host upgrading from a previous build
  will leave a `.cache/delendai/handoff/` on disk. The new code
