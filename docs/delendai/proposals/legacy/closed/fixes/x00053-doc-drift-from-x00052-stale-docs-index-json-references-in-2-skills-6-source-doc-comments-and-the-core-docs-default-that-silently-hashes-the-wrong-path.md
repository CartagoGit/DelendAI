...
ownership:
    - { agent: implementation_runner, task: 'S1: in `plugins/proposals/src/lib/swarm/round-context-types.ts`, replace the hardcoded `docs/delendai/proposals/index.json` in `CORE_DOCS` with `.cache/delendai/proposals/index.json` and update the JSDoc block above it; add a unit test that builds the default `CORE_DOCS` against a fixture where the cache-path index exists and asserts no `rh-missing` value' }
    - { agent: implementation_runner, task: 'S2: in 6 source doc-comments, replace `docs/delendai/proposals/index.json` with the cache path: `sync-proposal-registry.ts` (the `tracked, Biome-linted file` block, line ~589), `proposal-transition.tool.ts` (line ~67), `plan-closure.resolvers.ts` (line ~90), `locate.ts` (header, line ~8), `index-reader.ts` (header, line ~4), `worktree-sync-coordinator.ts` (line ~7)' }
    - { agent: proposal_guardian,    task: 'S3: in `plugins/proposals/skills/legacy-proposal-migration/SKILL.md`, update the "rebuild `docs/delendai/proposals/index.json`" line and the "Never hand-edit `docs/.../index.json`" rule to point at `<cacheDir>/proposals/index.json`; the migration flow ends with the same `proposals_sync_proposals` call but the artifact it rebuilds is now in `.cache/`' }
globalGate: validate
acceptance:
    ...
    - { command: grep -rn "docs/delendai/proposals/index.json" plugins/proposals/src plugins/proposals/skills packages/core/src packages/core/skills, expect: only in x00053 sources (locate.spec.ts comment, blocked-by.spec.ts const, layout-relocation.spec.ts F3 — pre-existing intentional cases) and the .gitignore rule }
...
proposals registry index moved from `docs/delendai/proposals/index.json`
...
- **`plugins/proposals/skills/legacy-proposal-migration/SKILL.md`**
  tells the operator: *"Never hand-edit `docs/delendai/proposals/
...
`docs/delendai/proposals/index.json`" is a recipe for a future
...
- Replace `'docs/delendai/proposals/index.json'` in `CORE_DOCS` with
...
- `plugins/proposals/src/lib/proposals/sync-proposal-registry.ts`
  ...
  sentence "`docs/delendai/proposals/index.json` is a tracked,
...
- `plugins/proposals/src/lib/proposals/locate.ts` (header,
  ...
  `docs/delendai/proposals/index.json`. Update to
...
- `plugins/proposals/src/lib/tools/proposal-transition.tool.ts`
  ...
  `absolute path to docs/delendai/proposals/index.json`. Update
...
- `plugins/proposals/src/lib/agents/worktree-sync-coordinator.ts`
  ...
  `reads + rewrites docs/delendai/proposals/index.json`. Update
...
- Line ~39: the "rebuild `docs/delendai/proposals/index.json`"
  reference. Replace with
  "rebuild `<cacheDir>/proposals/index.json`".
- Line ~100: the "Never hand-edit
  `docs/delendai/proposals/index.json`" rule. Update to
...
The acceptance check (`grep -rn "docs/delendai/proposals/index.json"`)
...
- `grep -rn "docs/delendai/proposals/index.json"` over
