...
closed-evidence:
  - x00051 predates the shipped-in convention (pre-2026-07-24)
...
`delendai.config.json#agentWorktree: true` is supposed to provide. Today
...
- **`auto_work` plan grows by one step.** Token-budget impact is one
  ...
  per the `delendai-token-budget-playbook` skill.