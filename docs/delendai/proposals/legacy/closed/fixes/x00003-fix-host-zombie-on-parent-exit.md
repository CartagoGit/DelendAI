...
`delendai_overview` (compact mode). Investigation (see
...
- `delendai_overview` was returning `Canceled: Canceled` because the
...
shared `.cache/delendai/agents.lock.json` and
...
- Changing the MCP SDK or `IDelendaiProject`'s public shape. The
