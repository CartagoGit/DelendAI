...
- **3 plugins (issues, memory, quality) hardcode `delendai.config.json` in user-visible text** — same anti-pattern a00032 fixed for the `audit` plugin.
...
Replace literal `delendai.config.json` strings in user-visible text with the `<config-file>` placeholder, mirroring what a00032 S2 did for the `audit` plugin.
