...
- **Not** changing the handoff packet format. The packet schema (`delendai/handoff/1`) stays; we only stop **writing** them in false-positive cases.
...
- **Acceptance**:
  ...
  - No change to the public `IDelendaiHostConfig.isAgentStuck` signature. [done — sync path only reads from `stuckAgents.get(agent)`, no detection logic]
...
- The handoff packet schema (`delendai/handoff/1`) is unchanged. [done]
- No public API breakage: `IDelendaiHostConfig.isAgentStuck` signature unchanged; core host config consumers unchanged. [done]
...
| Cooldown window of 30s is wrong for some hosts | Made it configurable per-host via `delendai.config.json#plugins.proposals.loopDetector.cooldownMs`. Default 30s, host can shrink to 5s or grow to 5min. |
...
- Origin proposal: l103 / `docs/delendai/proposals/.../l103-loop-detection-and-handoff.md`
- Detected false-positive case: `docs/delendai/handoff/copilot-minimax-m3-1782596736291.json` (2026-06-27, repeatCount: 8)
