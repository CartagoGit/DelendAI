...
- **Acceptance**:
    ...
    - Collapsed the verbose 4-line "first move" into a single
      1-line statement ("`Follow [bootstrap]; first move is delendai_overview
      { compact: true } then delendai_agent_catalog`"); dropped the
    ...
    - Audit modes / cross-cutting invariants / score dimensions /
      ...
      on demand via `delendai_knowledge` / `agent_catalog`); the
