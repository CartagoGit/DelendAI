...
ownership:
    - { agent: proposal_guardian,    task: 'S1: frontmatter + zod schema for `agentWorktree` in `delendai.config.json` (top-level, default false)' }
    - { agent: implementation_runner, task: 'S2: CLI parser accepts `--agent-worktree=true|false` in core; defaults to false' }
    - { agent: implementation_runner, task: 'S3: delendai human CLI exposes the flag and forwards it to the host via `--agent-worktree`' }
...
workflows). The flag is host-scoped (top-level in `delendai.config.json`) so it
governs a host capability, not a proposals-internal policy.

This repo (delendai itself) keeps the default `false`. Hosts that want
...
- The 4 skills (`proposal-swarm-runner`, `proposals-workflow-playbook`,
  `delendai-multi-agent-coordination`, `concurrency-patterns`) all start with
...
- **Acceptance**:
  ...
  - "`delendai.config.json` in this repo does not set it explicitly (relies on
    the default)."

### S2 — `--agent-worktree` in core CLI parser

- **Files**:
  - `packages/core/src/lib/plugins/parse-cli-args.ts` (`IDelendaiCliArgs`,
    `KNOWN_KEYS`)
...
- **Acceptance**:
  - "Parser recognises `--agent-worktree=true|false` and `--agent-worktree` (bare =
    true) and stores the resolved boolean on `IDelendaiCliArgs.agentWorktree`."
...
### S3 — `delendai --agent-worktree` + forwarding to host
...
- **Gate**: `bun run test`
- **Acceptance**:
  - "`delendai --agent-worktree` and `delendai --agent-worktree=true` both end up as
    `--agent-worktree` in the forwarded `__serve` argv."
  - "`delendai --no-agent-worktree` (or `--agent-worktree=false`) forwards the
    explicit `false`."
  - "`delendai --help` shows the new flag in the global help text in all 12 languages
    (apps/web/src/i18n/cli.ts) — i18n parity enforced by `bun run check:i18n:cli`."

### S4 — Project `agentWorktree` onto the plugin context

- **Files**:
  - `packages/core/src/lib/cli/assemble.ts` (`buildContext`, `IDelendaiHostConfig`)
...
- **Acceptance**:
  ...
  - "The `IDelendaiHostConfig` audit summary includes the resolved value so
...
- **Acceptance**:
  - "When `agentWorktreeEnabled === false`, the tool returns
    `{ ok: false, action: <echoed action>, reason: 'agent_worktree is disabled by host configuration. Pass --agent-worktree=true (CLI) or set agentWorktree: true in delendai.config.json to enable.' }`
...
- **Files**:
  ...
  - `skills/delendai-multi-agent-coordination/SKILL.md`
...
- **Acceptance**:
  - "Each skill adds a one-line decision node at the top of its
    ...
    *'Read `delendai.config.json#agentWorktree` (or the
...
- In this repo, the default config + a fresh `bun run host` start means
  ...
  documented `ok: false` error — verifiable by `delendai tools list` or by
...
- **`delendai` global help** has a long, hand-curated i18n surface. S3 has a
