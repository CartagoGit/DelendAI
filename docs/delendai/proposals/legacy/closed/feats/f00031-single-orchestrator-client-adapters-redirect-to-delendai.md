...
date: 2026-06-21
kind: feat
title: Single canonical orchestrator — client adapters are thin redirectors to delendai
shipped-in:
...
# f00031 — Single canonical orchestrator — client adapters are thin redirectors to delendai

## Goal

Make **delendai itself** the only source of truth for the orchestrator
...
2. Load the canonical contract from `delendai_overview` /
   `delendai_continue_proposal` / `delendai_auto_work` /
   `delendai_delegate`.
...
single `delendai` entry**, never a per-client clone. Per-client names
like `delendai-orchestrator` either disappear from the selector or
become pure redirectors that load delendai on first call.

## Why

- The user already pays the cost of dogfooding: the same workflow is
  restated in `.github/agents/delendai.agent.md` and in
  `.claude/agents/delendai-orchestrator.md`, with the second copy
  drifting (50 lines, bucle, mientras el primero son 7 líneas). Both
  end up in the Copilot selector and feel redundant.
- `delendai_overview` already returns `recommendedNextAction`; that
...
- The audit `a00019` (line 599) flagged the absence of a Copilot
  ...
  delendai, and adapters only bind to it.
- The scaffolder in
...
exactly these lines (modulo whitespace and the project's `delendai`
namespace prefix):
...
`.claude/agents/` whose `name:` is `delendai*` but whose body is not a
redirector.

## Non-goals

- Changing the public MCP surface (`delendai_overview` / `auto_work` /
...
- Forcing Claude Code to drop its native `.claude/agents/` folder. We
  ...
  delendai's slot) lives there; everything else is moved out of the
...
- **Files**: `.github/agents/delendai.agent.md`.
- Replace its current 7-line body with the redirector body above.
- Keep the existing frontmatter (`name: delendai`, `description: …`).
- **Gate**: `bun run validate`.
- Acceptance: `git diff .github/agents/delendai.agent.md` shows the
...
- **Files**: `.claude/agents/delendai-orchestrator.cc.md`.
- Original spec: move `.claude/agents/delendai-orchestrator.md` to
  `scripts/agents-runtime/claude/delendai-orchestrator.md`.
...
- The rule inspects `*.agent.md` under `.github/agents/` and `*.md`
  ...
  - The file is under `.claude/agents/`, its `name:` starts with
    `delendai`, and its body is not the redirector shape.
...
- Acceptance: `bun run test` green; the rule warns on a hand-rolled
  fixture and stays silent on the actual `delendai.agent.md`,
  the four bounded subagents, and `delendai-orchestrator.cc.md`.
...
- **Files**: `docs/proposals/index.json`,
  `docs/proposals/done/feats/f00031-single-orchestrator-client-adapters-redirect-to-delendai.md`.
...
- The Copilot agent selector shows exactly one `delendai` entry and
  zero `delendai-orchestrator` entries.
- All other `*.agent.md` files are either redirectors or bounded
  subagents; the lint rule from S3 enforces this.
- `.claude/agents/delendai-orchestrator.cc.md` is on disk; Claude Code
...
- **Other agents depending on `.claude/agents/delendai-orchestrator.md`**
  by relative path. The only known reference is in archived audits
  (`a00019`, `a00013`); S4 covers them.
- **Drift between the redirector and delendai**: if delendai
...
- **S1 was already done, by a prior round, before this proposal was
  read**: `.github/agents/delendai.agent.md` is an 11-line redirector
...
- **S2's goal was already done, via a different (simpler) mechanism**:
  the proposal specified "move `.claude/agents/delendai-orchestrator.md`
  ...
  `.claude/agents/delendai-orchestrator.cc.md`. No code in this repo
