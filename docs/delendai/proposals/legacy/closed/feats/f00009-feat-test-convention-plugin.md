...
> `delendai.config.json` raíz, documentado en
...
- **El humano que adopta delendai** en otro repo: copia el plugin
...
````jsonc
// delendai.config.json (raíz del repo que adopta)
{
  "plugins": {
...
````bash
delendai --preset=swarm   # ahora incluye test-convention
# o explícito:
delendai --plugins=quality,test-convention,status-marker
````
...
- [ ] Smoke test: `delendai --plugins=test-convention` lista los 3
      tools.
...
- Smoke test en `delendai --plugins=test-convention` (§10).
...
| `delendai.config.json` raíz   | ya carga `"test-convention": { "options": {} }` |
...
- [x] `delendai.config.json` raíz con
      `"plugins": { "test-convention": { "options": {} } }`.
...
- [ ] **Pendiente real**: smoke test end-to-end
      (`delendai --plugins=test-convention` lista los 3 tools).
