...
ownership:
  ...
  - { agent: implementation_runner, task: 'S2: bootstrap-canonical lint — verify docs/delendai/AGENT-BOOTSTRAP.md has the canonical 9-section ordering, the anchor preamble, and no duplicate H2s' }
  - { agent: implementation_runner, task: 'S3: host-hints fragments lint — verify the 3 generated fragments under docs/delendai/host-hints/ also point at the bootstrap and never enumerate content' }
...
- The renderer of `docs/delendai/host-hints/*.generated.md` originally
...
- **Acceptance**:
  ...
  - Each host file MUST contain a top-level link to `docs/delendai/AGENT-BOOTSTRAP.md` (relative or absolute path). Without the link, the host is not anchored and the lint fails.
  - The host file MUST NOT contain any of:
    - backtick-quoted skill ids: `\`[a-z][a-z0-9-]+-[a-z-]+\`` (e.g. `\`delendai-operator\``)
    - backtick-quoted tool names: `\`delendai_[a-z_]+\`` other than the three bootstrap entry points (`delendai_overview`, `delendai_agent_catalog`, `delendai_agent_bootstrap`).
  ...
  - Spec covers 4 cases: a clean host file (passes), a host file missing the bootstrap link (fails), a host file enumerating a skill id (fails with the right line number), a host file that legitimately mentions `delendai_overview` (passes).
...
- **Acceptance**:
  - The lint walks `docs/delendai/AGENT-BOOTSTRAP.md` and asserts:
...
- **Acceptance**:
  - The lint walks `docs/delendai/host-hints/{copilot-instructions,claude,agents}.generated.md`.
  - Each fragment MUST contain a reference to `docs/delendai/AGENT-BOOTSTRAP.md` and MUST NOT enumerate skill / tool / proposal ids (same rules as the host-instructions lint, restricted to the three bootstrap entry points for tool names).
...
- **Acceptance**:
  - `lefthook.yml` has three new `pre-commit` commands wired as **BLOCKING** (no `|| true`):
    - `bootstrap-canonical-discipline` — `glob_filter` matches `docs/delendai/AGENT-BOOTSTRAP.md` only; runs `bun run lint:bootstrap-canonical`.
