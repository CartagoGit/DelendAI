...
> **Estado: TODO para revisión.** Continúa f00012 con tres frentes: (1)
...
Después de f00012 la web ya tiene estructura de páginas (home minimalista +
...
- Componentes: `PromptsSection.astro`, `ResourcesSection.astro`,
  ...
  plugin.
- `gen-section-pages.sh` (existente, creado en f00012) se amplía con 3
  secciones más.
...
- **id: s3** — Páginas `/prompts`, `/resources`, `/knowledge` + componentes.
  ...
  - Reusar `gen-section-pages.sh` de f00012. Añadir las 3 secciones al loop.
...
- [ ] `/prompts`, `/resources`, `/knowledge` renderizan datos reales en
      12 idiomas, con descripciones localizadas (post f00012 s4, en este
...
| 12 × 9 keys i18n nuevas = 108 traducciones a mano | Slice s3 arranca con `en` completo y deja los otros 11 idiomas con `en` como fallback en el dict. Las traducciones se añaden en una propuesta posterior (f00007) usando un script de bootstrap (DeepL opcional). |
...
- Documentación en [docs/PLUGINS-DELENDAI.md](docs/PLUGINS-DELENDAI.md) si
