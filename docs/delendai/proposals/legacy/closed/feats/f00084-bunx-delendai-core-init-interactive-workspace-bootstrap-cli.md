...
ownership:
    ...
    - { agent: implementation_runner, task: 'S3: `.github/agents/delendai-*.agent.md` generator from the live catalog (proposal-guardian, technical-investigator, implementation-runner, delivery-verifier, orchestrator)' }
    ...
    - { agent: delivery_verifier,    task: 'S6: end-to-end test against a tmp directory; verify the resulting `.vscode/mcp.json` + `delendai.config.json` + `.github/agents/*.md` make `delendai_overview { compact: true }` load clean' }
globalGate: validate
acceptance:
...
existing) project into a fully wired **delendai workspace** in under five
...
1. A **`delendai.config.json`** that lists the chosen preset, the chosen
...
3. A **`docs/delendai/skills/`** folder populated with the core skills the user
   accepts (versioned copy, not a symlink — so the workspace is self-contained).
4. **`.github/agents/delendai-<role>.agent.md`** files generated from the live
...
`/home/cartago/_proyectos/propios/delendai/tools/scripts/host/host-server.script.ts`
when the npm package is not yet available — no behavioral difference).

## why

Today, wiring delendai into a fresh project is a nine-step manual ritual:

```text
1. mkdir docs/delendai/proposals
2. mkdir docs/delendai/skills
3. mkdir .github/agents
4. write delendai.config.json by hand
5. write .vscode/mcp.json by hand
6. cp -r <core>/skills/* docs/delendai/skills/
7. cp <core>/host-hints/* .github/
8. write .github/agents/delendai-*.agent.md by hand
9. bun run catalog:generate (only available in the delendai repo)
```
...
- It touches the project deeply (new files under `docs/delendai/proposals/`).
...
- **Modifying existing config beyond what the user opted into.** If a file
  ...
  and refuses for `delendai.config.json` unless `--force` is passed.
...
The CLI **does not write to `.cache/delendai/`.** All artefacts live in the
...
> `.github/agents/delendai-<role>.agent.md` from the live catalog (gated on
...
export const initCommand = defineCommand({
  name: 'init',
  description: 'Interactive workspace bootstrap for delendai.',
  run: async (args, ctx) => {
...
### S3 — `.github/agents/delendai-<role>.agent.md` generator

- **Files**: [packages/cli/src/commands/init/init-render.ts](packages/cli/src/commands/init/init-render.ts)
- **Status**: done
- **Gate**: bun run typecheck

Reads `docs/delendai/agent-catalog.generated.json` (regenerated on the fly
...
```text
docs/delendai/host-hints/copilot-instructions.generated.md
...
Append mode prepends a `<!-- delendai:begin -->` / `<!-- delendai:end -->`
...
1. Render `docs/delendai/proposals/ready/f00001-migrate-legacy-<scope>.md`
...
3. Print a follow-up: `Run \`delendai_proposals_auto_work\` to start the
   migration.`
...
it('produces a self-consistent bundle in an empty tmpdir', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'delendai-init-'));
  const answers = makeAnswers({ workspaceRoot: tmp });
  const rendered = renderInitBundle(answers);
  await writeInitBundle(rendered, answers);

  // Verify the resulting workspace makes delendai_overview load clean.
...
asserts that `delendai_overview { compact: true }` returns
...
- `init --force` overwrites `delendai.config.json` without prompting.
...
1. **Foreign-proposal detection** (`init-foreign-detect.ts`, new): scan the
   ...
   shape: 'delendai' | 'rfc' | 'adr' | 'ad-hoc' | 'none' }`.
...
`/home/cartago/_proyectos/propios/delendai` and is invoked from
...
bun /home/cartago/_proyectos/propios/delendai/packages/cli/src/index.ts init --dry-run
bun /home/cartago/_proyectos/propios/delendai/packages/cli/src/index.ts init --force
```
...
```bash
# 1. build the dist (one-time, in the delendai repo)
cd /home/cartago/_proyectos/propios/delendai
bun run build           # produces packages/cli/dist/index.js

# 2. symlink the bin into ~/.local/bin (in your HOME, not the project)
ln -sf /home/cartago/_proyectos/propios/delendai/packages/cli/dist/index.js \
       ~/.local/bin/delendai

# 3. invoke from any destination project
cd /home/cartago/_proyectos/propios/azur-lx
delendai init --dry-run
delendai init --force
```

Pros: short command; works in any shell. Cons: requires the
`delendai` repo to stay at the same absolute path; if you move it,
...
```bash
cd /home/cartago/_proyectos/propios/azur-lx
bun --bun /home/cartago/_proyectos/propios/delendai/packages/cli/src/index.ts init --dry-run
```
...
that points back to the delendai repo: `.vscode/mcp.json`. The
...
delendai --version                     # or `bun .../index.ts --version`

# 2. confirm the server boots in the project
cd /home/cartago/_proyectos/propios/azur-lx
delendai init --dry-run                # prints the planned bundle, writes nothing
delendai init --force                  # writes the bundle

# 3. confirm the host can load it
# (in VS Code) Ctrl+Shift+P → "MCP: Restart Server" on delendai
# then in the chat:
...
`docs/delendai/CROSS-PROJECT-SETUP.md` § "Quick parity check".
