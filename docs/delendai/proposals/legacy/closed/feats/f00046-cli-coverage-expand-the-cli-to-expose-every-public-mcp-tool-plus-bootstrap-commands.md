...
related:
    - f00038 # Single CLI for delendai — local + remote transport (the foundation this proposal extends)
    - f00037 # Contracts and conventions — S3/S9 of this proposal add the `delendai conventions` CLI surface f00037 promises
    - f00030 # Cross-project setup — `delendai doctor` here is the operator-facing complement to f00030 S2 setup-github (reserved to f00030, not duplicated here)
...
# f00046 — CLI coverage: every public MCP tool as a subcommand, plus `delendai conventions` and `delendai doctor`
...
1. **Coverage parity**: every tool listed in `delendai_overview` has a CLI subcommand. The mapping is mechanical: `<prefix>_<verb>` → `delendai <verb>` or `delendai <plugin> <verb>`.
2. **Bootstrap surface**: `delendai conventions {check,plan,apply}` (f00037 S3), `delendai doctor` (env + config + plugin resolution), and shell completion for `bash`, `zsh`, and `fish` ship as first-class commands.
...
- **a00022 S5** (try/catch in 4 vscode MCP-client commands) and **a00032 A4** (compact `delendai_overview`) are both forced to inspect "what the user sees" through the IDE. A shell-only equivalent of every tool removes the IDE as a hard requirement.
- **f00037 S3** promises `delendai conventions check|plan|apply --profile=typescript` but the command does not exist. Every agent or human wanting to validate file conventions has to `bun run lint:proposals` and parse stdout.
- **f00030 S2** reserved `delendai setup-github` for itself; this proposal stays out of that lane but adds `delendai doctor`, which is the operator-facing complement (env + config + plugin resolution).
...
- **One command per MCP tool, not per use case.** The user wrote `delendai metrics --reset`, so they will write `delendai deps check`, `delendai audit plan`, `delendai proposals auto-work`. One mechanical rule, no second-guessing. New MCP tools land with their CLI command in the same PR.
- **Command groups mirror plugin namespaces.** `delendai git status`, `delendai memory recall`, `delendai logs query`. This matches the tool naming convention (`git_status`, `memory_recall`, `logs_query`) so the mapping is grep-discoverable from either direction.
- **`delendai` for the short binary, `delendai` for the long one** is preserved from f00038. Both ship from the same `bin:` declaration.
- **`delendai conventions` is its own top-level command, not under `lint`.** It is a *consumer-facing* command (f00037 promises it), not just an internal linter. Keeping it under `lint` would hide it from human users.
- **Shell completion is generated, not hand-written.** A `delendai completion bash|zsh|fish` command prints the script to stdout. The script walks `registerAllCommands()` at runtime, so it cannot drift from the actual command surface.
...
- **No interactive REPL.** Subcommands only. A `delendai shell` REPL is a v2 conversation.
- **No `delendai setup-github`.** Reserved to f00030 S2. This proposal adds `delendai doctor` instead, which is the operator-facing diagnostic companion.
...
- **No extending `bun run cli --` invocation.** `bun run cli --` continues to be an alias for `bun packages/cli/src/index.ts`; the binary path stays `bin: delendai` + `bin: delendai`.
...
No hay más. Cualquier cosa más compleja (por ejemplo `delendai proposals auto-work`, que necesita el agent name y el modo de persistencia) sigue el mismo patrón pero pre-valida los flags con `parseSetExpression`-style helpers.
...
Usage: delendai [global flags] <command> [args]
...
### S1 — git commands (`delendai git <verb>`)
- **Status**: done
...
- **Acceptance**:
  ...
  - "`delendai git status --json` devuelve el mismo payload que `git_status {}` invocado vía MCP."
...
- status: done
### S2 — memory commands (`delendai memory <verb>`)
- **Status**: done
...
- **Acceptance**:
  ...
  - "`delendai memory save --scope=user --key=foo --value=bar` se traduce correctamente al shape del tool (`{scope, key, value}`)."
...
### S3 — deps + rules + test-convention (`delendai deps/rules/test-convention <verb>`)
- **Status**: done
...
- **Acceptance**:
  ...
  - "`delendai rules get --area=cli` devuelve el mismo payload que `rules_get_rules {area: 'cli'}`."
  - "Cobertura CLI >= 90%."

### S4 — quality + audit + logs (`delendai quality/audit/logs <verb>`)
- **Status**: done
...
- **Acceptance**:
  ...
  - "`delendai quality run-all` retorna exit code no-cero cuando un scope falla, pero `--json` siempre retorna `{ok, scopes: [...]}` para que CI pueda parsearlo."
  - "`delendai audit plan --kind=security` devuelve un plan estructurado que el orquestrador puede consumir."
  - "Cobertura CLI >= 90%."

### S5 — fs read/write + knowledge + analyze/plan/create-project (`delendai fs/knowledge/project <verb>`)
- **Status**: done
- **Files**: `packages/cli/src/commands/groups/core.ts` (extensión), tests, i18n
- **Tools mapped**: `delendai_fs_read`, `delendai_fs_write`, `delendai_knowledge`, `delendai_analyze_project`, `delendai_plan_mcp_project`, `delendai_create_project`
- **Gate**: `bun run test packages/cli && bun run typecheck`
- **Acceptance**:
  - "6 nuevos subcomandos."
  - "`delendai fs read <path>` y `delendai fs write <path> --content=<string>` escriben vía `writeWorkspaceFileSafely` (mismo path que `config set` ya usa). Nunca escriben fuera del workspace."
  - "`delendai knowledge <id>` imprime el bloque markdown del knowledge entry."
  - "`delendai project analyze` y `delendai project plan` devuelven los blueprints que el orquestrador consume — útiles para revisar el plan desde terminal antes de delegar."

### S6 — docs + search extended (`delendai docs search`, search flags `--context`, `--json-lines`)
- **Status**: done
...
- **Gate**: `bun run test packages/cli && bun run typecheck`
- **Acceptance**:
  - "`delendai docs search <query>` existe y delega a `docs_docs_search`."
  - "`delendai search <query> --context=N --json-lines` pasa `contextLines` al tool search."
  - "Flags existentes (`--include`, `--exclude`, `--regex`, `--max`) preservan comportamiento."

### S7 — proposals (`delendai proposals <verb>`)
- **Status**: done
...
- **Acceptance**:
  ...
  - "`delendai proposals auto-work --mode=commit-and-push` se traduce al shape correcto del tool (`{mode, claimScope}`)."
  - "`delendai proposals transition <id> <action>` valida la acción contra el DFA antes de llamar al tool, dando un mensaje claro si es ilegal."
...
### S8 — notification + web-fetch + status-marker (`delendai notification/web-fetch/status-marker <verb>`)
- **Status**: done
...
- **Acceptance**:
  - "6 nuevos subcomandos."
  - "`delendai status-marker close` imprime la línea exacta de cierre que el orquestrador espera (`status-marker_close` tool devuelve esa línea)."
  - "`delendai web-fetch <url>` soporta `--max-bytes` y `--timeout`."

### S9 — `delendai conventions {check,plan,apply}` (consumer-facing surface for f00037 S3)
- **Status**: done
...
- **Gate**: `bun run test packages/cli && bun run typecheck`
- **Acceptance**:
  - "`delendai conventions check --profile=typescript` ejecuta el linter existente y devuelve `{ok, violations: [...]}`."
  - "`delendai conventions plan --profile=typescript --dry-run` lista las migraciones planeadas sin tocar el árbol."
  - "`delendai conventions apply --profile=typescript --dry-run` falla con `EXIT_CODE.VALIDATION` si hay violations no resueltas; sin `--dry-run` solo aplica las renames que pasan check."
...
### S10 — `delendai doctor` + `delendai completion <shell>`
- **Status**: done
...
- **Gate**: `bun run test packages/cli && bun run typecheck`
- **Acceptance**:
  - "`delendai doctor` imprime un reporte con secciones (env, config, plugins, tools) y exit code 0 si todo está bien, 1 si hay advertencias, 2 si hay errores (sigue el patrón de `quality_run_all`)."
  - "`delendai doctor --json` devuelve `{sections: [{name, status, findings: [...]}]}` para que CI lo consuma."
  - "`delendai completion bash|zsh|fish` imprime el script de completion a stdout, derivado dinámicamente de `registerAllCommands()` para que nunca quede desfasado."
  - "`eval $(delendai completion bash)` en una shell nueva autocompleta todos los comandos y sus flags."
...
- **Acceptance**:
  ...
  - "`delendai --help --lang=es` produce ayuda agrupada por grupo en español."
...
- **f00030 S2** (`setup-github`) sigue siendo dueña de `delendai setup-github`; esta propuesta **no** la duplica.
...
- **a00022 / a00032** consumen los nuevos comandos desde CI: `delendai overview`, `delendai metrics`, `delendai audit plan`, `delendai proposals auto-work`, `delendai status-marker close` son las cinco superficies que las auditorías consumen hoy a través de tooling improvisado.
- **f00027 / f00024** (metrics longitudinal gate) consumen `delendai metrics --persist` desde CI.
...
- `delendai doctor`, `delendai conventions {check,plan,apply}`, y `delendai completion <shell>` existen y están documentados.
- El help (`delendai --help`) está agrupado por grupo y traducido a las 12 locales del repo.
...
- **R3: el shell completion se desincroniza de los comandos reales.** Mitigado por derivación dinámica: `delendai completion <shell>` ejecuta `registerAllCommands()` y serializa el resultado. No hay script estático.
...
- La lista exacta de tools por slice se revalidará al iniciar cada slice contra `delendai_overview`; si un plugin añade/quita tools, el linter `cli-coverage` falla y el slice se ajusta mecánicamente.
