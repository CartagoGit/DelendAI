...
`delendai_proposals_inherit_host_instructions { workspaceRoot, scope? }`
...
- **No new CLI.** The proposal registry surfaces the tool; the
  `delendai` CLI is a thin wrapper that already works for every other
  proposals tool. If we ever want a `delendai host audit` shortcut we can
...
export const inheritHostInstructions = async (
  ...
  return [{
    relPath: `docs/delendai/proposals/ready/${id}-inherit-host-instructions-${answers.workspaceRoot}.md`,
...
- **Status**: done
- **Files**: `docs/delendai/proposals/ready/f00094-proposals-audit-non-repo-host-instructions.md`
  (this proposal)
- **Gate**: typecheck
- **Acceptance**:
...
- `delendai_proposals_inherit_host_instructions { workspaceRoot }`
...
- `delendai_proposals_inherit_host_instructions { workspaceRoot, scope: 'all' }`
...
- The new tool is enumerated in the catalog artifact and surfaces in
  `delendai_agent_catalog { section: 'tools' }`.
- The bootstrap canonical rule (rules live in
  `docs/delendai/AGENT-BOOTSTRAP.md`, not in checked-in host files)
...
- **Risk**: the proposal body grows unbounded when a user-home file
  ...
  f00093 — over 16 KiB the renderer switches to a `.cache/delendai/
...
- **Follow-up proposal idea (f00095)**: a `bun tools/scripts/lint/
  ...
  f00094.
- **Follow-up proposal idea (f00095+)**: a `delendai host audit` CLI
  shortcut that calls `delendai_proposals_inherit_host_instructions`
