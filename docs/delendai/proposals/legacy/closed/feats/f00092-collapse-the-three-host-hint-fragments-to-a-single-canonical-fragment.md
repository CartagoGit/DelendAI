...
ownership:
    - { agent: proposal_guardian,    task: 'S1: collapse the three per-host fragments to a single `agent-instructions.generated.md` and move the host-specific footnote into the hand-edited host files (`.github/copilot-instructions.md`, `CLAUDE.md`, `AGENTS.md`) between their `<!-- delendai:begin/end -->` markers' }
    ...
    - { agent: implementation_runner, task: 'S4: update `lefthook.yml` glob + `docs/delendai/CROSS-IDE.md` "Host discovery surface" section + `packages/cli/src/commands/init/init-render.ts` HOST_INSTRUCTIONS_BLOCKS to point at the single fragment filename' }
...
> - **S1**: `docs/delendai/host-hints/` holds ONLY
...
**three** near-identical fragments under `docs/delendai/host-hints/`:
...
- **One fragment on disk** — `docs/delendai/host-hints/agent-instructions.generated.md`.
- **The 3 hand-edited host files** (`.github/copilot-instructions.md`,
  `CLAUDE.md`, `AGENTS.md`) embed their host-specific footnote **inline**
  between the `<!-- delendai:begin -->` /
  `<!-- delendai:end -->` markers — exactly the place where the rest of
...
2. **The host files already have a hand-edited block.** The 3 host files all
   carry a `<!-- delendai:begin -->` / `<!-- delendai:end -->` region
   ...
   already says "this file is the delendai block".
...
4. **The agent catalog is the only source of truth for everything else.**
   ...
   NEVER enumerated here — they are served live by `delendai_agent_catalog`".
...
- **The footnote is the host's problem.** The footnote is host-specific
  ...
  `<!-- delendai:begin -->` region; the new footnote rides the same
...
- **No removal of the `<!-- delendai:begin/end -->` markers** — those
...
- **Status**: done
- **Files**: `docs/delendai/host-hints/agent-instructions.generated.md` (new),
  `.github/copilot-instructions.md`, `CLAUDE.md`, `AGENTS.md`
- **Gate**: typecheck
- **Acceptance**:
  - "The 3 old fragments are deleted; `agent-instructions.generated.md`
    is the only file under `docs/delendai/host-hints/`."
  - "Each of the 3 hand-edited host files references
    `docs/delendai/host-hints/agent-instructions.generated.md` AND
...
- **Gate**: typecheck
- **Acceptance**:
  - "`HOST_HINT_FRAGMENTS` is `[ 'docs/delendai/host-hints/agent-instructions.generated.md' ]`;
...
- **Status**: done
- **Files**: `docs/delendai/CROSS-IDE.md`,
  `lefthook.yml`,
  `packages/cli/src/commands/init/init-render.ts`
- **Gate**: typecheck
- **Acceptance**:
  - "`docs/delendai/CROSS-IDE.md` §Host discovery surface lists
    exactly `docs/delendai/host-hints/agent-instructions.generated.md`
...
- **Gate**: validate
- **Acceptance**:
  - "`ls docs/delendai/host-hints/` shows exactly
...
- The 3 host files carry their own host-specific footnote inline
  (between `<!-- delendai:begin -->` and `<!-- delendai:end -->`).
- Only `docs/delendai/host-hints/agent-instructions.generated.md`
  exists on disk under `docs/delendai/host-hints/`.
