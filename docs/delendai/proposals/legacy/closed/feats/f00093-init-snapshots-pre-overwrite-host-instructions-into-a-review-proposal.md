...
related:
    ...
    - f00092 # host-hints single fragment — the canonical delendai block this slice writes; the LLM will compare what's lost against this block in context
ownership:
...
are silently overwritten with the canonical delendai block (f00092:
...
`docs/delendai/proposals/ready/`, allocated the next free id (per
...
`delendai_proposals_auto_work` pass — at which point the LLM
already has the delendai bootstrap in context (via the canonical
...
The canonical delendai rules are the **single source of truth**
(`docs/delendai/AGENT-BOOTSTRAP.md`); the proposal is the workspace's
...
1. **Silent overwrite is a liability.** `init:default` exists because the
   ...
   would leave stale delendai rules in the host file forever. The
   fix is "overwrite + audit-trail".
2. **The LLM already knows the new rules.** When the proposal enters
   `auto_work`, the agent's first call is `delendai_overview`,
...
- **LLM has full context.** By the time `auto_work` picks up the
  ...
  `delendai_overview`) AND the new canonical block (via the
...
2. The file exists but lacks the canonical `<!-- delendai:begin -->`
...
- **`inventory`** — three fenced code blocks (one per host file),
  ...
  block names the file path and the canonical-delendai block that
...
- **Status**: done
- **Files**: `docs/delendai/proposals/ready/f00093-init-snapshots-replaced-host-instructions.md`
- **Gate**: typecheck
- **Acceptance**:
...
- **Acceptance**:
  ...
  - "When ANY host file has non-canonical content, the function
    ...
    `relPath: docs/delendai/proposals/ready/<id>-review-replaced-host-instructions-<workspace-hash>.md`
...
- The bootstrap canonical rule ("rules live in
  `docs/delendai/AGENT-BOOTSTRAP.md`, not in checked-in host
...
- **Risk**: the proposal becomes a graveyard if the LLM never gets
  ...
  `delendai_proposals_auto_work` is the very next call after
...
- **Risk**: the proposal body grows unbounded if the user has
  ...
  to `.cache/delendai/legacy-host-instructions/<host>.md` and
...
- **Follow-up proposal idea**: a `delendai_proposals_inherit_host_instructions`
