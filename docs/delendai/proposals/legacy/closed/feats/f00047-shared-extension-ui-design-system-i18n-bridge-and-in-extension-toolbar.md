...
actions. Users who don't know the command name (`delendai.open
...
- **The in-extension toolbar is a single webview panel**
  (`delendai.toolbar`) that uses `IHostAdapter` exclusively; no
...
- **Acceptance**:
  - "Every webview the VS Code host opens
    (`delendai.dashboard`, `delendai.knowledge`,
    `delendai.settings`, `delendai.toolDetail`) loads
...
- **Files** (rewired):
  ...
  `delendai.openToolbar` command,
  `extensions/vscode/src/commands/open-toolbar.ts` (new),
  `extensions/vscode/src/extension.ts` → registers
  `delendai.openToolbar` in `contributes.commands` and the
  activity-bar container `delendai.toolbar`,
  `extensions/vscode/package.json` → adds
  `viewsContainers.activitybar[].id: 'delendai.toolbar'` and the
  `delendai.openToolbar` command entry.
...
- **Acceptance**:
  ...
  - "The VS Code host registers a new webview panel
    `delendai.toolbar` and a new activity-bar icon that opens it.
    ...
    `delendai.openToolbar` when the action is to open another
...
- The extension gains a new activity-bar entry (`delendai.toolbar`)
...
- The `@delendai/shared` package is `private: true`. It is internal
  infrastructure for `delendai` itself and is not published.
