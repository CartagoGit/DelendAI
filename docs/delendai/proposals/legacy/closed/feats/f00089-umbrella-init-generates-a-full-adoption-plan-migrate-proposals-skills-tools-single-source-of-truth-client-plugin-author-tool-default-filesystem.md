...
1. **Detect** whether the target project already has a proposal system and
   **migrate it** to ours.
2. **Emit a proposal = full adoption plan** covering: (a) migrate delendai's
...
3. **Give the target's LLM a client tool to author plugins automatically** —
   without inspecting how delendai or its internal plugins are wired: the
...
4. **Default filesystem**: anyone using delendai can read files in their own
...
| 3 | Client tool: author + register a plugin by path | f00087 S2 operator script + client scaffold export; f00087 S1 `config.plugins.<name>.path` loader | No **MCP client tool** the target LLM can call; no auto-registration step that writes the `path` entry into `delendai.config.json` | **f00087 §vision U4** |
...
- **Security posture**: external paths are off by default (`[]`). Authorization
  is *explicit and durable* — it lives in `delendai.config.json` (committed,
...
- **Reconciled (2026-07-07) — RATIFIED AS SHIPPED.** Every vision point
  ...
  - **U1** foreign-proposal detection + adoption-plan emission:
    ...
    `f00095-adopt-delendai-delendai.md` was scaffolded by this very
    machinery.
...
- **Files**: docs/delendai/proposals/ready/f00089-init-genera-plan-de-adopcion-completo-umbrella.md, docs/delendai/proposals/ready/f00084-bunx-delendai-core-init-cli.md, docs/delendai/proposals/ready/f00088-init-respeta-el-proyecto-destino.md, docs/delendai/proposals/ready/f00087-mejorar-carga-de-plugins-locales-y-client-scaffold-export.md, docs/delendai/proposals/paused/f00068-external-mcps-plugin-paused.md
