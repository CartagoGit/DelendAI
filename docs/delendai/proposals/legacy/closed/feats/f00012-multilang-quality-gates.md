...
| `IDelendaiHostConfig.validationMatrix?` los usa | ✅ Sí | `host-config.interface.ts:40` |
| El JSON Schema de `delendai.config.json` los valida | ✅ Sí | `packages/core/schema/delendai.config.schema.json` |
...
| `delendai.config.json` del repo carga `quality` | ❌ **Laguna** | Solo `docs, search, git, status-marker, test-convention` |
...
2. **El repo dogfoods su propio plugin `quality`** en
   `delendai.config.json`, con un scope útil que aproveche los
...
`delendai.config.json` raíz no carga `quality`. Cambiar:
...
**Verificación**: tras el cambio, `delendai_overview` debe listar
...
- **Archivos**:
  - `delendai.config.json` (añadido `"quality": { "options": {} }`)
- **Cambios**: 1 línea.
- **Gate**: `lint`.
- **Aceptación**:
  - `delendai.config.json` carga `quality`.
  - `delendai_overview` lista 3 tools nuevos con prefijo `quality_`.
...
- [x] `IScopeCommand` ya no se declara localmente en el plugin
      (s1, hecho 2026-06-20).
- [x] `delendai.config.json` raíz carga `quality` (s2, hecho
      2026-06-20).
...
- No añadir `language` al `IDelendaiProjectMetadata`
