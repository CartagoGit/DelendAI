...
| **Contenido** | Knowledge navigator (S3) | `delendai_knowledge` |
...
3. **Knowledge is opaque** — `delendai_knowledge` returns
...
- `SearchService` wraps `search_search` and provides:
  ...
  - `searchKnowledge(query, opts): Promise<IKnowledgeHit[]>` — same
    idea but over `delendai_knowledge`.
...
- `KnowledgeService` gains:
  ...
  - `search(query): Promise<readonly IKnowledgeHit[]>` — uses
    `delendai_knowledge` with body matching.
...
- VS Code command: `delendai.openKnowledge` — opens the
...
- `HealthService` aggregates:
  ...
  - `delendai_status` → server health snapshot.
...
- New tree provider: `delendai.memory` — every key as a node with
  hover preview, click → open the entry in the Memory panel.
- 2 new commands: `delendai.memorySave`,
  `delendai.memoryForget`. Both have command-palette entries.
- **Gate**: `bun run test packages/client packages/ui-extension extensions/vscode` exit 0.
...
- `SettingsService` reads/writes `delendai.config.json#extension`
...
- VS Code command: `delendai.openSettings`.
- **Gate**: `bun run test packages/client packages/ui-extension extensions/vscode` exit 0.
...
- `ConnectionHealthService` pings the server every 5s with
  ...
  - `restart(): Promise<void>` — calls the host's
    ...
    `bun run delendai`).
- Status bar shows: `$(circle-green) delendai` (up) /
  `$(circle-red) delendai (down 12s)` (down) /
  `$(sync~spin) delendai (retrying)` (retrying). Click → open the
  dashboard.
- New command: `delendai.restartServer`.
- **Gate**: `bun run test packages/client extensions/vscode` exit 0.
...
- The 2 new view containers are:
  - `delendai.memory` — Memory tree (S5).
  - `delendai.health` — Health KPI webview view (S4).
...
- **Gate**:
  ...
  - `cd extensions/vscode && bun run package` exit 0, produces
    `extensions/vscode/delendai-vscode-0.3.0.vsix`.
...
- [x] A new `dist/extensions/vscode/1.0.0/delendai-vscode-1.0.0.vsix`
