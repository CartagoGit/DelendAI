...
closed-evidence:
  - f00018 predates the shipped-in convention (pre-2026-07-24)
...
# f00012 — Web: i18n real de herramientas, estructura por página y docs profundas
...
- **No hay documentación del JSON de configuración** del plugin
  (`delendai.config.json` → `plugins.<name>.options`).
- **No hay tutoriales** por plugin (solo una descripción de una línea en la
  home y en la página de plugin).

## 2. Goal

Que un usuario que aterriza en `cartagogit.github.io/delendai/es/` pueda:
...
- **id: s3** — Catálogos `i18n/<lang>.json` en cada plugin + validador
  ...
  - status: partial (commit 896ced5, 2026-06-20 01:21). El catálogo
    ...
    `delendai_overview` (12 idiomas). Spec de regresión en
...
- **id: s7** — Tutoriales markdown por plugin mayor (5 plugins × 12 idiomas =
  ...
  - status: done (render + descubrimiento, commits `8d03a09` y `b1be3a0`,
    ...
    otros 11 idiomas se difieren a una propuesta dedicada (f00010) — la
...
| Tutoriales en 5 plugins × 12 idiomas = mucho texto | Empezar con `en` (slice s7) y dejar la i18n de tutoriales para una propuesta dedicada (f00011). |
...
con la auditoría del plugin `proposals_plugin_review` (ver f00004 si está
