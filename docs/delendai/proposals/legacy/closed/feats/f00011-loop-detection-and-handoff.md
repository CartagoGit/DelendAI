...
closed-evidence:
  - f00011 predates the shipped-in convention (pre-2026-07-24)
...
# f00008 — Detección de "el agente dice que va a hacer X y no lo hace" + handoff a otro agente
...
> muestran). Quiero que el delendai detecte eso y pase todo el
...
- **Acción** cuando se confirma el "stuck":
  1. Escribir un **handoff packet** a
     `.delendai/handoff/<agent>-<isoTimestamp>.json` con la
...
- **Versionado del schema**: `schema: "delendai/handoff/1"` y
...
### 5.1 Fichero `delendai.config.json`

```jsonc
// delendai.config.json (fragmento)
{
  "loopDetector": {
    "enabled": true,
...
editar `delendai.config.json`** al arrancar el servidor. Mismo
patrón que el resto de flags del CLI (`delendai --help`
muestra todos). Precedentes en el repo: `--preset=swarm`,
`--check`, etc.

```text
# Desactivar el detector en esta sesión (no escribe nada a disco)
delendai --no-loop-detector

# Forzar activación explícita (pisar config.enabled: false)
delendai --loop-detector

# Ajustar umbrales sin tocar el fichero (útil en CI / smoke)
delendai --loop-detector.repeat-threshold=5
delendai --loop-detector.no-progress-threshold=10

# Cambiar la carpeta de handoffs (p.ej. en tests efímeros)
delendai --loop-detector.handoff-dir=/tmp/mcp-test-handoff
```
...
- El flag debe aparecer en `delendai --help` con una línea
...
| Caso | Comando |
|---|---|
| Sesión donde sé que voy a reintentar la misma tool call N veces (debug de tests) | `delendai --no-loop-detector` |
| Workspace donde ya vi varios handoffs y quiero parar | `delendai --no-loop-detector` |
| CI / smoke que monta un workspace efímero y no quiero escribir `.delendai/handoff/` | `delendai --loop-detector.handoff-dir=/tmp/x` |
| Forzar el detector aunque la config diga `false` (auditoría) | `delendai --loop-detector` |
| Subir umbrales porque el modelo legítimo reintenta mucho | `delendai --loop-detector.repeat-threshold=10` |
...
- `handoffDir` bajo `.delendai/` (mismo namespace que ya usan
...
| s6 | Config `loopDetector.*` en `delendai.config.json` + flags CLI (`--no-loop-detector`, `--loop-detector`, `--loop-detector.*`) con deep-merge + `bun run config:schema` regenera + tests de override (config / CLI / config+CLI) | type+test |
...
`.delendai/handoff/<agent>-<isoTimestamp>.json` con el schema
...
- **DoD**: `delendai --no-loop-detector` desactiva el detector.
  `delendai --loop-detector.repeat-threshold=5` lo afina sin
...
| Per-agent tracking | **sí**, clave `agent` del agent_lock | Dos agentes con la misma tool + mismo args no se confunden; matches f00008 §1 "agente lógico". |
  fichero.