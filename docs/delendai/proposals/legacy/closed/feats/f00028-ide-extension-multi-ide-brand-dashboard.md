...
shipped-in:
    ...
    - 046dce0 # Add unit tests and implement metrics rendering for delendai extension
related:
...
3. **Embedded documentation** — a `Web-Embed` webview that loads the
   deployed docs site (`https://delendai.dev` by default, configurable
   via `delendai.config.json#extension.docsUrl`). Plus a **Knowledge
   panel** that surfaces `delendai_knowledge` results inside the IDE
...
- **`extensions/vscode/` is the reference implementation** of the adapter. It
  ...
  via `vscode.commands.executeCommand('delendai.showMetrics')` still
  works.
...
loads the configured docs URL (default `https://delendai.dev`).
...
- **Files**:
  ...
  - `packages/client/src/lib/services/embed-service.ts`
    - `resolveDocsUrl(config: { extension?: { docsUrl?: string } }): string` (defaults to `https://delendai.dev`)
...
- **Files**:
  ...
  - `extensions/vscode/src/extension.ts` — updated: replaces the per-view provider wiring with a single `activate(ctx)` that:
    ...
    4. registers 3 webview commands (`delendai.openDashboard`, `delendai.openDocs`, `delendai.openToolDetail`),
...
- **Status**: done
- **Files**:
  - `extensions/vscode/src/providers/activity-bar-icon.ts` — exposes the brand logo as the activity-bar icon for the `delendai` view container.
  - `extensions/vscode/src/providers/status-bar.ts` — upgraded: now shows `$(delendai) delendai v0.1.0 • 32 tools • 7 proposals • 12.3k tok • 4 agents`, click opens the dashboard. Subscribes to `lock-released` / `cap` / `bloqueado` and re-renders.
...
- **Files**:
  ...
  - `extensions/vscode/src/commands/open-docs.ts` — `delendai.openDocs` command.
- **Gate**: `bun run test extensions/vscode` exit 0.
- **Estimated work**: 1 session.
...
- **Gate**:
  ...
  - `cd extensions/vscode && bun run package` exit 0, produces `extensions/vscode/delendai-vscode-0.2.0.vsix`.
- **Estimated work**: 1 session.
...
- **Files**:
  ...
  - `extensions/vscode/CHANGELOG.md` — note: `delendai.showOverview` and `delendai.showMetrics` now open the dashboard (no breaking change for users scripting the commands; visual change for users who had pinned the webviews).
- **Gate**: `bun run test extensions/vscode` exit 0.
...
- A new `extensions/vscode/delendai-vscode-0.2.0.vsix` is produced.
...
- The web embed loads `https://delendai.dev` by default; localhost
...
- **`.vsix` hygiene already correct**: `git ls-files extensions/vscode/
  ...
  (`extensions/vscode/delendai-vscode-{0.1.0,0.2.0}.vsix`) but are
  correctly untracked.
