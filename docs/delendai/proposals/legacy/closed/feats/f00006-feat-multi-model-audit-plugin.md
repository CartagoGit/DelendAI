...
> carga con `delendai --plugins=audit`, y expone `<prefix>_audit_plan`
...
- ✅ **Sí** puede saberlo por tres vías:
  ...
  3. **Que el usuario declare su roster** en `delendai.config.json`.
...
- `audit_plan { scope? }` → devuelve **el brief canónico**: rúbrica y formato exactos de
  ...
  **checklist de secciones** (arquitectura, cada plugin, seguridad, tokens, tests, CI,
  ...
  Claude Code, Copilot, Codex…); cada uno escribe su `*.md` en `docs/delendai/audits/`.
