...
ownership:
    ...
    - { agent: implementation_runner, task: 'S2: extend plugin `OptionsSchema` to accept `markers.add`, `markers.disable`, `markers.override` from `delendai.config.json`' }
    ...
    - { agent: implementation_runner, task: 'S7: `delendai.config.json` example in README; CI lint that the declared markers collide-cleanly with the built-ins' }
globalGate: validate
acceptance:
...
export const UserMarkerSchema = z.object({
   ...
   *  `delendai-status-marker-and-closure` skill so the agent knows
...
The `delendai-status-marker-and-closure` skill currently lists 8
states. With this proposal it adds a 9th section:

> **9. User-defined markers**: any host may extend this table via
> `delendai.config.json`. See [the proposal](#f00071) for the merge
...
- **Do not auto-discover markers from disk**. The plugin does not walk
  ...
  The host declares them in `delendai.config.json`, period.
...
makes drift invisible. A single JSON block in `delendai.config.json` is
