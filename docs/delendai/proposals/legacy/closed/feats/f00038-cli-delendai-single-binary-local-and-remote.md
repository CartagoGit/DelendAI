...
date: 2026-06-21
kind: feat
title: Single CLI for delendai — local + remote transport, one binary, private package
related:
...
# f00034 — Single CLI for `@delendai/core` — `delendai` / `delendai`, local + `--remote=stdio`, one private package

## goal

Give humans (and CI scripts) a **single console entrypoint** to govern the repo without an IDE. One binary, two published names (`delendai`, `delendai`), one package, one source of truth. The CLI is **a thin shell** over the public surface of `@delendai/core` and `@delendai/client` — it adds zero domain logic and never imports from `core/src/lib/*`.
...
- "What is the state of the repo right now?" → `delendai status`, `delendai overview`
- "What is this MCP server actually doing?" → `delendai plugin list`, `delendai plugin inspect <name>`
- "How much have we spent / where is the latency / which tool is hot?" → `delendai metrics`
- "Is the code green? which gate failed?" → `delendai validate`, `delendai validate-matrix`
- "What is the effective config? did I misconfigure something?" → `delendai config {schema,show,get,doctor}`
- "Help me find something in the docs/code/memory." → `delendai search`, `delendai docs {list,read}`
- "Help me author a plugin/tool/skill." → `delendai scaffold <kind>`, `delendai plugin inspect` (for examples)
- "Set up a fresh repo." → `delendai init`

The same binary must work **against the local repo** (workspace=`.`) and **against a remote `delendai` server** (`--remote=stdio` in v1; `--remote=tcp://host:port` parsed but unimplemented in v1). One parser, one set of subcommands, two transports. The remote mode is what lets a human (or a CI runner) without an IDE consume the same tool surface the IDE exposes.

## why

- **The cost of dogfooding is not yet closed for humans.** Every auditor in the `a00021`/`a00023`/`a00024`/`a00026` family — and the master `a00022` S5 (try/catch in 4 `extensions/vscode` MCP-client commands) — was forced to look at "what the user sees" through the IDE. There is **no equivalent surface for the shell**: today a developer wanting to ask "what plugins are loaded" runs `bun run validate` and reads the output, or hand-parses `delendai.config.json`. A single CLI removes the IDE as a hard requirement for inspection.
...
- **One package, two bin names** is the right shape: it gives the user a daily-driver (`delendai`) and a stable, brand-aligned name (`delendai`) without paying for two packages.
...
- **Replacing the host's bin.** We migrate the `bin: delendai` declaration from `@delendai/core` to `@delendai/cli`. `core/cli.ts` becomes a thin re-export (or stays as the canonical entry, with `cli/package.json` declaring `bin` instead — see S1). The user-visible `delendai` command keeps working.
- **Implementing `--remote=tcp://...` in v1.** The flag is **parsed and validated** in v1 (so v2 can ship without a parser change), but the transport implementation is deferred. v1 only ships `--remote=stdio` (spawns the local `delendai` binary as a child process and talks stdio via `@delendai/client`).
- **An interactive REPL.** No `delendai` without arguments. A `delendai help` (or `delendai --help`) lists subcommands; subcommands take flags. A REPL is a v2 conversation.
...
```
packages/
├── core/                    (sin cambios funcionales; pierde el `bin: delendai`)
├── client/                  (sin cambios)
└── cli/                     NUEVO · private: true
    ├── package.json         (bin: delendai + bin: delendai; workspace deps: core + client)
...
**Data flow for a single command invocation** (`delendai status`):
...
- **Status**: done
- **Files**:
  - `packages/cli/package.json` (NEW; `private: true`; `bin: { "delendai": "./dist/index.js", "delendai": "./dist/index.js" }`; `workspace:*` deps on `@delendai/core` and `@delendai/client`; scripts: `test`, `typecheck`, `build`, `lint`)
  - `packages/cli/tsconfig.json` (NEW; extends `tsconfig.base.json`)
  - `packages/cli/vitest.config.ts` (NEW; extends `vitest.shared.ts`)
  - `packages/cli/src/index.ts` (NEW; minimal: prints `delendai 0.1.0` and exits 0; reserved for the dispatcher)
  - `packages/cli/tests/index.spec.ts` (NEW; 2 tests: version flag, `--help` lists "status/overview/...")
  - `packages/core/package.json` (1 line edit: drop the `bin: delendai` block, keep `exports` and `main` untouched)
...
- **Acceptance**:
  ...
  - `node_modules/.bin/delendai` resolves to `packages/cli/dist/index.js` (post-build) and prints the version.
  - `node_modules/.bin/delendai` resolves to the same file and behaves identically (single entrypoint, two bin names).
  - The 1-line edit to `packages/core/package.json` does not break any consumer test (`bun run validate`).
- **Implementation note (deferred)**: when S1 lands, `bin: delendai` no longer comes from `core`. The docstring on `core/src/cli.ts` is the only user-facing hint that the canonical CLI moved; nothing in the contract changes.
...
- **Acceptance**:
  - `delendai --help` prints the list of registered subcommands in `--lang=en` by default, in the system locale if `--lang` is unset and `LANG` is one of the 12 supported, and in `en` otherwise.
  - `delendai bogus` exits 2 with the `USAGE` exit code and a single-line error pointing to `delendai help`.
  - `delendai <sub> --json` switches the formatter globally; the JSON is stable (sorted keys) and parseable with `bun -e`.
...
- **Implementation note (deferred)**: at this point there are 0 subcommands registered. The parser, formatters and i18n are exercised by a 2-test smoke in S1's `index.spec.ts` (`delendai --version`, `delendai --help`).
...
- **Files**:
  ...
  - `packages/cli/src/commands/overview.ts` (NEW; consumes the same engine as `delendai_overview`; `--full` for verbose, default is compact)
  ...
  - `packages/cli/src/commands/validate-matrix.ts` (NEW; consumes the same engine as `delendai_get_validation_matrix`; `--json` for CI)
  ...
  - `packages/cli/src/transport/remote-stdio.ts` (NEW; `runRemoteStdio(command, ctx)` that spawns the `delendai` binary (resolved via `which delendai || node_modules/.bin/delendai`) and pipes one `tools/call` request via the existing `packages/client/src/lib/transport/stdio.ts`; **propagates `outputSchema` byte-for-byte**)
...
- **Acceptance**:
  ...
  - **All 8 subcommands that make sense remotely (skip `validate` in remote, since the gate is local-only) work in `--remote=stdio` mode** by spawning a child `delendai` and re-using `@delendai/client`'s stdio transport.
...
- **Implementation note (deferred)**: at this point the user can already replace 90% of the IDE inspection flow with `delendai` from a terminal. The remaining 10% (`search`, `docs`, `scaffold`, `init`, write-side `config set`) lands in S4–S5.
...
- **Status**: done
- **Files**:
  - `packages/cli/src/commands/scaffold.ts` (NEW; wraps `delendai_scaffold`; **outputs to stdout by default** for piping in CI: `delendai scaffold tool --name=foo > foo.ts`; `--out=<path>` for direct write, which goes through `withFileMutex` + `writeFileAtomic`)
  - `packages/cli/src/commands/init.ts` (NEW; creates a minimal `delendai.config.json`; refuses to overwrite; respects `--workspace=`)
  ...
  - `packages/cli/tests/integration/durable-write.spec.ts` (NEW; spawns a real child process that calls `delendai config set` while a parallel `delendai config get` runs; asserts the read never sees a partial state)
- **Gate**:
  - `bun --filter @delendai/cli test` — verde
  - `bun --filter @delendai/cli typecheck` — verde
  - `bun run validate` (root) — verde
  - `bun run cli -- scaffold tool --name=foo --dry-run` — prints the generated tool file to stdout, exits 0, **does not write any file**
  - `bun run cli -- init --workspace=.` — refuses to overwrite when `delendai.config.json` already exists (exits `VALIDATION`)
...
- **Status**: done
- **Files**:
  - `packages/cli/src/transport/remote-stdio.ts` (UPDATE; resolve binary path: `process.env.DELENDAI_BIN || which("delendai") || join(repoRoot, "packages/cli/dist/index.js")`; verify it exists; spawn with `--quiet`; pipe `tools/call` requests)
  ...
  - `packages/cli/tests/transport/remote-stdio-integration.spec.ts` (NEW; spawns the real binary, runs `delendai --remote=stdio status`, asserts it matches the local `delendai status` shape)
- **Gate**:
  - `bun --filter @delendai/cli test` — verde
  - `bun run validate` (root) — verde
  - `bun run cli --remote=stdio status` — exits 0 and prints the same summary as `delendai status`
...
- **Files**:
  ...
  - `package.json` (root; `scripts`): add `"lint:cli-imports": "bun tools/scripts/lint/no-internal-core-imports.script.ts"` and run it in the `validate` chain
  - `package.json` (root; `scripts`): add `"lint:cli-coverage": "bun tools/scripts/lint/cli-coverage.script.ts"` and run it in the `validate` chain
  ...
  - `docs/CROSS-IDE.md` (UPDATE; one new section: "Console access (delendai / delendai)" pointing to `packages/cli/README.md`)
...
- **Gate**:
  - `bun run validate` (root) — verde
  - `bun run lint:cli-imports` — verde
  - `bun run lint:cli-coverage` — verde
  - `bun run site:strict` — verde (the i18n check in `apps/web/scripts/check-i18n.ts` is part of `site:strict`)
  - `bun run cli -- --remote=stdio status --json` — exits 0
  - `bun run cli -- --remote=tcp://localhost:1234 status` — exits 6 with the planned-v2 message
- **Acceptance**:
  ...
  - The README of `packages/cli` is the single source of truth for "how to use the CLI". `docs/CROSS-IDE.md` and `docs/PLUGINS-DELENDAI.md` get a one-line cross-link.
...
- `delendai` and `delendai` resolve to the same file and behave identically.
...
4. **The `bin: delendai` migration in S1 is a user-facing change.** A user with `@delendai/core@0.1.0` installed globally who upgrades to a version where the bin is gone will lose the `delendai` command. Mitigated by: (a) `core/src/cli.ts` keeps its `if (import.meta.main) runCli(...)` entrypoint, and (b) the new `cli/package.json` declares `delendai` in `bin`, so `npm i @delendai/cli` reinstalls the command. The CHANGELOG entry for this transition is a one-liner owned by `bun run release`.
...
- `packages/cli` is publishable (`private` omitted, `publishConfig.access: public`) and owns both `delendai` and `delendai`.
...
1. **One package, two bin names** (`delendai` and `delendai`, both pointing to `dist/index.js`), not one name. Reason: `delendai` was already the published bin of `core`; `delendai` is the daily-driver short form. Re-cluttering the user's PATH is not worth the savings.
...
5. **`scaffold` outputs to stdout by default** (CI-friendly: `delendai scaffold tool --name=foo > foo.ts`). Writing to disk is opt-in via `--out=<path>`. Reasonable: a CLI for humans should not write files silently.
6. **`config set` is schema-gated**, never silent-coerces. Reasonable: the whole point of `delendai config get/set` is to give the user precise control, and silent coercion is the bug that turns into a 2-day incident.
