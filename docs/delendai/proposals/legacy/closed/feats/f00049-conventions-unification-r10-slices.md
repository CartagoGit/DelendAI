...
ownership:
    ...
    - { agent: implementation_runner, task: 'S7: de-host i18n (drop delendai_ literals, consume @delendai/shared)' }
...
[`docs/ARCHITECTURE.md`](../../../ARCHITECTURE.md), the `delendai-plugin-authoring` skill and the
...
7. **i18n leaks host vocabulary**: `apps/web/src/i18n/tools/delendai_*.ts` hardcode the
   `delendai_` namespace; the same literal is repeated in `skills/delendai-token-budget-discipline/SKILL.md`
   and `delendai.config.json#auditDir` (which points to a directory that does not exist — audits
   actually live in `docs/proposals/done/audits/`).
8. **Skill names use two prefixes** (`delendai-*` × 11, bare × 5) with three pairs of
...
| 7 — i18n `delendai_*` literals | 7 files under `apps/web/src/i18n/tools/` | **+12 files under `apps/web/src/i18n/langs/<code>.ts`** (zh, hi, fr, ar, es, th, vi, ja, de, en, pt, it) | **+12** | **S7 grew.** Listed below. |
...
same `delendai_*` tool-id literals the original §evidence flagged under
...
### S7 — De-host i18n (drop `delendai_` literals, consume `@delendai/shared`)

- **Status**: done
- **Files**:
  - `apps/web/src/i18n/tools/delendai_*.ts` (7 files) → renamed to use the resolved namespace
    (template-rendered in the loader, not hardcoded).
  - `apps/web/scripts/load-tools-i18n.ts` (new) — resolves the namespace from
    `delendai.config.json` (the same `crossCuttingAdditions` field the audit plugin reads
    per a00032-S4) and stamps it into the per-tool dict at build time.
  - `delendai.config.json#auditDir` corrected to `"docs/proposals/done/audits"`.
  - `skills/delendai-token-budget-discipline/SKILL.md#L20` — replace literal
    `delendai_overview` with the dynamic namespace.
- **Gate**: `bun run site:strict` green; `bun run check:i18n:plugins` green.
- **Commit**: `feat(i18n): de-host apps/web i18n — drop delendai_ literals`
- **Shipped**: commit `6b11cb80`. Discovered the `delendai_*.ts` count was 69 (not 7 —
...
- **Status**: done
- **Decision**: drop the `delendai-` prefix on skill directory names (the manifest `id`
...
- **Files**: [`skills/`](../../../skills),
  ...
  - `skills/delendai-failure-modes/` → `skills/failure-modes/`
  - `skills/delendai-operator/` → `skills/operator/`
  - `skills/delendai-multi-agent-coordination/` → `skills/multi-agent-coordination/`
  - `skills/delendai-conventional-commits-and-release/` → `skills/conventional-commits-and-release/`
  - `skills/delendai-token-budget-discipline/` → `skills/token-budget-discipline/`
  - `skills/delendai-status-marker-and-closure/` → `skills/status-marker-and-closure/`
  - `skills/delendai-audit-runner/` → `skills/audit-runner/`
  - `skills/delendai-quality-and-rules-gates/` → `skills/quality-and-rules-gates/`
  - `skills/delendai-legacy-proposal-migration/` → `skills/legacy-proposal-migration/`
  - `skills/delendai-plugin-authoring/` → `skills/plugin-authoring/`
  - `skills/manifest.json` updated: every `bodyPath` rewritten; `id` field keeps `delendai-*`
    for host compat.
- **Merges**:
  - `delendai-token-budget-discipline` content is folded into `token-budget-playbook`
    (the playbook is the canonical home; the discipline skill becomes a thin pointer).
  - `delendai-audit-runner` content is folded into `audit-playbook`.
