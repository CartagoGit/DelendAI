...
reservedFiles:
  ...
  - .cache/delendai/proposal-id-counters.json
...
`.cache/delendai/proposal-id-counters.json` (gitignored machine state,
...
- **Scope correction (2 of the original 5 bullets deliberately NOT
  ...
  - **`adopt.ts#kindOf` was NOT refactored to use
    ...
    delendai is dropped into it — its `f` prefix means "fix,
...
### S13 — Race-safe ID allocator *(excl. `proposal-id-allocator.ts`, `.cache/delendai/proposal-id-counters.json`)*
