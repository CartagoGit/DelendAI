...
related:
    ...
    - a00032 # the universal-scopes refactor that proved the plugin could already be hosted outside delendai
...
ownership:
    ...
    - { agent: implementation_runner, task: 'S8: docs + skills — update plugins/rules/README.md, delendai-plugin-authoring, audit-playbook, token-budget-playbook, applying-rules knowledge' }
...
1. **Language coverage** — make the plugin useful in **every** language
   ...
   `delendai --plugins=rules` and get useful defaults per area.
...
- the manifest writes one config + one tsconfig per preset under `.cache/delendai/rules/`,
...
### The rest of delendai is already project-agnostic

`a00032`'s universal-scopes refactor proved the `audit` plugin could be loaded
inside a non-delendai host with no leakage of `delendai_metrics` /
...
microservice repo (the kind delendai's "polyglot workspace" use case implies in
[`docs/CROSS-PROJECT-SETUP.md`](../../../CROSS-PROJECT-SETUP.md)) should be able to
run the same `delendai --plugins=rules` and get useful defaults per area.
Today it cannot.
...
| **S8** | `plugins/rules/README.md` adds the "Polyglot workspaces" + "Dogmas" sections. `delendai-plugin-authoring` skill gains the "Adding a new language + dogma" subsection. | The `lint:proposals` / `lint:tools` linters exit 0. | The new skill is registered in `skills/manifest.json`. |
...
Priority order (highest → lowest):
  ...
  3. PLUGIN DEFAULT     — the materialised preset under .cache/delendai/rules/
```
...
- `check_rules { area: 'apps/web' }` returns:
  ...
  - `fromDogma`: `'eslint apps/web --config .cache/delendai/rules/react-ts.eslint.config.mjs'`
...
versions of delendai) and is the failure mode SOLID is designed to
...
- **Files**: skills/delendai-plugin-authoring/SKILL.md
...
3. **Cache pollution across language switches.** The cache dir is
   `.cache/delendai/rules/`; a project that switches between Python and JS
...
7. **Dogma fragmentation.** Some communities disagree on dogma
   ...
   **one** opinionated dogma per language; projects that want a
   different one override via `delendai.config.json#plugins.rules.dogmaOverrides`
   (a future extension). For this proposal the default is the most
   mainstream community choice.

## notes

### How delendai is used in non-Node projects
...
--plugins=rules`), reads the same `delendai.config.json`, and discovers
...
A pure-Python user that loads `delendai --plugins=rules` against a
