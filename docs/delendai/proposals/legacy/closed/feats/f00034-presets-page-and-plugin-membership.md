...
ownership:
    ...
    - {
          agent: implementation_runner,
          task: 'S4: i18n keys (preset names, descriptions, "requires" chip, table headers) × 12 languages + plugins.astro card "requires" chip + docs/PLUGINS-DELENDAI.md presets section',
      }
    - {
          agent: implementation_runner,
...
2. There's no web page showing "what's in each preset" — the
   install docs (`docs/README-DELENDAI.md`) mention the presets
   in a one-line table but don't list membership.
3. There's no preset that includes `issues`. The user has to
   manually write `delendai --plugins=proposals,issues`, which
...
3. **`docs/README-DELENDAI.md`** — its "presets" section becomes
...
docs/
├─ README-DELENDAI.md       (S4: presets section points at the page)
└─ PLUGINS-DELENDAI.md      (S4: presets subsection updated)
```
...
- **Files**:
  ...
  - `docs/README-DELENDAI.md` (replace the one-line preset
    table with a paragraph linking to `/es/presets`)
  - `docs/PLUGINS-DELENDAI.md` (add a "Presets" subsection
...
- [x] `delendai --preset=full` boots cleanly with the expected 9+
  plugins.
