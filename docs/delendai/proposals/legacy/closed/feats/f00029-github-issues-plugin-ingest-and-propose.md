...
ownership:
    ...
    - {
          agent: implementation_runner,
          task: 'S4: docs (plugins/issues/README.md + docs/proposals/retired/issues/README.md) + plugin enabled in delendai.config.json preset',
      }
globalGate: lint
acceptance:
...
2. **The plugin must be opt-in.** Almost nobody wants a GitHub
   ...
   user has to add it explicitly to `delendai.config.json#plugins`.
...
| **Docs** | `apps/web/src/pages/plugins.astro`, `apps/web/src/pages/presets.astro` (f00043), `extensions/vscode/src/embeds/plugin-help.ts`, `docs/PLUGINS-DELENDAI.md` | The plugin page renders a yellow "requires `proposals`" badge that links to the install snippet. The presets table shows `issues` ONLY under the "full" / "personal" preset, never under `swarm`. The VS Code plugin-help embed shows the same warning before the user enables the plugin. |
...
1. **`plugins/issues/README.md`** — first paragraph states the
   dependency, shows the install command (`delendai
   --plugins=proposals,issues`) and the
   `delendai.config.json` snippet.
2. **`docs/PLUGINS-DELENDAI.md`** — add `issues` under a new
...
5. **`extensions/vscode/src/embeds/plugin-help.ts`** — when the
   user types `delendai.issues`, the help panel renders the
...
- **No multi-repo.** Single hardcoded `owner/repo` from
  `delendai.config.json#plugins.issues.options.repo` (default
...
- The plugin is **not** listed in the `swarm` preset. Users opt in
  by editing `delendai.config.json` and adding
...
- **Files**:
  ...
  - `docs/proposals/retired/issues/README.md` (the canonical
    shape + lifecycle)
  - `delendai.config.json` — add `"issues": { "options":
  ...
  - `apps/web/src/pages/plugins.astro` — the `issues` card renders
    ...
    plugin page.
  - `docs/PLUGINS-DELENDAI.md` — add `issues` under a new
  ...
  - `extensions/vscode/src/embeds/plugin-help.ts` — when the user
    types `delendai.issues`, the help panel shows the dependency
...
- Plugin README documents:
  ...
  - The **hard dependency on `proposals`** (top-of-file, with the
    install command and the `delendai.config.json` snippet).
...
- Plugin loads cleanly via `delendai --plugins=proposals,issues`.
...
- **Dependency on `proposals` is visible at every user-facing
  surface**:
  - `delendai --plugins=issues` (without `proposals`) exits
  ...
  - `docs/PLUGINS-DELENDAI.md` lists `issues` under a
