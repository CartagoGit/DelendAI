...
ownership:
    - { agent: implementation_runner, task: 'S1: extend delendai.config.json schema and assembleCliConfig so each plugin entry may carry an explicit `path` (relative to workspace or absolute); precedence CLI > config > npm name' }
    ...
    - { agent: delivery_verifier, task: 'V1: bun run validate green; new unit specs cover path resolution and the public scaffold surface; e2e spec simulates a consumer project that registers a local plugin via `delendai.config.json#plugins.<name>.path` and sees its tool' }
globalGate: validate
acceptance:
...
Two related consumer ergonomics fixes for downstream delendai users:

1. **`delendai.config.json#plugins.<name>.path` (S1)** — let a consumer
...
- **Config-level `path` is the right home for the first problem.** The
  ...
  config file is delendai's portable project surface and is the one
  place every host reads. A consumer that commits `plugins.lx-app.path`
  to `delendai.config.json` gets a working setup in every host, today.
...
### S1 sub-section — `delendai.config.json#plugins.<name>.path`
...
`packages/core/schema/delendai.config.schema.json` gets the same
addition so editor tooling (the `$schema` URL) picks it up.

**Type interface** (`IDelendaiPluginConfig` in `load-config-file.ts`):
...
- Help text (`help-translation.constant.ts`): the `--plugins=` flag
  ...
  `delendai.config.json` may declare `path` to bypass the npm-name
  fallback."
- `docs/delendai/PLUGINS-DELENDAI.md` (the canon page): a
  "loading a local plugin" subsection with three worked examples
  (config path, CLI flag, symlink — recommended order).
- A new example under `docs/delendai/examples/local-plugin/` —
  a minimal workspace (`delendai.config.json` + `plugins/<x>/src/index.ts`
...
`docs/delendai/PLUGINS-DELENDAI.md` next to the S1 examples.
...
This lets a consumer of delendai (the operator) skip the explanatory
README step when they're confident.

## Slices

### S1 — `delendai.config.json#plugins.<name>.path`

- **Status**: done
- **Files**: packages/core/schema/delendai.config.schema.json, packages/core/src/lib/plugins/config-file-schema.ts, packages/core/src/lib/plugins/load-config-file.ts, packages/core/src/lib/cli/assemble.ts, packages/cli/src/contracts/constants/help-translation.constant.ts, docs/delendai/PLUGINS-DELENDAI.md, packages/core/tests/src/lib/plugins/plugin-path.spec.ts
- **Gate**: bun run validate
- **Acceptance**:
  - New schema field passes Zod + the published JSON Schema.
  - `IDelendaiPluginConfig.path` is typed.
  ...
  - Worked example in PLUGINS-DELENDAI.md renders correctly
...
- **Files**:
  ...
  - `docs/delendai/PLUGINS-DELENDAI.md` (example)
- **Gate**: bun run validate
- **Acceptance**:
...
*without inspecting how delendai or its internal plugins are wired*. Today the
...
`delendai.config.json`. U4 lifts both into one client-callable action:
...
- **`authorPlugin` (new, `packages/client/src/lib/scaffold/author-plugin.ts`)**:
  ...
  `delendai.config.json` (the loader from S1 then picks it up on next host
...
- After the release, the operator's sibling project can delete the
  ...
  to use `plugins.lx-app.path`, then commit `delendai.config.json`
  with the change.
...
3. **Do we want to warn when the same name appears in
   `delendai.config.json#plugins` AND in `--plugins`?** Today both
