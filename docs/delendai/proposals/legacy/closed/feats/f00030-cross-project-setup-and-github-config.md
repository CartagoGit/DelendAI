...
date: 2026-06-21
kind: feat
title: Cross-project setup — how to wire delendai (and the issues plugin) into any project
shipped-in:
...
ownership:
    ...
    - {
          agent: implementation_runner,
          task: 'S4: extensions/vscode: command `delendai.setupGithub` opens a multi-step webview that mirrors setup.astro + i18n for all 12 languages + README addendum',
      }
    - {
          agent: implementation_runner,
...
Make it **obvious how to set up delendai in any project** the
...
1. A **canonical markdown guide** at
   ...
   `docs/README-DELENDAI.md`, `docs/IDE-EXTENSION.md`,
   `docs/CROSS-IDE.md`, `docs/PLUGINS-DELENDAI.md`).
2. A **CLI subcommand + MCP tool** (`delendai setup-github`,
...
3. A **wizard web page** at `apps/web/src/pages/setup.astro`
   ...
   commands.
4. A **VS Code command** `delendai.setupGithub` that opens a
...
have ways to configure this so that whoever uses delendai
...
3. **No verification step**. The user runs `delendai
...
6. Prints the exact `delendai` invocation the user should add
   to their IDE config.
...
- **No global config.** All configuration is per-repo (the
  `delendai.config.json` in the workspace). Cross-repo config
  is delegated to the user's `~/.config/delendai/...` (if and
...
| 4 | **Write config** | Atomically patch `delendai.config.json#plugins.issues.options.repo` via `writeFileAtomic` + `redactSecrets` | re-read config and assert |
| 5 | **Verify tier** | Call `issues_fetch` on a sentinel issue (last open issue or the user's own #1) | `tier` field in the response |
| 6 | **Print invocation** | Emit the exact `delendai --plugins=proposals,issues --preset=full` (or `--plugins=...`) line + the `mcp.json` snippet for the user's IDE | string equality with the catalog |
...
- `docs/README-DELENDAI.md` adds a "First run in a new project"
...
- `docs/PLUGINS-DELENDAI.md` adds the setup command under the
  `issues` entry.
...
- **Files**:
  ...
  - `docs/README-DELENDAI.md` (added "First run in a new project"
    section)
  - `docs/IDE-EXTENSION.md` (added cross-project callout)
  - `docs/CROSS-IDE.md` (added cross-project callout)
  - `docs/PLUGINS-DELENDAI.md` (added explicit `Issues plugin`
...
- **Lost-work note (2026-06-22)**: a prior session reported this slice
  ...
  the spec below. See `.delendai/handoff/mensa-orchestrator-*.json`
...
- Walks `docs/`, `apps/web/src/pages/`, `extensions/vscode/`
  ...
  narrow — it only matches `delendai ... --preset=NAME` or
...
- `docs/CROSS-PROJECT-SETUP.md` exists, is the canonical
  reference, and is linked from `README-DELENDAI.md`,
  `IDE-EXTENSION.md`, `CROSS-IDE.md`, `PLUGINS-DELENDAI.md`.
- `delendai setup-github` runs end-to-end on a fresh repo and
  prints the exact invocation the user needs.
- `apps/web` shows `/es/setup` with the 7-step wizard.
- VS Code command palette shows `delendai.setupGithub` and
