...
related:
  ...
  - f00009 # multi-lang quality gates (este plugin complementa: quality ejecuta, auto-work coordina la persistencia)
...
1. **Input opcional del tool** — el orquestador puede sobrescribir el
   ...
   ```
2. **Config en plugin** (`delendai.config.json`) — default
...
Ejemplo renderizado: `docs(f00003): slice-3 implement persist modes`.
...
`delendai.config.json#plugins.proposals.persist` ya no es free-form:
...
| Lo sobreescribes por proyecto | no | `delendai.config.json` |
...
- [docs/PLUGINS-DELENDAI.md](../../../PLUGINS-DELENDAI.md) — contrato de plugins (config tipada).