...
Today every LLM call in the user's `delendai` workflow goes to
...
- **Cache lives inside the workspace, under per-plugin subfolders.**
  ...
  convention. No `~/.cache/delendai/` anywhere.
...
- **No multi-tenant aggregation.** The cache is per-`delendai`
...
- **No retroactive schema break.** The `providers` block at the
  root of `delendai.config.json` is additive; existing configs
...
- **No rollout in the swarm preset.** Both new plugins ship as
  opt-in via `delendai.config.json#plugins.orchestrator-runner`
...
packages/core/schema/
  delendai.config.schema.json                 EXT · top-level "providers" array,
...
| #5 Workspace-scoped paths | Cache stays inside the workspace. No `~/.cache/delendai/` anywhere. |
...
- **Deferred (own follow-up)**: the *schema + catalog* half of S1 —
  root-level `providers` block in `delendai.config.schema.json` + Zod
...
- **Files**: `packages/core/src/lib/contracts/interfaces/provider-capabilities.interface.ts` (NEW), `packages/core/schema/delendai.config.schema.json` (EXT), `packages/core/src/lib/plugins/config-file-schema.ts` (EXT, Zod mirror), `packages/core/src/lib/catalog/agent-discovery-types.ts` (EXT), `packages/core/src/lib/catalog/agent-discovery-catalog.ts` (EXT), `packages/core/tests/src/lib/contracts/interfaces/provider-capabilities.spec.ts` (NEW), `packages/core/tests/src/lib/catalog/agent-discovery-catalog.spec.ts` (EXT).
...
- **Acceptance**:
  ...
  - "Root-level `providers` block in `delendai.config.schema.json` accepts an array of entries with kebab-case `id` (regex `^[a-z][a-z0-9-]+$`), `uniqueItems: true`, `kind` enum `{api, subscription, cli, mcp-server}`."
...
- **Landed (2026-07-04)**: added the discovery layer + quota tracker, reusing
  ...
  **RFC 6902 JSON Patch** returned to the caller, effects `spawn`+`write`).
  ...
  elicitation/CLI (no `delendai.config.json` write from the plugin); (I3)
...
- **Acceptance**:
  ...
  - "Tool `<prefix>_bootstrap_providers` runs the full wizard: PATH probe → auth RPC per detected tool → write `${cacheDir}/orchestrator-runner/roster.draft.json` → return prose brief to caller → on user confirm, copy subset to `delendai.config.json#providers`."
...
- **Acceptance**:
  ...
  - "CLI commands added under `packages/cli/src/commands/groups/orchestrator-runner.ts` (consumes f00046 conventions): `delendai orchestrator-runner invoke`, `cancel-invocation`, `format-handoff`, `list-models`, `set-provider-state`, `bootstrap`, `discover`, `get-quota`, `advise-routing`, `healthcheck`."
...
- **Landed (2026-07-05, lint half)**: the `no-cleartext-secrets.script.ts` gate
  ...
  walks every tracked `delendai.config.json` / `*.config.json` EXCLUDING test
  fixtures and `docs/delendai/examples/**`, and fails on any field whose NAME
...
- **Acceptance**:
  - "`no-cleartext-secrets.script.ts` walks `delendai.config.json` and `*.config.json` under the workspace, excluding test fixture directories (e.g., `**/tests/fixtures/**`) and example directories (e.g., `docs/delendai/examples/**`); rejects any field whose name matches `/key|secret|token|password/i` AND whose value is not a `${ENV_VAR}` reference."
...
- **Acceptance**:
  ...
  - "Plugins are **opt-in**, not in the default preset. `delendai.config.json` example shows them under `plugins` but not in the `swarm` preset."
  - "**CLI surface for usage-tracking** is added under `packages/cli/src/commands/groups/usage-tracking.ts` (consumes f00046 conventions): `delendai usage-tracking report --group-by=provider|plugin|agent|extension --window-days=7 --json` and `delendai usage-tracking clear --confirm`. These wrap `<prefix>_usage_report` and `<prefix>_usage_clear` and let the user inspect spend from a terminal without an IDE — useful for CI dashboards and shell workflows."
...
- The new plugins are **opt-in**: if a user reports trouble, they
  can `delendai config set plugins.orchestrator-runner.options.enabled
...
moves to `done` and is filed under `docs/delendai/proposals/done/`.