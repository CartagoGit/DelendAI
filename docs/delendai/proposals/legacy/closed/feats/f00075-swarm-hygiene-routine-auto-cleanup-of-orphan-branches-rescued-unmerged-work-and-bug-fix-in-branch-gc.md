...
- **Out-of-cache worktrees are warned, never escalated.** The branch
  status engine flags `/tmp/delendai-x00056` (a previous-session
...
- **Files**:
  ...
  `docs/delendai/agent-catalog.generated.json` (regenerate via
  `bun run catalog:generate`),
  `docs/PAGES-AUDIT.md` (row for f00075),
  `docs/delendai/proposals/ready/f00075-swarm-hygiene-routine-cleanup.md`
...
| `outOfCache.length > 0` | **`warning`** | Plan proceeds; `plan.hygieneWarnings = ['N worktrees outside <cacheDir>/delendai/.worktrees (AGENTS.md violation)']`. Out-of-cache is never blocking because live sessions in `/tmp` or anywhere else would otherwise self-block. |
...
- The 2026-06-28 cleanup that motivated this proposal dropped
  ...
  worktree `.cache/delendai/.worktrees/copilot-minimax-m3-x00056`
