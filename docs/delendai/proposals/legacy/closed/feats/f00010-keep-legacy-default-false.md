...
budget: 4
kind: feat
title: `keepLegacy: false` por defecto en `delendai` y en el `mcp-project` que genera
...
# f00007 — `keepLegacy: false` por defecto en `delendai` y en el `mcp-project` que genera
...
1. **Refactors de un proyecto existente.** El usuario quiere
   "regenerar el host porque cambié la versión de `delendai`" o
...
1. **`keepLegacy: false` por defecto** en `delendai.config.json`
   (`IDelendaiConfigFile`), en `scaffold-tool.ts`
...
4. El analyzer `build-blueprint.ts` (que es el que arma el
   ...
   proyecto destino ya detecta un `delendai.config.json` con
...
5. Documentación: el campo se documenta en `packages/core/README.md`
   ...
   `docs/PLUGINS-DELENDAI.md` si toca algún plugin. La propuesta
...
### 3.1 El campo en `IDelendaiConfigFile`

`packages/core/src/lib/plugins/load-config-file.ts`:

```ts
export interface IDelendaiConfigFile {
...
- El `delendai.config.json` del usuario (si existe) tiene plugins
...
`docs/PLUGINS-DELENDAI.md` como parte del contrato de autor
de plugin.
...
> con `delendai_proposals_plan` antes de delegar).
...
- **id: s5** — README + ejemplos.
  - files: [packages/core/README.md,
    examples/minimal/delendai.config.json,
    examples/custom-plugin/delendai.config.json]
...
- `delendai.config.schema.json` regenerado y commiteado (no es
...
- **Riesgo**: los plugins existentes (proposals, memory,
  ...
  **Mitigación**: slice s1 expone el campo; los plugins lo
  consumen en su PR siguiente (out of scope de f00007). El doc
  `PLUGINS-DELENDAI.md` deja claro que es contrato de autor.
...
1. **No hay un refactor o migración activa** en `delendai` /
...
se reabre f00007, se ejecuta s1–s5 en orden y se cierra.

## 8. Cierre 2026-06-20

Implementado:

- `keepLegacy` en `delendai.config.json`, schema, `IMcpPluginContext`,
  `IDelendaiHostConfig`, assemble y scaffold generado.
...
- Documentación en `packages/core/README.md`, `docs/PLUGINS-DELENDAI.md` y
  ejemplos públicos.
