...
`.cache/delendai/logs/2026-06-2{7,8}.jsonl`):
...
2. **MCP `agent-loop-detector` log loop.** The `agent-loop-detector`
   ...
   `[delendai] onToolCall error: deriveOutcomeFromResult is not
...
- `create_file`, `replace_string_in_file`,
  ...
  `sed -i`, `awk`.
- `mcp_delendai_delendai_proposals_proposal_*` for any
  proposal move / status query / lint query that would normally
  use a shell script.
- `mcp_delendai_delendai_git_*` for read-only git work
...
- `docs/delendai/AGENT-BOOTSTRAP.md` (UPDATE) — the universal
  bootstrap gains a one-liner: "if the shell is stuck, see
  `withShellFallback`".
- `docs/delendai/skills/shell-fallback/SKILL.md` (NEW) — operator
...
Document the ladder in `docs/delendai/skills/shell-fallback/SKILL.md`
and add a one-liner to `docs/delendai/AGENT-BOOTSTRAP.md` that
points at it. The bootstrap edit is intentionally small: the
bootstrap already links to many skills; this is one more.

- **Status**: done
- **Files**:
    - `docs/delendai/skills/shell-fallback/SKILL.md` [NEW]
...
- **Done**: wrote the operator runbook (symptom, sentinel table, three
  ...
  documentation (per the `docs/delendai/skills/README.md` note that
