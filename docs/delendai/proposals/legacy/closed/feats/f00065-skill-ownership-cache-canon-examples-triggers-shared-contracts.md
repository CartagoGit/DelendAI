...
ownership:
  - { agent: implementation_runner, task: 'A: move skills out of docs/delendai/skills into their owners (packages/core for core skills, plugins/<x> for plugin skills); docs becomes documentation only; rewire load-skills, assemble, gen-skills, lint:skills' }
  ...
  - { agent: implementation_runner, task: 'C: canonicalize cache to root .cache/delendai; remove/migrate tools/scripts/.cache and any per-app/per-folder cache; document + contract the root-cache rule' }
  ...
  - { agent: host_runner, task: 'E: ensure slash (/) and prompt/skill trigger characters surface delendai tools/prompts/skills in Claude, Codex, Copilot, OpenCode, etc.; investigate per-host registration and propose integration' }
...
examples, and skills under `docs/delendai/**`. Several of those moves conflict
...
agent's decision to make `docs/delendai/skills` the canonical skill store and
...
- Skills currently live at `docs/delendai/skills/<skill>/SKILL.md` plus
  `docs/delendai/skills/manifest.json` (moved there by the uncommitted refactor).
- `packages/core/src/lib/skills/load-skills.ts` documents and resolves the
  ...
  `<workspace>/docs/delendai/skills/manifest.json` with a `<workspace>/skills`
  fallback. `apps/web/scripts/gen-skills.ts` hardcodes
  `SKILLS_REL = 'docs/delendai/skills'`. So the move is wired across core, the
  web generator, and the lint gate (`lint:skills`).
- A stray cache directory exists at `tools/scripts/.cache/delendai/**`
...
- **Files**: packages/core/src/lib/skills/**, packages/core/skills/** (new),
  ...
  packages/core/src/lib/cli/assemble.ts, docs/delendai/skills/**
- **Gate**: bun run validate
- **Goal**: skills no longer live in `docs/delendai/skills`. Core skills live
...
- **Acceptance**:
  ...
  - `load-skills`, `assemble`, `gen-skills`, and `lint:skills` resolve skills
    ...
    `docs/delendai/skills` literal.
  - A migration note documents that `docs/delendai/skills` is documentation
...
- **Gate**: bun run validate
- **Depends on**: A
- **Goal**: when a host loads delendai, the AI knows about and can use the
...
- **Acceptance**:
  - The active preset's skills (core + enabled plugins) are advertised through a
    ...
    prompt now tells the AI to call `delendai_skill`, and `agent_catalog`'s
...
- **Gate**: bun run validate
- **Goal**: exactly one canonical cache, the root `.cache/delendai/`. Remove or
...
- **Acceptance**:
  ...
  - Any generator that wrote to `tools/scripts/.cache` now writes to
    `.cache/delendai/**`. ✓ — the rules cache materializes to the root cache
...
- **Resolver (single source of truth)**: `cacheRoot()` / `CACHE_DIR_REL` in
  ...
  `.cache/delendai` segment is defined exactly once across engine + tooling.
...
- **Goal**: typing `/` (and the prompt/skill trigger character) in any AI host
  (Claude, Codex, Copilot, OpenCode, etc.) surfaces delendai tools/prompts and
...
- **Acceptance**:
  - A documented mapping of how each target host exposes `/` commands and
    prompts/skills, and what delendai must register for each.
...
- `docs/delendai/proposals/index.json` includes f00065 after `sync_proposals`.
- Skills resolve from their owners (core/plugins), not from `docs/delendai/skills`.
- No `.cache` directory exists outside the root `.cache/delendai`.
...
- **Risk**: removing `tools/scripts/.cache` breaks a generator that wrote there.
  **Mitigation**: redirect its writes to `.cache/delendai` before deleting and
