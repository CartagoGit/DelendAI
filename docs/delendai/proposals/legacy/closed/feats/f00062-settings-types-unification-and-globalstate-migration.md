...
| Direct `ctx.globalState.update('delendai.settings', ...)` | `store.write({ extension: { ...next } })` where `store` is the wired `ISettingsStore` | Lets the same service back tests (in-memory) and production (globalState) with zero branching. The `ISettingsStore` impl that wraps `context.globalState` lives in `extension.ts` (~line 332–357). |
