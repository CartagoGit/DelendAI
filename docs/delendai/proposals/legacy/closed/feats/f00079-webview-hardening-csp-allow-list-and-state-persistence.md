...
ownership:
    ...
    - { agent: implementation_runner, task: 'S4: implement `DelendaiProposalsView` `TreeDataProvider` and register it on activation; tree nodes mirror `delendai.proposals_status` shape (H5)' }
...
**`DelendaiProposalsView` as a real `TreeDataProvider`** mirrors the proposals plugin's
status output (`delendai.proposals_status`) — one source of truth, no shape drift. The
...
extensions/vscode/src/
  ...
  views/
    proposals-tree.ts         # NEW: DelendaiProposalsView extends TreeDataProvider<ProposalNode>
...
### S4 — `DelendaiProposalsView` `TreeDataProvider` (closes H5)
...
- **Shipped**: c3207c49. `ProposalBoardProvider` (mirrors
  `delendai_proposals_proposal_board`) is now registered against the
  `delendai.proposals` view id in `extension.ts` (previously the view was declared in
...
```typescript
export class DelendaiProposalsView implements vscode.TreeDataProvider<ProposalNode> {
...
```typescript
const tree = new DelendaiProposalsView(bridge);
vscode.window.registerTreeDataProvider('delendai.proposals', tree);
...
- **Status**: done
- **Gate**: bun run validate
- **Shipped**: c3207c49. `delendai.openProposal` now reads and validates its
