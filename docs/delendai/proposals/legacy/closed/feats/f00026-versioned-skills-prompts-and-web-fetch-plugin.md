...
1. **Versioning** the existing in-repo skills (`delendai-plugin-authoring`,
   `delendai-failure-modes`) and adding the missing ones
   (`delendai-operator`, `proposal-swarm-runner`, `state-repair-playbook`,
...
### S2 — Add the 5 missing skills
  - **Status**: ready
  - **Files**: `skills/delendai-operator/SKILL.md` (new),
...
### S3 — `web`/`fetch` plugin
  ...
  - **Files**: `plugins/web/package.json` (new),
    ...
    `delendai.config.json` (add `web` to the `swarm` preset as opt-in).
...
- Existing skills: `skills/delendai-plugin-authoring/SKILL.md`,
  `skills/delendai-failure-modes/SKILL.md`.
- Plugin preset config: `delendai.config.json`,
...
- **Flat `z.object` output schema, not `z.discriminatedUnion`**: the first
  ...
  *runtime* — the installed `@modelcontextprotocol/sdk`'s `outputSchema`
  ...
  `delendai-plugin-authoring/SKILL.md` as a follow-up.
