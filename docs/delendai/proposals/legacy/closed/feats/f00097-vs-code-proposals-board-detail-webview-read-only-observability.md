...
ownership:
    ...
    - { agent: implementation_runner, task: "S4: wire `delendai.openProposal` → board view selection; add `delendai.proposals.refresh` command (also bound to the existing `delendai.refresh` for parity). Update `package.json` `contributes.views` icon to use `media/logo.svg` (the activitybar container already references it). No new menu contribution until S5." }
...
1. **The activitybar container is reserved but empty.**
   ...
   `delendai.proposals` and the `delendai.openProposal` command.
...
- **Snapshot caching in `context.globalState`.** Each refresh writes
  ...
  (`delendai.proposals.snapshot`). The webview reads from cache
...
- **Status**: done
- **Files**: `docs/delendai/proposals/ready/f00097-vscode-proposals-board-and-detail-webview.md`
  (this proposal), `docs/delendai/PLUGINS-DELENDAI.md` (design note appended)
- **Agent**: proposal_guardian
- **Gate**: typecheck
- **Acceptance**:
  - "The design note in `docs/delendai/PLUGINS-DELENDAI.md` documents
...
- **Done note (2026-07-03)**: shipped by EVOLVING the existing
  ...
  is what S6 asserts) + leaves routing to `delendai.openProposal` + a
  `recoverable` banner with a `delendai.proposals.copyError` command. Filters
  ...
  **Deviations, deliberate:** (1) `views/proposals-board.css` was NOT created —
  ...
  window-focus + the `delendai.proposals.refresh` command is S4 (command
...
- **Reconciliation note (2026-07-03, S1 follow-up)**: this proposal's premise
  ...
  **f00079 S4 already shipped** `extensions/vscode/src/providers/proposal-board-provider.ts`
  ...
  with a plain invalidate-cache + `refresh()` and the `delendai.openProposal`
  ...
  `delendai_proposals_proposal_board` name. Keep the existing spec green.
...
- **Acceptance**:
  ...
  - "Refresh triggers: explicit `delendai.proposals.refresh`, window
  ...
  - "Filters persist per-session in `context.globalState` under stable
    keys (`delendai.proposals.filters.*`)."
...
- **Done note (2026-07-03)**: new `commands/proposals-commands.ts` registers
  the two commands the board owns — `delendai.proposals.refresh` (invalidates
  the shared snapshot + repaints; also on the Proposals view title bar via a
  `view/title` `$(refresh)` button) and `delendai.proposals.copyError`
  ...
  `delendai.refresh` now also calls `proposalsTree.refresh()`. `package.json`:
  the `delendai.proposals` view gains `icon: media/logo.svg`, plus the two
...
- **Gate**: typecheck
- **Acceptance**:
  - "`delendai.openProposal` opens the proposal board view, focused,
    with the existing selection."
  - "`delendai.refresh` now also refreshes the proposals board snapshot
    (calls into `proposals-snapshot.ts` invalidate)."
  - "New command `delendai.proposals.refresh` is registered, bound to
    the board's local refresh action; same handler as `delendai.refresh`
    but scoped to the proposals cache key."
  - "`package.json` `contributes.views['delendai.proposals'].icon`
...
- Design note appended to `docs/delendai/PLUGINS-DELENDAI.md`
...
- After S1 closes, the proposals plugin emits no new tools. Status
  ...
  `docs/delendai/PLUGINS-DELENDAI.md` and mirrored as a
