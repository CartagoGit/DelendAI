...
- ✅ Publicar **knowledge** que el core sirve cuando el agente lo pide
  (`delendai_knowledge` ya existe en el core — ver
...
delendai --plugins=notification,status-marker
````

### Uso por el agente (A: knowledge + tools)

1. El agente lee la tabla canónica:
   `delendai_knowledge { id: "status-marker-table" }` → bloque markdown.
...
- [x] `bun run validate` verde (typecheck + lint + tests): 634 tests
      pasan a 2026-06-20.
- [x] Carga del plugin probada con `delendai --plugins=status-marker`
      (registrado en swarm preset; `gen-capabilities.ts` lo importa;
      `delendai.config.json` raíz lo activa).
...
- [ ] **Shim de compat en Azur.lx**: fuera del repo
      `@CartagoGit/delendai`, no se toca aquí.
