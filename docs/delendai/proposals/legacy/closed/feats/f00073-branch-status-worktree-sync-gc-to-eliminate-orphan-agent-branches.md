...
- **Cache location invariant is enforced.** `branch_status` reports any worktree whose path does not start with `<cacheDir>/delendai/.worktrees/` as `outOfCache: true` so the orchestrator sees AGENTS.md violations immediately.
...
docs/delendai/proposals/       # this file
```
...
- **Acceptance**:
  ...
  - All paths returned by `branch_status` resolve through `<cacheDir>/delendai/.worktrees/` (or report `outOfCache: true`).
...
- **Files**: `plugins/proposals/skills/multi-agent-coordination/SKILL.md`, `apps/web/src/i18n/tools/delendai_proposals_branch_status.ts`, `apps/web/src/i18n/tools/delendai_proposals_branch_gc.ts`, `apps/web/src/i18n/tools/index.ts`
- **Status**: done
- **Gate**: `bun run validate`
- **Acceptance**:
...
| Worktree created outside `.cache/delendai/.worktrees/` (AGENTS.md violation) | `branch_status` flags it as `outOfCache: true` |
