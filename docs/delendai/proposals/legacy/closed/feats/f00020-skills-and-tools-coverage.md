...
- **Evidencia de atasco sin skills**: el plugin `proposals` tiene 26
  tools y `delendai.config.json` (preset por defecto) solo carga 6
...
- **Acceptance**:
  ...
  - "1 caso de smoke: importarlo desde `delendai_knowledge` no
    devuelve 404."
...
### S2 — Skill `delendai-multi-agent-coordination`
- **Files**: `skills/delendai-multi-agent-coordination/SKILL.md` (nuevo).
- **Status**: pending
- **Gate**: `bun run validate`
- **Acceptance**:
...
### S4 — Skill `delendai-token-budget-discipline`
...
- **Acceptance**:
  ...
  - "Lista de tools a **no** usar en el main thread (delegar al
    `delendai-orchestrator` subagent): `proposal_board` verbose,
...
- status: done
### S5 — Skill `delendai-status-marker-and-closure`
- **Files**: `skills/delendai-status-marker-and-closure/SKILL.md`
  (nuevo).
- **Status**: pending
- **Gate**: `bun run validate`
- **Acceptance**:
...
- status: done
### S6 — Skill `delendai-audit-runner` (plugin `audit`)
- **Files**: `skills/delendai-audit-runner/SKILL.md` (nuevo).
- **Status**: pending
- **Gate**: `bun run validate`
- **Acceptance**:
...
- status: done
### S7 — Skill `delendai-quality-and-rules-gates`
- **Files**: `skills/delendai-quality-and-rules-gates/SKILL.md`
  (nuevo).
- **Status**: pending
- **Gate**: `bun run validate`
- **Acceptance**:
  ...
  - "Cuándo `apply_rules` debe crear proposal vs aplicar: si
    ...
    pisar)."
- status: done
### S8 — Skill `delendai-legacy-proposal-migration`
- **Files**: `skills/delendai-legacy-proposal-migration/SKILL.md`
  (nuevo).
- **Status**: pending
- **Gate**: `bun run validate`
- **Acceptance**:
...
- [x] 8 SKILL.md nuevos en `/skills/` siguiendo el patrón de
      `delendai-failure-modes/SKILL.md` (frontmatter `appliesTo`,
      árbol de decisión al inicio, sección "never do", ejemplo
      de smoke).
- [x] 9 tools nuevos (`git_commit`, `git_push`, `proposals_edit`,
      `proposals_add_slice`, `package_install`, `package_run_script`,
      `quality_run_all` aggregator, `fs_read`, `fs_write`) — todos
      con `outputSchema` explícito, tests `*.spec.ts`, `effects: [...]`
      declarados.
- [x] `bun run validate` verde tras cerrar todas las slices (typecheck +
      vitest de los paquetes tocados; los fallos residuales en
      `bun run validate` global pertenecen a `plugins/audit`, en
      edición concurrente por otro agente, ajenos a esta propuesta).
- [ ] `bun run site:strict` no marca ningún tool nuevo como
      "undocumented" — pendiente de verificación en una sesión de
      generación del sitio (no bloqueante para el cierre de código).
- [x] `proposals_sync_proposals` no devuelve warnings.
- [x] `git ls-files extensions/vscode/ | grep '\.vsix$'` devuelve vacío.
- [x] `packages/client/README.md` existe.
...
- **R1 — `git_commit` + `git_push` rompen la postura read-only del
  ...
  `options.allowWrite: true` está en `delendai.config.json`. Por
...
- **R4 — `proposals_edit` puede romper el formato `.md`**: el
  test golden con `f00023` (s10; el catálogo no tiene `f00028`
  como ese fixture — se usó `docs/proposals/done/feats/f00023-plugins-depth-extension.md`,
  el archivo real, como golden) detecta regresiones de parseo;
...
- **R5 — Carga cognitiva de 8 skills nuevas**: la decisión de
  ...
  skill `proposals-workflow-playbook` (s1) es el primero que
...
### Closure rationale (decisions made without re-asking the user)

- **S3/S4/S5/S6/S7/S8** corrigieron varias afirmaciones desactualizadas
  del texto original durante la implementación: `PUBLISH_ORDER` real
  tiene 10 paquetes (no 12); el spec de presupuesto de tokens real es
  `token-budget.e2e.spec.ts`; los tools de `status-marker` siguen el
  patrón `<prefix>_close`/`<prefix>_validate` namespaced, no nombres
  literales; `audit` se activa con `--preset=full` o `--plugins=audit`
  (no `swarm`, que lo excluye explícitamente); la rúbrica de auditoría
  real tiene 6 bandas, no 5; `apply_rules` nunca escribe en disco por sí
  mismo (siempre devuelve un plan); los scripts de migración legacy
  reales se llaman `migrate-legacy.script.ts` / `rewrite-refs.script.ts`
  / `normalize-legacy.script.ts`.
- **S9/S10** usaron el archivo real `docs/proposals/done/feats/f00023-plugins-depth-extension.md`
  como fixture golden para `proposals_edit`/`proposals_add_slice`,
  porque `f00028-plugins-depth-extension.md` (citado en el texto
  original de S10) no existe con ese nombre.
- **S11/S12** dejaron `package_install`/`package_run_script` y
  `quality_run_all`/`fs_read`/`fs_write` implementados como builders
  exportados (`buildDepsWriteToolRegistrations`, `buildRunAllToolRegistration`,
  `buildFsToolRegistrations`) sin wiring porque sus archivos `index.ts`
  no estaban en el lock de esas slices (para evitar colisión con otro
  agente activo); el hilo principal completó el wiring: `package_install`/
  `package_run_script` quedaron detrás de `plugins.deps.options.allowWrite:
  true` (mismo patrón que `allowNetwork`); `quality_run_all` se añadió
  siempre disponible junto a `run_quality`/`get_quality_scopes`; `fs_read`/
  `fs_write` se registraron como tools de core en `assemble.ts` junto a
  `buildBootstrapToolRegistrations` (sin flag adicional — mismo nivel de
  confianza que `delendai_scaffold`/`delendai_create_project`, que ya
  escriben en el workspace sin un flag propio, protegidos solo por
  `resolveWorkspaceContained`).
- **S9** (`git_commit`/`git_push`) quedó detrás de `plugins.git.options.allowWrite:
  true`, implementado por el propio agente de esa slice siguiendo el
  mismo patrón opt-in.
- El golden test de `quality.spec.ts` se actualizó para incluir
  `quality_run_all` en la lista esperada de tool ids tras el wiring.
