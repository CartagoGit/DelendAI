...
2. **In the log**: `tool-started { kind: 'tool-started', tool:
   'delendai_overview', agent: null, taskId: null, ts: T0 }`.
...
4. **What the log shows**: `tool-started { tool:
   'delendai_overview' }` followed *minutes later* by
   `tool-completed { tool: 'delendai_overview', outcome: 'ok' }`.
...
### S1 — Plugin skeleton + persistence + redaction + rotation *(excl. `plugins/logs/`, `.cache/delendai/logs/`)*
...
- [x] `.cache/delendai/logs/<date>.jsonl` appends one line per
...
- [x] Manual smoke: every documented failure mode of every tool
      ...
      `delendai-failure-modes` skill convention), not an
      unhandled exception.
...
- The log is **per-host**, not per-workspace. A single MCP server
  process owns its `.cache/delendai/logs/`. Multi-host aggregation
  is out of scope.
