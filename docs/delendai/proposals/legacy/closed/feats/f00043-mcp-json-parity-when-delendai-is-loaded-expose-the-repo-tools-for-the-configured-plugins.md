...
# f00050 — mcp.json parity — when delendai is loaded, expose the repo tools for the configured plugins
...
- **Files**: tools/scripts/host/host-server.script.ts, packages/core/src/lib/plugins/parse-cli-args.ts, delendai.config.json
- **Status**: done
...
- **Delivered**:
  - `docs/README-DELENDAI.md` — replaced the "Install / register" section
    ...
    `--preset → --plugins → delendai.config.json#plugins → --exclude-plugins`
    resolution order, with the concrete worked example of this repo's
    own `.vscode/mcp.json` + `delendai.config.json` surface.
