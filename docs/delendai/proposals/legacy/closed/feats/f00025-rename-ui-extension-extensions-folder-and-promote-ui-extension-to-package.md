...
shipped-via: parallel + orchestrator. S1 (apps/ide → packages/ui-extension) and S3 (extensions/vscode → extensions/vscode) were committed by parallel agents before the orchestrator reached them; verified via `git log --follow`. S2, S4-S8 + fixup authored by `delendai-orchestrator`.
reservedFiles:
    - apps/ide/
...
related:
    ...
    - f00034 # CLI delendai single binary — its `packages/cli/` move is the precedent for non-app workspace packages
...
ownership:
    ...
    - {
          agent: implementation_runner,
          task: 'S3: git mv extensions/vscode → extensions/vscode + rename package.json name "delendai-vscode" → "@delendai/extension-vscode"',
      }
    - {
          agent: implementation_runner,
...
- **No bump major de `@delendai/core`, `@delendai/client`,
  ...
  `delendai-vscode` → `@delendai/extension-vscode` reciben un
...
- En `extensions/vscode/package.json`:
  - `name`: `delendai-vscode` → `@delendai/extension-vscode`
...
- `cd extensions/vscode && bun run package` — produce el `.vsix`
  con el nuevo nombre en `dist/extensions/vscode/<version>/@delendai-extension-vscode-<version>.vsix`
...
- [`CHANGELOG.md`](CHANGELOG.md) — añadir entrada bajo la sección
  ...
  - `@delendai/ide` renombrado a `@delendai/ui-extension`.
    ...
    `packages/ui-extension`).
  - `delendai-vscode` renombrado a `@delendai/extension-vscode`.
...
- **R1 — `vsce` rechaza el nuevo package name `@delendai/extension-vscode`**.
  ...
  `delendai-vscode` como `name` y añadir `@delendai/extension-vscode`
  como `displayName`.
...
| `delendai-vscode` (package) | `@delendai/extension-vscode` (package) |
