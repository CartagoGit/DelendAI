...
ownership:
    ...
    - { agent: implementation_runner, task: 'S2: replace the hardcoded `/home/cartago/_proyectos/propios/delendai/tools/scripts/...` path in `renderVscodeMcpJson` with a resolver that picks the canonical install path for the consumer (npm-installed `@delendai/core`, sibling checkout, or explicit `--delendai-root` flag)' }
    - { agent: implementation_runner, task: 'S3: localize the agent descriptor fallback (init-catalog.ts) using the operator locale; honour the resolved namespace prefix (not the hardcoded `delendai_*`) so generated `.github/agents/*.md` tools match what `init` actually produces in `delendai.config.json`' }
    - { agent: implementation_runner, task: 'S4: respect the consumer convention for generated code paths — when the project uses `libs/` (Angular/monorepo) or `src/` (NestJS) or `packages/` (yarn workspaces), write under the matching root instead of always `plugins/<name>/`; document the discovered root in `delendai.config.json#pluginPathsRoot`' }
...
Make `delendai init` (f00084) **integrate with the target project** instead of
overwriting it with delendai's own defaults. Today, the command:

- **Hardcodes a path** to `/home/cartago/_proyectos/propios/delendai/tools/scripts/host/host-server.script.ts` in every generated `.vscode/mcp.json` ([init-render.ts:103-119](packages/cli/src/commands/init/init-render.ts#L103-L119)). The path only resolves in the operator's own checkout — every other consumer gets a broken `.vscode/mcp.json` until they hand-edit it.
...
- **Hardcodes `delendai_*` tool names** in those same agents — but the resolved namespace prefix may be `acme_*`, `lx_*`, etc. when the user follows f00081 (namespace-aware client services). The generated agents reference tools the project does not actually expose.
- **Writes under hardcoded paths** (`plugins/<name>/`, `docs/delendai/`, `.github/agents/`) without inspecting what the project already uses. A NestJS project that lives under `src/`, an Angular workspace under `libs/`, or a yarn-workspaces monorepo under `packages/<scope>/<pkg>/` all get `delendai`-shaped folders that fight their own conventions.
...
✓ namespace prefix: 'delendai' (default; press Enter to keep, type to override)
?  How to centralize host-instructions? (1-3) [1]: 1
?  Copy core skills into libs/delendai/skills/? (y/n) [y]: y
?  Generate .github/agents/delendai-*.agent.md from the live catalog? (y/n) [y]: y
?  Scaffold the first migration proposal? (y/n) [y]: y
   ✓ wrote delendai.config.json (pluginPathsRoot: libs/)
   ✓ wrote .vscode/mcp.json (host path: @delendai/core — npm install)
   ✓ wrote libs/delendai/skills/manifest.json
   ✓ wrote .github/agents/delendai-orchestrator.agent.md (English, namespace=delendai)
   ✓ wrote docs/delendai/proposals/ready/f00001-migrate-legacy-my-angular-app.md
```
...
The default `delendai` namespace comes from the CLI's `--prefix` flag
...
the tools they reference (`delendai_proposals_auto_work` etc.) match
...
- No change to the canonical `delendai.config.json` schema.
...
```ts
/**
 * Resolve the absolute path to delendai's host-server script in the
 * order documented in PLUGINS-DELENDAI.md:
 *   1. `--delendai-root=<abs>` flag (explicit override)
 *   2. `<workspace>/node_modules/@delendai/core/tools/scripts/host/host-server.script.ts`
 *   3. `<workspace>/../delendai/tools/scripts/host/host-server.script.ts`
...
Every `tools: ['delendai_proposals_auto_work']` in the fallback becomes
...
- [`init-render.ts`](packages/cli/src/commands/init/init-render.ts) — no path change needed (only writes `delendai.config.json` + `.vscode/mcp.json` + agent files + host instructions + migration proposal, none under `plugins/`).
...
- **Gate**: bun run validate
- **Acceptance**:
  - `resolveHostEntryPath` returns the explicit override when `--delendai-root` is set.
...
- **Acceptance**:
  ...
  - `renderDelendaiConfig` writes a `convention` block with the discovered root and source root.
  - `tools/scripts/create-plugin.ts` accepts `--root=<path>` and defaults to `pluginPathsRoot` when the current directory contains a `delendai.config.json` with a `convention` block.
...
> + spec; S2 `host-entry-resolver.service.ts` + spec; the `--delendai-root` /
...
- The validation gate (`bun run validate`) is green for the four slices
  above.
- `bun run cli -- init --help` exits 0 and the new `--delendai-root`
...
2. **`hostEntryPath` resolution might disagree with what the operator
   ...
   `--delendai-root` as the override. We do NOT silently default to
...
4. **The new `convention` block in `delendai.config.json` is ignored by
...
- The proposal is additive: an empty workspace still produces the same
  ...
  change to `delendai.config.json` consumers — the new `convention`
  block is optional.
...
- **Skill inventory** (`init-skill-inventory.ts`, new): list delendai's own
  skills (from `docs/delendai/skills/`) and scan the target for existing
...
- **Tool-namespace map** (`init-adoption-plan.ts`, new): list the resolved
  delendai tool ids (already namespaced via the prefix from S3) and the
...
- Produce **one canonical source of truth** (the bootstrap doc) plus thin host
  ...
  `<!-- delendai:begin -->`/`end` block already used by the centralizer.
