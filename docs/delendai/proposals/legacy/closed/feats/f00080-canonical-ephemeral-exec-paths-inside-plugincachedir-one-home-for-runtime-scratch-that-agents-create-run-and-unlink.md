...
ownership:
  ...
  - { agent: implementation_runner, task: 'S4: document the canonical rule in AGENTS.md, docs/delendai/FILE-CONVENTIONS.md, and the plugin authoring skill' }
globalGate: validate
acceptance:
...
1. The **handoff packets** the proposals plugin writes when the
   loop-detector fires — `.cache/delendai/handoff/*.json` — are already
   inside the canonical cache, but the resolver hardcodes the
   `.cache/delendai` literal when a host reconfigures the cache root
...
human can `ls .cache/delendai/<plugin>/exec/` to debug a misbehaving
agent.
...
- **Files**:
  ...
  - `tools/scripts/verify/plugin-tool-verify.script.ts` (move
    the old .verify-tmp/ path to `.cache/delendai/.scratch/verify/<pid>/` and
...
- **Files**:
  - `AGENTS.md` (extend the "Hard rules" section with rule #12 — see
    above)
  - `docs/delendai/FILE-CONVENTIONS.md` (link to the helper + lint
