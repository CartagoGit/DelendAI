...
Audit `a00045 H8` found that three services in `packages/client/src/lib/services/` hardcode the `delendai_` namespace in their `request(...)` calls:

- `overview.service.ts:22` → `'delendai_overview'`
- `dashboard.service.ts:92` → `'delendai_overview'`
- `notifications.service.ts:102` → `'delendai_notification_notify_status'`
...
The core already exposes `snap.namespacePrefix` (returned by `delendai_overview { compact: true }`) and the assemble CLI accepts `--prefix`. The client just needs to thread it through.

## non-goals

- No new tools, no new contracts, no new plugin.
- No breaking changes to existing default-prefix consumers (default = `'delendai_'` preserves current behaviour).
...
Define an interface or type for the namespace prefix and add an optional `namespacePrefix?: string` parameter to each service constructor. Default to `'delendai_'` when omitted.
...
- **Files**:
    ...
    - one shared helper `packages/client/src/lib/services/_namespace.ts` [CREATE] exposing `formatToolName(prefix, suffix)` and `parsePrefix(raw)` with `prefix ?? 'delendai_'` semantics
- **Gate**: `bun run test`
...
- **Landed**: commit `52b24fd4`. Added `_namespace.ts` (`formatToolName`,
  ...
  `delendai_*` request in those four services is now composed via
  ...
  default → `delendai_*`, custom → `<prefix>_*`, missing → default).

### S2 — Wire `namespacePrefix` through the VS Code host

The VS Code host already knows the prefix (it's the workspace's `delendai.config.json` `server.prefix` field, or the default). Pipe it through `McpStdioClient` construction and into the service constructors.
...
- **Landed**: commit `4f725d0a`. Implementation deviated from the named
  ...
  `delendai.server.prefix` (declared as a new config contribution in
  `extensions/vscode/package.json`) and threaded into the Overview /
  Notifications services, the `DelendaiStatusBar` (which has its own
...
1. Server reports its prefix via `delendai_overview { compact: true }`.
...
- **Files**:
    ...
    - `docs/delendai/IDE-EXTENSION.md` [MODIFY — add "namespace-aware client" subsection]
    - `docs/delendai/examples/minimal-host/*` [MODIFY — show the new constructor signature]
...
- **Landed**: commit `43ac9b58`. Added a "Namespace-aware services" section
  ...
  client" subsection to `docs/delendai/IDE-EXTENSION.md` (config example +
  the `delendai.server.prefix` flow). `docs/delendai/examples/minimal-host/`
...
- The default constructor `OverviewService()` continues to call `delendai_overview` (no behaviour change for existing consumers).
- `bun run validate` exits 0; existing tests still pass; new tests cover at least:
    - Default prefix produces `delendai_overview`.
...
Low. The change is additive: every existing call site passes through `formatToolName(prefix ?? 'delendai_', suffix)` and the default matches current behaviour bit-for-bit. The only real risk is in `extensions/vscode` test fixtures; if a test was previously asserting the literal string `'delendai_overview'` it will need updating to assert `formatToolName(undefined, 'overview')` instead.