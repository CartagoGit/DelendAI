...
- **Acceptance**:
  ...
  - Manual reload of the VS Code window shows no leaked
    `delendai` items in the status bar after 5 reloads.
...
- **Gate**: `bun run test`.
- **Acceptance**:
  - The hardcoded array (with `copilot · minimax-m3`, `delendai`, etc.)
...
- **Status**: done. search/docs/deps/git each gained an explicit
  ...
  rejected before any tool is wired. The repo's `delendai.config.json`
...
- **Acceptance**:
  ...
  - `delendai.config.json` of this repo still validates with no
