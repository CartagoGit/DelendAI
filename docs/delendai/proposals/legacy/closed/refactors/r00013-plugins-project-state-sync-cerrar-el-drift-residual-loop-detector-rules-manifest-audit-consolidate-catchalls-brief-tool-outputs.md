...
- gate: lint
- acceptance:
  - "Reemplazar path.resolve(options.workspaceRoot, relDir) por resolveWorkspaceContained(options.workspaceRoot, relDir) del barrel @delendai/core/public."
...
- Las propuestas puntuales `f00020` (race en `sync-proposal-registry.ts:331`), `f00019` (sync I/O en `notification/watcher.ts`), `r00001` y `r00002` (catchalls residuales) cierran **un hallazgo cada una** pero dejan sin tocar:
  ...
  4. **Catchalls residuales** en `rules/get_rules.areas[].rules` y `proposals/adopt.layout` (los otros catchalls están cubiertos por r00001/r00002).
  5. **Audit brief desactualizado** — no menciona `delendai_metrics`, ni `keepLegacy`, ni el invariante "tool-outputs commiteado".
...
- [ ] `audit/brief.ts` menciona `delendai_metrics`, `keepLegacy`, y `tool-outputs.ts` commiteado.
...
| F-07 | P2 | `audit/brief.ts` no menciona `delendai_metrics` | [brief.ts:130](plugins/audit/src/lib/brief.ts) | slice `s5` |
...
- **Primitivas del core usadas**: `writeFileAtomic`, `withFileMutex`, `resolveWorkspaceContained`, `redactSecrets`, `walkAllowedFiles` (todas del barrel `@delendai/core/public`).
- **Relación con `delendai_metrics`**: el spec `s7` registra los anti-patrones en cada test; `delendai_metrics` (M29) los observará en producción tras el primer release post-merge.
