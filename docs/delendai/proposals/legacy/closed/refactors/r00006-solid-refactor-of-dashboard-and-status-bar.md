...
ownership:
    ...
    - { agent: implementation_runner, task: 'S7: `DelendaiStatusBar` `STATUS_BAR_EVENTS` becomes a discriminated union keyed by event + locale (H30 OCP)' }
globalGate: validate
acceptance:
...
- **H22** — [`render-panel-tools.ts`](packages/ui-extension/src/renderers/render-panel-tools.ts )
  ...
  `delendai.metrics` but the renderer ignores it.
