...
- **Riesgo**: olvidos futuros — alguien añade una tercera ruta de creación de worktrees (p. ej. `proposals_spawn`, `setup-github`) y olvida propagar `worktreesDirRel`. **Mitigación**: `q00018` (Scope Model) introduce una única autoridad `ResolvedWorktreeScope` que ningún caller podrá eludir. Este fix lo deja todo listo para ese próximo paso.

## notes

- `q00018` (futuro): Repository / Worktree / Coordination Scope Model — la propuesta de plan que define `IStateScope` y los scopes `project` / `swarm` / `worktree`, y que se apoyará en este fix como pre-requisito.
