...
- Core dogfooding rule — the repo will adopt `defaultMode: "auto"` in
  `delendai.config.json` once S5 closes (not part of S1..S4).

## Architecture

```
                ┌────────────────────────────────────────────────────┐
                │           delendai.config.json (S5)              │
                │  policy:                                          │
                │    defaultMode: "auto"                            │
                │    defaults:                                      │
...
- **Status**: pending
- **Files**: `delendai.config.json`, `apps/web/` docs and integration tests
- **Gate**: `bun run validate`

**Goal**: this repo adopts `agent-orchestrator` with `defaultMode: "auto"`
via `delendai.config.json → orchestrator.policy`. Proves the plugin
...
1. `delendai_overview` once per session (no fan-out of knowledge
...
3. The orchestrator subagent (`delendai-orchestrator`) drives the
