...
To inspect the repository that hosts `delendai`, enable the `github` plugin and configure the actual GitHub repository explicitly. The plugin works without the `git` plugin, without a local checkout, and without a configured `origin` remote.

- Set `GITHUB_TOKEN` in the process environment. Never place the token in `delendai.config.json`, source files, proposal files, logs, snapshots, or tool arguments.
...
- Set the default `owner` and `repository` only when useful. Otherwise pass them explicitly to each tool. They must identify the GitHub repository that actually contains `delendai`; do not rely on the local directory name.
