...
contains:
    proposals:
        ...
        # ─── Track I — CLI / delendai doctor / web (P1) ──────────────────────────
...
last-transition-id: t-2026-09-07-q00006-done
last-correlation-id: c-2026-09-07-q00006-final
last-transition-from: in-progress
last-idempotency-key: idem-2026-09-07-q00006-done
shipped-in: closureGate.requireAllChildrenDone satisfied (63 done + 2 retired across 65 children)
...
`docs/delendai/AGENT-BOOTSTRAP.md §6`; este plan las re-enfatiza y
...
- **Files (expected):**
  ...
  - `docs/delendai/GOVERNANCE-BRANCH-PROTECTION.md` (instrucciones)
...
- **Acceptance:**
  - Script pasa contra la API real (o contra un mock que represente
    la policy declarada).
  - Documento versionado en `docs/delendai/`.
...
- **Files (expected):**
  ...
  - `docs/delendai/CORE-PUBLIC-API-INVENTORY.md` (generado)
- **Behavior desired:**
...
- **Files:**
  - `docs/delendai/CAPABILITY-MATRIX.md` (generado)
...
- **Files:** `tools/scripts/gen/all.script.ts`, `docs/delendai/*.md`
  regenerados.
...
> **Goal:** introducir `delendai doctor` y arreglar drift web.

### I.0 — Tabla de hijas

| ID       | Kind  | Priority | Title                                          |
| -------- | ----- | -------- | ---------------------------------------------- |
| `f00191` | feat  | P1       | `delendai doctor`: health check completo           |
| `c00142` | chore | P1       | Web: regenerar docs/capacities desde manifests |

### I.1 — Detalle por hija

#### `f00191` — `delendai doctor`
...
| I     | `f00191` | feat         | P1       | pending | —            | delendai doctor                     |
...
# Cómo ejecutar este plan con `delendai`

1. `delendai_overview { compact: true }` (orientación).
2. `delendai_proposals_auto_work` para seleccionar la siguiente
   hija claimable.
3. `delendai_proposals_continue_proposal { id, mode: "plan" }` para
   inspeccionar slices.
4. `delendai_proposals_delegate { taskId, slot, files }` para
...
7. `delendai_proposals_close_slice { id, sliceId }`.
8. Tras cerrar todas las hijas, `delendai_proposals_close_plan
...
REPOSITORIO: CartagoGit/delendai
...
1. `delendai_proposals_sync_proposals` para regenerar el índice.
2. `delendai_proposals_close_plan { planId: "q00006", reason: "..." }`.
