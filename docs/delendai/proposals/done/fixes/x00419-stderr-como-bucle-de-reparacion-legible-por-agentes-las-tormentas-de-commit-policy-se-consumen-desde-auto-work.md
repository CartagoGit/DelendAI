...
`.cache/delendai/storms/<hash>.json` per storm. The hash is
...
2. For each storm where `exceedsThreshold === true` AND
  `sampleProposalIds.length >= 3`, create a `kind: fix`
  proposal under `docs/delendai/proposals/ready/fixes/`:
   ```
  xNNNNN-{kebab-code}.md
   ```
3. Body is generated by the commit-policy repair proposer with a
  canonical proposal shape (`kind: fix`, `status: ready`,
  `## Slices`, acceptance bullets) and the source-file hint from
  `suggestedFix`.
...
5. Write a `kind: fix` proposal under `ready/fixes/` with the
  storm-derived source-file hint in S1.
