...
- review-log: approved by sonnet-worker-tests — Ran npx vitest run plugins/commit-policy/tests/integration/cross-agent.spec.ts: 5/5 tests green covering all AUD-CP-005 scenarios (isolated stage/commit with A staged, isolated stage/commit with A dirty-unstaged, shared-registry listener isolation, reload-no-duplicate-commit, parallel A/B commit isolation).
## acceptance
