...
priority: P2
shipped-in:
    - 4facbef9d # S1 acción compuesta setup-bun-repo
    - 92f84e0be # S2 eliminar duplicación tier2 vs ci.yml
    - a19fe9848 # S3 lint workflow-command-duplication
related:
    - q00011
...
1. [x] Un PR a `develop` no ejecuta el mismo comando (`typecheck`,
...
2. [x] El bloque checkout+setup-bun+install vive en un único fichero
   (`.github/actions/setup-bun-repo/action.yml`) consumido por los 4
   workflows tier, en vez de estar copiado en cada job.
3. [x] `bun tools/scripts/lint/workflow-command-duplication.script.ts`
   falla contra un fixture con el mismo comando en dos jobs del mismo
   evento, y pasa contra el estado post-refactor.
4. [x] Tiempo total de feedback de un PR (suma de duración de todos los
   workflows disparados) documentado antes/después en esta propuesta,
   mostrando una reducción medible (ver `92f84e0be`).
5. [x] `bun tools/scripts/lint/proposals.script.ts` sin errores ni
