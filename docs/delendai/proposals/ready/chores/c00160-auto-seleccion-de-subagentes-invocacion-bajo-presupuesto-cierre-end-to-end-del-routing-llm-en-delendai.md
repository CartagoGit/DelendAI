...
related:
    ...
    - docs/delendai/TOKEN-BUDGETS.md # source of truth de presupuestos por tool
    - docs/delendai/AGENT-BOOTSTRAP.md §1 # orientación "one cheap call"
...
- `delendai_proposals_delegate` — atomic handoff (símbolo + lock).
- `delendai_agent_names` — registro nombrado.
...
`684c902cf chore: update delendai.config.json,
...
Las seis hijas + TEST están `status: done` en `docs/delendai/proposals/done/`:
...
| S5 — dogfood `defaultMode: auto` | f00186 | ver `git log -1 --oneline -- delendai.config.json` | Shipped pre-S6 |
...
- `TOKEN_BUDGETS` en `@delendai/core/public` — cada tool declara su
  presupuesto.
- Skill `delendai-token-budget-playbook` en `packages/core/skills/`.
- `delendai_proposals_round_context` — digest para reanudar swarms.
...
```
  ...
  Cross-cutting:
    ...
    • skills/delendai-token-budget-playbook        — disciplina de coste
...
- **Why-needed**: hoy, ningún lint detecta si los cinco plugins del scope
  ...
  `delendai --plugins=...` y solo enterarse del fallo en runtime. El
...
- **Files**:
  ...
  - `delendai.config.json` (re-verify; cambios mínimos si los hay)
...
- **R3 — Dependencia dura `orchestrator-runner` ⇒ `usage-tracking`**:
  ...
  *"Depends on the usage-tracking plugin. Load with
  `delendai --plugins=usage-tracking,orchestrator-runner`"*. Si el
...
- **`f00183..f00187 + t00007` están en `done/`**: el cuerpo del plan
  ...
  `git log --oneline -- docs/delendai/proposals/done/feats/f0018*`
...
cd /home/cartago/_projects/delendai/.worktrees/q00018
...
tool `delendai_proposals_agent_lock` (uno por hija; el path es la
...
delendai_proposals_agent_lock {
  files: ["docs/delendai/proposals/done/feats/f00186-agent-orchestrator-s5-dogfood-on-develop-with-defaultmode-auto-regen.md",
...
| **C.2** | Crear la hija `<NEW-1>` con id del servidor | `delendai_proposals_create_proposal { kind:"chore", title:"Sniffer cross-plugin de coherencia entre los 5 plugins de routing", goal:"...", track:"routing-coherence" }` | Devuelve `id` (algo tipo `c00161`) y path |
...
| **C.5** | Cerrar Track 1 vía `proposal_review action=submit` y luego `proposal_review action=approve` (debe hacerlo un agente diferente; para auto-flujo usar `force:true` con justificación) | `delendai_proposals_proposal_review` | Review-state: done + lock liberado |
| **C.6** | Repetir C.2-C.5 para `<NEW-2>` | — | Idem |
| **C.7** | Repetir C.2-C.5 para `<NEW-3>` | — | Idem |
| **C.8** | Cerrar c00160 | `delendai_proposals_proposal_transition { id:"c00160", to:"done", reason:"..." }` | c00160 → `in-progress/` → `done/` |
...
5. **NO** añadir nuevos commits `Co-authored-by:` ni atribución
   LLM (regla `f00500`).
6. **SÍ** usar `delendai_overview { compact:true }` como primera
...
| `proposal_board` no muestra las hijas aún | Estás viendo antes de la primera sincronización | Esperar 30s y reintentar; o invocar `delendai_proposals_sync_proposals` |
...
| Aparece un `agent-dead` notification | El agente remoto crasheó | `delendai_proposals_state_health` para diagnóstico, luego `state_repair` con `dryRun:true` primero |
