---
applies-to: docs/delendai/proposals/**/a*.md
---
...
- **Pending Tasks**: If the audit has tasks or slices to be executed within its own scope, the proposal must be created under `docs/delendai/proposals/ready/` with `status: ready` and all slices set to `pending`.
- **No Internal Tasks**: If the audit has no internal tasks (e.g., all findings are deferred to separate proposals, or there are no findings), the audit should be created directly under `docs/delendai/proposals/done/audits/` with `status: done` and reference/link the deferred proposals.
...
- Read `docs/delendai/skills/` and check alignment between skill instructions and current implementation (stale paths, wrong tool names, missing new tools).
...
- files: docs/delendai/proposals/in-progress/aNNN-auditoria-[slug].md
- gate: lint
- status: pending
...
Before running an audit, read `docs/delendai/skills/delendai-audit-playbook/SKILL.md` — it contains the step-by-step protocol for the mandatory qualitative LLM code-reading phases that must precede writing any audit document.
