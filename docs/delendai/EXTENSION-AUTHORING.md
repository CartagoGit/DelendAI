...
- **Any-language host**: speak MCP over stdio to the configured
  `delendai` server, call the same public tools, validate their JSON
...
1. Launch or connect to the configured `delendai` MCP server over stdio.
...
`create_project { kind: "host", existingDelendai: true|false }` is the
...
`existingDelendai: true` switches the scaffolder into **non-invasive**
mode: it skips emitting `libs/mcp-project/`, `src/server.ts`,
`src/index.ts`, `src/lib/shared/host-config.ts`, and `.vscode/mcp.json`
because the project already wires delendai via its own
`delendai.config.json` + `plugins/` layout. The agents, instructions
and skill are still emitted (those are the contract surface any host
needs to honour `AGENT-BOOTSTRAP.md`).

**x00201: both `existingDelendai` and `mcpServerName` are optional —
...
`delendai.config.json` presence and `.vscode/mcp.json` / `.mcp.json`
server entries for an delendai-shaped launch, and:
...
- `mcpServerName` is the real MCP server registration key
  ...
  (routinely just `delendai`). Passing the wrong (or default, when
  guest-mode) key produces agent files whose first tool call addresses a
  server that does not exist.

When to use `existingDelendai: true`:

- The project keeps delendai as a **tooling guest** (not its own MCP
  server). It mounts the repo-local delendai via `.vscode/mcp.json`
  pointing at the host's `host-server.script.ts` and loads extra
  plugins (e.g. a domain-specific Postman generator) through
  `delendai.config.json → plugins`.
- The project has been on `delendai init` once already and is being
  re-bootstrapped after a shape change — `create_project` re-emits the
  agents / instructions / skill without disturbing the working
  `delendai.config.json` + plugin set.

When to omit it (default `false` — greenfield):

- The project has **never** integrated delendai before.
...
regardless of `existingDelendai`. Dropping one would silently break
the docs that tell every Claude Code / Codex / Copilot Chat session to
delegate to the orchestrator subagent (see
`docs/delendai/AGENT-BOOTSTRAP.md` §8.1, §8.2, §8.3).
