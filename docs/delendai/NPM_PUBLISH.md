...
## 1. Validar antes de publicar (desde la raíz del repo `/_projects/delendai`)
```bash
bun install
...
Cualquier consumidor que estuviera enlazando `delendai` por **rutas locales**
(tsconfig paths + alias de vitest apuntando a `../../../delendai/...`)
...
- Sube versión con `npm version patch|minor` en cada paquete que cambie (o usa
  changesets si quieres automatizarlo). Mantén `delendai` y plugins compatibles
