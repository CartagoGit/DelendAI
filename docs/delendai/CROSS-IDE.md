...
`@delendai` IDE host. Every other IDE (JetBrains, Zed, Cursor,
...
`delendai` and `delendai` both point at `packages/cli/dist/index.js`.
...
`bunx --package @delendai/cli delendai __serve --workspace …`. The executable
uses the same core assembly path as every host:
it resolves preset plugins, explicit `--plugins`, plugins declared in
`delendai.config.json`, and finally subtracts `--exclude-plugins`.

That means "delendai is loaded" implies the configured repo plugin tools are
loaded too. A compact `delendai_overview { "compact": true }` includes
...
## Host discovery surface

The unified discovery surface (`delendai_overview` + `delendai_agent_catalog`)
is the same across every host. For hosts that still rely on checked-in
instruction files (Copilot Chat, Claude Code, Cursor, generic AGENTS
consumers), the canonical discovery block is regenerated by
`bun run catalog:hints` and lives at:

- `docs/delendai/host-hints/agent-instructions.generated.md` (single fragment, f00092)

The single fragment is the same for every host — the only host-specific
content is a 1-line footnote pointing at an appendix in the universal
bootstrap (Copilot §8.1, Claude §8.2, AGENTS §7), and that footnote now
lives inline in each hand-edited host file between its
`<!-- delendai:begin -->` / `<!-- delendai:end -->` markers.
The renderer and the lint both assert the single-fragment invariant
and fail loudly if a second `*.generated.md` file appears under
`docs/delendai/host-hints/`.

The generator reads `docs/delendai/agent-catalog.generated.json`, which is
itself produced from the live tool registry, `packages/core/skills/manifest.json`,
and `docs/delendai/proposals/index.json`. Drift in any of the four inputs
fails `bun run catalog:check` / `bun run catalog:hints:check`. A pre-commit
hook (`lefthook.yml` → `host-hints-drift-check`) regenerates the fragment
and prints the diff; CI runs the same check on every push.

### Cross-IDE: configuring the issues plugin in any host

When you need the GitHub issues tools in VS Code, Cursor, Claude Code, or another host, use [CROSS-PROJECT-SETUP.md](./CROSS-PROJECT-SETUP.md) as the canonical workflow. It keeps the per-repo `delendai.config.json` entry, the chosen preset or explicit plugin list, and the GitHub auth tier check aligned so every host launches the same effective `issues` surface.
...
Every chat client listed in [README-DELENDAI.md § Install / register](./README-DELENDAI.md#install--register)
...
> [README-DELENDAI.md](./README-DELENDAI.md#install--register))
...
4. **Wire commands**: register `delendai.openDashboard`,
   `delendai.openDocs`, `delendai.refresh` against the host's
   command palette.
...
- `delendai.toolSearch` — opens a QuickPick over the live tool
  registry + knowledge entries (from `SearchService`).
- `delendai.restartServer` — re-spawns the MCP server. Hosts
...
status-bar `delendai.restartServer` falls back to an info message
