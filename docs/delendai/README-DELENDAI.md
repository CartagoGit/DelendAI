...
## Brand origin — *AI delenda est*

The name **DelendAI** borrows the construction of Cato the Elder's
*Carthago delenda est* — "Carthage must be destroyed", the phrase he ended
every Roman senate speech with until the Third Punic War did the thing
itself.
...
what it has touched. DelendAI is what we are building instead — named
tools, hermetic runtime, public errors and private data.

The full brand contract (when to write `delendai` vs `DelendAI`, the
reference table, and the gates that enforce it) lives in
[`BRAND.md`](./BRAND.md).
