...
- You are registering `delendai` in a project for the first time and want one known-good launch shape.
...
- You need to debug a mismatch between what your host launches from `mcp.json` and what the repo declares in `delendai.config.json`.
...
authority — delendai never replaces them.

```text
delendai_adopt_project { write: true }
```
...
|---|---|
| Derived config (preset expanded to the `plugins` map) | `delendai.config.json` |
...
2. **Optional GitHub issues** — run `delendai_setup_github` (read-only
...
`delendai` keeps session lifetime and context carry-over consistent across
...
| 2 | Config | Add the `plugins.issues.options.repo` block to `delendai.config.json` (or run `init_config` with `write: true` to derive the whole config). |
| 3 | Load | Launch the host with `--plugins=proposals,issues` (issues hard-depends on proposals). |
| 4 | Verify | Run `delendai issues list` (or call the `issues_list` MCP tool) to confirm the repo + auth tier resolve. |
...
|---|---|---|
| `gh` | You are on a development machine and can sign in interactively with GitHub CLI. | Install `gh`, run `gh auth login`, then confirm `gh auth status` is healthy before launching `delendai`. |
| `rest-authed` | You are in CI, a remote shell, or any environment where a token is easier than an interactive login. | Export `GITHUB_TOKEN` before starting the host and keep the token outside `delendai.config.json`. |
...
```bash
bunx --package @delendai/cli delendai __serve --workspace . --plugins proposals,issues
```
...
```jsonc
{
	"servers": {
		"delendai": {
			"command": "bunx",
			"args": ["--package", "@delendai/cli", "delendai", "__serve", "--workspace", ".", "--preset", "full"]
		}
	}
}
```
...
```bash
bunx --package @delendai/cli delendai __serve --workspace . --preset swarm --plugins usage-tracking,orchestrator-runner
```

The router needs a **provider roster**. The canonical home is a root-level
`providers` block in `delendai.config.json`; the two plugins stay opt-in under
...
| `delendai.config.json` not found | Create `delendai.config.json` at the workspace root, add the `plugins.issues.options.repo` block shown above, then relaunch the host so it reads the config file from the expected root. |
...
| Host loads `delendai` but no issues tools appear | Your `mcp.json` launch shape is missing the required preset/plugin flags. Change it to `--preset=full` or `--plugins=proposals,issues`, then restart the host and confirm the compact overview lists `issues` as loaded. |
...
- [PLUGINS-DELENDAI.md](./PLUGINS-DELENDAI.md) for plugin authoring and the issues-plugin setup note.
