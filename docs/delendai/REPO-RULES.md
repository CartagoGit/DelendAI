...
- `apps/web` — Astro product/docs site, generated from the **live** tool
  registry.
- `docs/delendai/examples/*` — adoption examples (minimal host, custom
  plugin, swarm).
- `scripts/*` — build, release, type/schema generation.
- `docs/delendai/agent-catalog.generated.json` — checked-in snapshot of
...
- Swarm proposals workflow. If a proposals task needs more than 3 tool
  ...
  `agentWorktree: false` in `delendai.config.json` (this repo's
...
- **No orphaned branches or stashes — always reconcile.** Before closing
  ...
  behaviour: delendai does not enforce it on other repos.
...
- **Optional: relax `exactOptionalPropertyTypes` (c00123).** The
  ...
  cryptic error — set `DELENDAI_RELAX_EXACT_OPTIONAL=1` before
...
Always read the audit playbook skill first (use `delendai_agent_catalog`
...
- Config files that STAY at root are the ones their tool/editor
  ...
  `stylelint.config.mjs`, `lefthook.yml`, `delendai.config.json`.
...
- A new root file must justify itself against the above; otherwise it
  belongs in `.github/`, `docs/delendai/`, `tools/`, `config/`, or
  under `.cache/`.
