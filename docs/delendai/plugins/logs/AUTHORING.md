...
If you only want to add a `delendai_logs_log` call to your own
...
The helper is the same writer `delendai_logs_log` uses, so an
emitted entry is queryable by `delendai_logs_query` /
`delendai_logs_search` / `delendai_logs_incidents` with the same
...
```bash
delendai --plugins=audit,quality,security --strict-logs
```
...
(`.cache/delendai/results/logs/*.jsonl`) are populated as if
...
add it explicitly (`delendai --plugins=my-plugin,logs`) or
