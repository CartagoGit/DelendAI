...
`@delendai/error-reporting` reports only DelendAI-owned diagnostic data
for failures that originate inside DelendAI itself.
...
`ISafeDelendaiReport` DTOs whose provenance has been resolved through the
...
`delendaiVersion` in the public DTO comes from the published
