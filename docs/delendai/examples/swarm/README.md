...
```jsonc
{
  "servers": {
    "delendai": {
      "command": "bunx",
      "args": ["--package", "@delendai/cli", "delendai", "__serve", "--workspace", ".", "--preset", "swarm"]
    }
  }
}
```
...
| `audit`           | Multi-model audit plugin (l99 alcance A): canonical brief generator + consolidator. **Not in the swarm preset** — opt-in per project, useful after a round to consolidate findings from N models. |

Optional config (`delendai.config.json` at the workspace root):

```jsonc
{
  "$schema": "./node_modules/@delendai/core/schema/delendai.config.schema.json",
...
1. `delendai_overview` → see the server + `recommendedNextAction`.
...
> [`delendai.config.json`](../../../../delendai.config.json) at the root.
