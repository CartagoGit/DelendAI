...
The smallest useful delendai server — orientation plus a couple of read-only
...
```jsonc
{
  "servers": {
    "delendai": {
      "command": "bunx",
      "args": ["--package", "@delendai/cli", "delendai", "__serve", "--workspace", ".", "--preset", "minimal"]
    }
  }
}
```

That's it — point your MCP client at it and call `delendai_overview` to map the
