/**
 * Example delendai plugin: `wordcount`.
 *
 ...
 *      from `delendai.config.json`. The loader rejects bad options BEFORE
 *      `register` runs, and `delendai --check` reports them.
 ...
 *
 * Run it:  `delendai --plugins=@delendai/example-wordcount`
 * (or point your client's `mcp.json` at the core bin with that `--plugins`).
 */
import z from 'zod';

import { definePlugin, toolJson } from '@delendai/core/public';
...
const buildWordcountTool = (
	...
	register: async (server) => {
		server.registerTool(
			...
			{
				description:
					'Count the words and characters in `text`. Pure, no I/O. (Example plugin showing the delendai contract.)',
