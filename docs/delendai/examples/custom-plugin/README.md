# Example: a custom delendai plugin (`wordcount`)
...
- **`optionsSchema`** (zod) — declarative validation of the options a host passes
  via `plugins.example-wordcount.options` in `delendai.config.json`. The loader
  rejects bad options *before* `register` runs, and `delendai --check` reports them.
