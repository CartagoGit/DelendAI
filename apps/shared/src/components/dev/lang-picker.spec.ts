 ...
 *
 * Contract pinned:
 *   - root is `<label class="delendai-lang-picker">` with a `<span>{caption}</span>`
 *     and a `<select name="...">`
...
describe('renderLangPicker', () => {
	it('emits the canonical label + select root', () => {
		...
		expect(out).toContain('<label class="delendai-lang-picker">');
	...
	it('applies the inline modifier when requested', () => {
		...
		expect(out).toContain(
			'class="delendai-lang-picker delendai-lang-picker--inline"',
		);
	});
