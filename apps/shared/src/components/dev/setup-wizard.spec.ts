 ...
 *     a `<section class="delendai-setup setup" data-kind="unconfigured">`
 ...
 *     `<p class="delendai-status-banner delendai-status-banner--ok">` shape
...
describe('renderSetupWizard', () => {
	it('emits the canonical section + signals + CTAs', () => {
		...
		expect(out).toContain(
			'<section class="delendai-setup setup" data-kind="unconfigured">',
		);
		expect(out).toContain(
		...
		expect(out).toContain(
			'<button type="button" id="setup-install" class="delendai-setup__primary setup__primary">',
		);
		expect(out).toContain(
			'<button type="button" id="setup-refresh" class="delendai-setup__secondary setup__secondary">',
		);
	});
	...
	it('renders one <li> per signal with the canonical classes', () => {
		...
		const matches = out.match(
			/<li class="delendai-setup__signal setup__signal (is-on|is-off)">/g,
		);
	...
	it('auto-derives the CTA label from kind', () => {
		...
		).toContain('Install delendai here');
	});

	it('honours an explicit ctaLabel override', () => {
		const out = renderSetupWizard(
			{ ...baseStatus, kind: 'unconfigured' },
			{ ctaLabel: 'Run `bunx delendai init`' },
		);
		expect(out).toContain('Run `bunx delendai init`');
		expect(out).not.toContain('Install delendai here');
	});

	it('honours a custom heading', () => {
		const out = renderSetupWizard(
			{ ...baseStatus, kind: 'unconfigured' },
			{ heading: 'Wire delendai in this project' },
		);
		expect(out).toContain('Wire delendai in this project');
		expect(out).not.toContain("delendai isn't fully wired");
	});
...
describe('renderStatusBanner', () => {
	...
	it('emits the warn variant for partial + unconfigured', () => {
		...
		).toContain('delendai-status-banner--warn');
		expect(
