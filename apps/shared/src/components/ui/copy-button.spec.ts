 ...
 *
 * Contract pinned:
 *   - root is `<button class="delendai-copybtn delendai-copybtn--{variant}" data-copy-text="..." aria-label="...">`
 *   - default variant is `ghost`
 *   - default label is `Copy`
 *   - icon glyph is the unicode `⧉` (U+29C9) in
 *     `<span class="delendai-copybtn__icon" aria-hidden="true">`
...
describe('renderCopyButton', () => {
	it('emits the canonical button root with the default variant + label', () => {
		...
		expect(out).toContain(
			'<button type="button" class="delendai-copybtn delendai-copybtn--ghost" data-copy-text="npm install x" aria-label="Copy">',
		);
	});
	...
	it('emits the icon glyph + label span', () => {
		...
		expect(out).toContain(
			'<span class="delendai-copybtn__label" data-copy-label="idle">Copy</span>',
		);
	});

	it('honours the solid variant', () => {
		const out = renderCopyButton({ text: 'x', variant: 'solid' });
		expect(out).toContain('delendai-copybtn--solid');
	});
	...
	it('honours a custom label', () => {
		...
		expect(out).toContain(
			'<span class="delendai-copybtn__label" data-copy-label="idle">Copiar</span>',
		);
	});
