// filepath: apps/shared/src/i18n/langs/zh.ts
//
// S2 — merged language dictionary for `zh`. Site section lifted
// from apps/web/src/i18n/langs/zh.ts with the proposals/recovery/logs
// references inlined from apps/web/src/i18n/{proposals,logs}.ts. Extension
// section lifted from extensions/vscode/src/i18n/langs/zh.ts. Tools
...
const site = {
	...
	install: {
		title: '安装与运行',
		lead: '添加它，并让你的 MCP 客户端指向 delendai 可执行文件：',
		verify: '验证可运行',
		addto: '添加到你的 IDE / 智能体',
		presets: '预设：',
		oneCmd: '一条命令 · 任意 IDE',
		oneCmdNote:
			'自动检测你的 IDE 并加入 delendai——绝不动你其它的 MCP 服务器。',
		...
		faqQ3: '可以同时运行多个预设吗？',
		faqA3: '不行——一个服务器对应一个预设。如果不同项目需要不同的插件集，可以在项目下放一个 `delendai.config.json`，加载器会优先读取它。',
	},
	tools: {
		title: '工具',
...
const extension = {
	...
	openLogsToday: 'delendai: 打开今天的日志',
	gitStatus: 'delendai: Git 状态',
	openMemory: 'delendai: 打开记忆搜索',
	notificationTest: 'delendai: 测试通知',
	depsCheck: 'delendai: 检查依赖',
	webFetch: 'delendai: 获取网页',
	...
	setupGithub: 'delendai：配置 GitHub issues',
};
...
const dev = {
	...
	firstRunHeading: '欢迎使用 delendai',
	firstRunLede:
		'此扩展为 delendai MCP 服务器提供仪表板、设置和工具面板。此工作区尚未安装 MCP 服务器 — 安装完成后,仪表板将切换为拉取真实数据。',
	firstRunSkip: '跳过 — 仍然显示仪表板',
	firstRunInstall: '在此工作区中安装 delendai',
};
