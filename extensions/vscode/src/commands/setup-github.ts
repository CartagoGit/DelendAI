/**
 * `delendai.setupGithub` — opens the multi-step setup-github webview
 * (f00030 S4). The webview mirrors the web wizard
 * (`apps/web/src/pages/setup.astro`) and the canonical 7-step guide in
 * `docs/delendai/CROSS-PROJECT-SETUP.md`: 7 steps, one per screen, each with a
 * Back / Next / Copy-command control. State lives in the webview; closing it
 * forgets the state (no persistence beyond what `delendai_issues_setup_github` writes
 * to disk).
 *
...
export const SETUP_GITHUB_COMMAND = 'delendai.setupGithub';

const SETUP_GITHUB_VIEW_TYPE = 'delendaiSetupGithub';

/** Key under which the host persists the user's preferred language. */
export const HOST_LANG_KEY = 'delendai:lang';
